#!/usr/bin/env bash
# Behavior tests for n1-fleet-sync.sh drift handling.
#
# n1-fleet-sync fast-forwards a clone that is cleanly on its default branch. This
...
TMP_ROOT=$(fm_test_tmproot n1-fleet-sync-tests)
HOME_N=0

# --- fixtures ---------------------------------------------------------------

# new_home: fresh isolated N1_HOME with an empty projects/ dir. Each test gets its
...
run_sync() {
  local home=$1
  shift
  N1_HOME="$home" N1_ROOT_OVERRIDE="$ROOT" "$ROOT/bin/n1-fleet-sync.sh" "$@" 2>/dev/null
}
...
test_bootstrap_relays_recovered_and_stuck() {
  ...
  # Full bootstrap: no state/ dir -> lieutenant sync no-ops; no .env -> X mode off.
  # We only assert the fleet-sync relay lines; other detect lines are irrelevant.
  out=$(N1_HOME="$home" N1_ROOT_OVERRIDE="$ROOT" "$ROOT/bin/n1-bootstrap.sh" 2>/dev/null)
