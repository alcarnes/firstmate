#!/usr/bin/env bash
# Tests for the local-HEAD lieutenant sync: every lieutenant home tracks the
# PRIMARY numberone checkout's current default-branch commit by a purely LOCAL
# fast-forward (no origin fetch). Two hook points drive it - bin/n1-spawn.sh
# (before launching a lieutenant) and bin/n1-bootstrap.sh (a startup sweep of
# every live lieutenant home) - and both share the ff machinery in
# bin/n1-ff-lib.sh.
#
...
#   - The bootstrap sweep fast-forwards every live lieutenant home and reports a
#     nudge (NUDGE_LIEUTENANTS:) ONLY for a running lieutenant whose instruction
#     surface actually changed; an already-current or readme-only home is never
#     nudged, a skipped home is reported as LIEUTENANT_SYNC:, and a home with no
#     live metadata is never swept.
#   - Spawning a lieutenant fast-forwards its worktree to the primary's HEAD
...
TMP_ROOT=$(fm_test_tmproot n1-lieutenant-sync)

# --- world builders --------------------------------------------------------

# new_world <name>: a PRIMARY numberone repo on `main` with one commit (the
...
new_world() {
  ...
  # Fresh watcher beacon keeps n1-guard quiet for the spawn path.
  touch "$w/home/state/.last-watcher-beat"

  git init -q -b main "$w/main"
  # Mirror the real repo: the gitignored operational dirs never dirty a worktree,
  # so a lieutenant home's data/state/projects can never block its fast-forward.
  printf 'projects/\nstate/\ndata/\n.no-mistakes/\nconfig/crew-harness\n' > "$w/main/.gitignore"
  printf 'v1\n' > "$w/main/AGENTS.md"
  printf 'r1\n' > "$w/main/README.md"
...
# add_sm_worktree <w> <id> <commit>: a lieutenant home as a DETACHED worktree of
# the primary at <commit>, plus its seed marker and a LIVE kind=lieutenant meta
...
# status line. Uses allow_detached=yes, ignore_seed_marker=yes (the lieutenant
# home contract).
FF_OUT=""
run_ff() {
  local dir=$1 base=$2 outfile="$TMP_ROOT/ff.out"
  ff_target "$dir" "lieutenant sm" "$base" yes yes >"$outfile" 2>&1
  FF_OUT=$(cat "$outfile")
}
...
test_ff_updated() {
  ...
  assert_contains "$FF_OUT" "lieutenant sm: updated " "updated home prints an advance line"
...
test_ff_current() {
  ...
  assert_contains "$FF_OUT" "lieutenant sm: already current" "current home reports already current"
...
test_ff_dirty() {
  ...
  assert_contains "$FF_OUT" "lieutenant sm: skipped: dirty working tree" "dirty home is skipped"
...
test_ff_diverged() {
  ...
  [ "$FF_STATUS" = skipped ] || fail "FF_STATUS: expected skipped, got '$FF_STATUS'"
  assert_contains "$FF_OUT" "lieutenant sm: skipped: diverged from $base" "diverged home is skipped"
...
# A lieutenant home carrying its own in-flight work sits on a named feature
...
test_ff_inflight_feature_branch() {
  ...
  [ "$FF_STATUS" = skipped ] || fail "FF_STATUS: expected skipped, got '$FF_STATUS'"
  assert_contains "$FF_OUT" "lieutenant sm: skipped: on feature/wip, expected main" \
...
test_sweep_nudge_requires_instruction_change() {
  ...
  N1_ROOT="$w/main" N1_HOME="$w/home"
  FF_NUDGE_WINDOWS=""
  FF_SEEN_HOMES=""
  sweep_live_lieutenant_metas "$w/home/state" "$base" yes >/dev/null
...
test_bootstrap_sweep_nudges_only_instruction_change() {
  ...
  printf 'sm-nonlive\n' > "$w/sm-nonlive/.n1-lieutenant-home"

  fakebin=$(make_fake_toolchain "$w")
  out=$(PATH="$fakebin:$BASE_PATH" N1_HOME="$w/home" N1_ROOT_OVERRIDE="$w/main" \
    "$ROOT/bin/n1-bootstrap.sh" 2>/dev/null)

  nudge_line=$(printf '%s\n' "$out" | grep '^NUDGE_LIEUTENANTS:' || true)
  [ -n "$nudge_line" ] || fail "no NUDGE_LIEUTENANTS line emitted (got: $out)"
  assert_contains "$nudge_line" "numberone:n1-sm-instr" "instruction-changed running lieutenant is nudged"
  assert_not_contains "$nudge_line" "sm-readme" "readme-only advance is not nudged"
  assert_not_contains "$nudge_line" "sm-current" "already-current lieutenant is not nudged"
  ...
  pass "T8 bootstrap sweeps live homes, nudges only the running real-instruction-change lieutenant"
}

# --- T9: bootstrap surfaces a skipped dirty live lieutenant home --------------
test_bootstrap_sweep_surfaces_skipped_home() {
  ...
  skip_line=$(printf '%s\n' "$out" | grep '^LIEUTENANT_SYNC: lieutenant sm-dirty: skipped:' || true)
  [ -n "$skip_line" ] || fail "no LIEUTENANT_SYNC skip line emitted (got: $out)"
  ...
  pass "T9 bootstrap surfaces a skipped dirty live lieutenant home"
}

# --- T10: spawning a lieutenant fast-forwards its worktree before launch ------
test_spawn_fast_forwards_before_launch() {
  local w c1 c2 fakebin
  w=$(new_world spawn-ff)
  c1=$(head_of "$w/main")
  git -C "$w/main" worktree add -q --detach "$w/sm" "$c1"
  printf 'sm\n' > "$w/sm/.n1-lieutenant-home"
  mkdir -p "$w/sm/data"
...
SH
  ...
  PATH="$fakebin:$BASE_PATH" TMUX='' \
    N1_ROOT_OVERRIDE="$w/main" N1_HOME="$w/home" \
    N1_STATE_OVERRIDE="$w/home/state" N1_DATA_OVERRIDE="$w/home/data" \
    N1_PROJECTS_OVERRIDE="$w/home/projects" N1_CONFIG_OVERRIDE="$w/home/config" \
    N1_SPAWN_NO_GUARD=1 \
    "$ROOT/bin/n1-spawn.sh" sm "$w/sm" codex --lieutenant >/dev/null 2>&1 || true

  [ "$(head_of "$w/sm")" = "$c2" ] \
    || fail "spawn did not fast-forward the lieutenant worktree to the primary's HEAD"
  pass "T10 spawn fast-forwards a lieutenant worktree to the primary's local HEAD before launch"
}
...
SH
  ...
  PATH="$fakebin:$BASE_PATH" TMUX='' \
    ...
    N1_PROJECTS_OVERRIDE="$w/home/projects" N1_CONFIG_OVERRIDE="$w/home/config" \
    N1_SPAWN_NO_GUARD=1 \
    "$ROOT/bin/n1-spawn.sh" sm "$w/sm" codex --lieutenant >/dev/null 2>"$err" || true

  assert_contains "$(cat "$err")" \
    "warning: lieutenant sm sync skipped before launch: dirty working tree" \
...
echo "# all n1-lieutenant-sync tests passed"
