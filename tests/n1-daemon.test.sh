#!/usr/bin/env bash
# tests/n1-daemon.test.sh - supervise-daemon classifiers, the captain-relevant
...
# n1-send swallow reporting, composer-pending ANSI parsing). The operator-visible
# inject flow lives in n1-afk-inject-e2e and n1-wake-daemon-lifecycle-e2e.
set -u
...
TMP_ROOT=$(fm_test_tmproot n1-daemon-tests)


test_daemon_state_root_uses_fm_home() {
  local dir home override out
  dir=$(make_supercase daemon-n1-home)
  home="$dir/numberone-home"
  override="$dir/override-state"
  mkdir -p "$home" "$override"

  out=$(N1_HOME="$home" N1_STATE_OVERRIDE='' _state_root)
  [ "$out" = "$home/state" ] || fail "daemon state root ignored N1_HOME: $out"

  out=$(N1_HOME="$home" N1_STATE_OVERRIDE="$override" _state_root)
  [ "$out" = "$override" ] || fail "daemon state root ignored N1_STATE_OVERRIDE: $out"

  pass "supervise daemon state root is scoped by N1_HOME"
}
...
test_classify_routine_signal_self() {
  ...
  out=$(N1_STATE_OVERRIDE="$state" classify_signal "$state/foo-x1.status" "$state")
...
test_classify_terminal_signal_escalates() {
  ...
  for kw in "done: PR https://x/y/pull/1" "needs-decision: pick A" "blocked: no perms" \
    ...
    printf 'working\n%s\n' "$kw" > "$state/t.status"
    out=$(N1_STATE_OVERRIDE="$state" classify_signal "$state/t.status" "$state")
...
test_stale_transient_self_records_marker() {
  ...
  stale_marker_record "sess:n1-qux-w4" "$state"
  out=$(N1_STATE_OVERRIDE="$state" classify_stale "sess:n1-qux-w4" "$state")
  case "$out" in self\|*) ;; *) fail "transient stale did not self-handle: $out" ;; esac
  key=$(printf '%s' "$(window_to_task "sess:n1-qux-w4")" | tr ':/.' '___')
...
test_stale_terminal_escalates() {
  ...
  out=$(N1_STATE_OVERRIDE="$state" classify_stale "sess:n1-fin-t5" "$state")
...
test_housekeeping_persistent_stale_escalates() {
  ...
  PATH="$fakebin:$PATH" N1_FAKE_TMUX_WINDOW="$win" N1_FAKE_TMUX_CAPTURE="$pane" \
...
test_escalate_batches_into_one_digest() {
  ...
  PATH="$fakebin:$PATH" N1_FAKE_TMUX_PANE_ALIVE=1 N1_FAKE_TMUX_SENT="$sent" \
    N1_FAKE_TMUX_CAPTURE="$capture" N1_ESCALATE_BATCH_SECS=0 escalate_flush "$state" \
...
test_escalate_batch_age_uses_first_append() {
  ...
  afk_enter "$state"
  PATH="$fakebin:$PATH" N1_FAKE_TMUX_PANE_ALIVE=1 N1_FAKE_TMUX_SENT="$sent" \
    N1_FAKE_TMUX_CAPTURE="$capture" N1_ESCALATE_BATCH_SECS=90 N1_HOUSEKEEPING_TICK=0 \
    housekeeping "$state"
  grep -F 'event A: done: PR 1 | event B: done: PR 2' "$sent" >/dev/null \
...
test_handle_wake_routes_self_and_escalate() {
  ...
  N1_STATE_OVERRIDE="$state" handle_wake "signal: $state/h-routine.status" "$state"
  [ -s "$state/.subsuper-escalations" ] && fail "routine signal was escalated by handle_wake"
  printf 'done: PR 1\n' > "$state/h-done.status"
  N1_STATE_OVERRIDE="$state" handle_wake "signal: $state/h-done.status" "$state"
...
test_inject_skip_forces_self() {
  ...
  state="$dir/state"
  printf 'done: PR 1\n' > "$state/s1.status"
  N1_STATE_OVERRIDE="$state" N1_INJECT_SKIP="signal" handle_wake "signal: $state/s1.status" "$state"
...
test_is_wake_reason_distinguishes_status_stdout() {
  ...
  is_wake_reason "stale: s:n1-x" || fail "stale: not recognized as wake"
...
test_terminal_stale_escalate_leaves_no_marker() {
  ...
  echo $(( $(date +%s) - 500 )) > "$state/.subsuper-stale-$key"
  N1_STATE_OVERRIDE="$state" handle_wake "stale: $win" "$state"
...
test_signal_escalate_marks_seen_no_catchall_refire() {
  ...
  N1_STATE_OVERRIDE="$state" handle_wake "signal: $state/sig-t8.status" "$state"
...
test_afk_absent_daemon_does_not_inject() {
  ...
  if PATH="$fakebin:$PATH" N1_FAKE_TMUX_PANE_ALIVE=1 N1_FAKE_TMUX_SENT="$sent" \
    N1_FAKE_TMUX_CAPTURE="$capture" N1_ESCALATE_BATCH_SECS=0 escalate_flush "$state"; then
...
test_busy_guard_defers_when_supervisor_busy() {
  ...
  # pane shows a busy signature (numberone mid-turn)
...
test_marker_detection() {
  ...
  message_is_injection "${N1_INJECT_MARK}Supervisor escalate: done" \
  ...
  state="$dir/state"
  afk_enter "$state"
  should_exit_afk "$state" "${N1_INJECT_MARK}escalate" \
...
test_should_exit_afk_when_afk_inactive() {
  ...
  should_exit_afk "$state" "${N1_INJECT_MARK}test" \
...
test_strip_injection_marker() {
  local stripped
  stripped=$(strip_injection_marker "${N1_INJECT_MARK}Supervisor escalate: done")
  ...
  # Only marker → empty.
  stripped=$(strip_injection_marker "$N1_INJECT_MARK")
...
test_pane_input_pending_detects_partial_input() {
  ...
  printf 'line one\nline two\nhuman draft text\n' > "$capture"
  PATH="$fakebin:$PATH" N1_FAKE_TMUX_CAPTURE="$capture" N1_FAKE_TMUX_CURSOR_Y=2 \
...
test_pane_input_pending_honors_idle_override_after_border_strip() {
  ...
  capture="$dir/pane.txt"
  printf '│ custom idle> │\n' > "$capture"
  PATH="$fakebin:$PATH" N1_FAKE_TMUX_CAPTURE="$capture" N1_FAKE_TMUX_CURSOR_Y=0 \
    N1_COMPOSER_IDLE_RE='^custom idle>$' pane_input_pending "fakepane" \
    && fail "N1_COMPOSER_IDLE_RE was not applied after border stripping"
  pass "pane_input_pending honors N1_COMPOSER_IDLE_RE after border stripping"
}
...
test_classify_signal_dedup_against_scan() {
  ...
  out=$(N1_STATE_OVERRIDE="$state" classify_signal "$state/dup-s9.status" "$state")
...
test_classify_stale_dedup_against_signal() {
  ...
  out=$(N1_STATE_OVERRIDE="$state" classify_stale "sess:n1-dup-s10" "$state")
...
test_pane_input_pending_bordered_idle_not_pending() {
  ...
  for line in \
    ...
    "│                                              │"; do
    printf '%s\n' "$line" > "$capture"
    if PATH="$fakebin:$PATH" N1_FAKE_TMUX_CAPTURE="$capture" N1_FAKE_TMUX_CURSOR_Y=0 \
...
test_submit_ack_confirms_on_bordered_empty_composer() {
  ...
  verdict=$(PATH="$fakebin:$PATH" N1_FAKE_COMPOSER="$dir/composer" N1_FAKE_SENT="$sent" \
...
test_submit_ack_reports_pending_on_persistent_swallow() {
  ...
  # reported as "pending" — the daemon keeps the buffer, n1-send exits non-zero —
  ...
  touch "$dir/.swallow"
  verdict=$(PATH="$fakebin:$PATH" N1_FAKE_COMPOSER="$dir/composer" N1_FAKE_SENT="$sent" \
    N1_FAKE_SWALLOW="$dir/.swallow" N1_FAKE_PERSIST_SWALLOW=1 \
    fm_tmux_submit_core "win" "the digest" 3 0.05 0.05)
...
test_max_defer_empty_swallow_types_once_and_alarms() {
  ...
  afk_enter "$state"
  PATH="$fakebin:$PATH" N1_FAKE_COMPOSER="$dir/composer" N1_FAKE_SENT="$sent" \
    N1_FAKE_SWALLOW="$dir/.swallow" N1_FAKE_PERSIST_SWALLOW=1 N1_INJECT_CONFIRM_SLEEP=0.05 \
    N1_ESCALATE_BATCH_SECS=99999 N1_MAX_DEFER_SECS=60 housekeeping "$state"
...
test_max_defer_flushes_empty_idle_pane() {
  ...
  afk_enter "$state"
  PATH="$fakebin:$PATH" N1_FAKE_COMPOSER="$dir/composer" N1_FAKE_SENT="$sent" \
    N1_ESCALATE_BATCH_SECS=99999 N1_MAX_DEFER_SECS=60 N1_INJECT_CONFIRM_SLEEP=0.05 \
    housekeeping "$state"
...
test_normal_flush_clears_stale_wedge_marker() {
  ...
  afk_enter "$state"
  PATH="$fakebin:$PATH" N1_FAKE_COMPOSER="$dir/composer" N1_FAKE_SENT="$sent" \
    N1_INJECT_CONFIRM_SLEEP=0.05 escalate_flush "$state" \
...
test_below_max_defer_does_nothing() {
  ...
  afk_enter "$state"
  PATH="$fakebin:$PATH" N1_FAKE_TMUX_PANE_ALIVE=1 N1_FAKE_TMUX_SENT="$sent" \
    N1_FAKE_TMUX_CAPTURE="$capture" N1_FAKE_TMUX_CURSOR_Y=0 \
    N1_ESCALATE_BATCH_SECS=99999 N1_MAX_DEFER_SECS=300 housekeeping "$state"
...
test_fm_send_exits_nonzero_on_confirmed_swallow() {
  # n1-send.sh must exit NON-ZERO when a steer's Enter is positively swallowed
  # (text left in the composer), so numberone learns the instruction did not land
  ...
  # Clean submit -> exit 0.
  PATH="$fakebin:$PATH" N1_STATE_OVERRIDE="$dir/state" N1_FAKE_COMPOSER="$dir/composer" \
    N1_SEND_SLEEP=0.05 "$ROOT/bin/n1-send.sh" sess:win 'route this work' >/dev/null 2>"$err" \
    || fail "n1-send exited non-zero on a clean submit: $(cat "$err")"
  # Persistent swallow -> exit non-zero with a clear message.
  printf '│ > │\n' > "$dir/composer"
  touch "$dir/.swallow"
  if PATH="$fakebin:$PATH" N1_STATE_OVERRIDE="$dir/state" N1_FAKE_COMPOSER="$dir/composer" \
    N1_FAKE_SWALLOW="$dir/.swallow" N1_FAKE_PERSIST_SWALLOW=1 N1_SEND_SLEEP=0.05 \
    "$ROOT/bin/n1-send.sh" sess:win 'fix findings 1 and 3, skip 2' >/dev/null 2>"$err"; then
    fail "n1-send exited zero despite a swallowed Enter (silent unsubmitted instruction)"
  fi
  grep -F 'not submitted' "$err" >/dev/null || fail "n1-send did not explain the swallowed submit: $(cat "$err")"
  pass "n1-send exits non-zero on a confirmed swallow, zero on a clean submit"
}
...
test_fm_send_exits_nonzero_on_initial_send_failure() {
  ...
  fakebin="$dir/fakebin"; err="$dir/send.err"
  if PATH="$fakebin:$PATH" N1_STATE_OVERRIDE="$dir/state" N1_FAKE_COMPOSER="$dir/composer" \
    N1_FAKE_SEND_FAIL=1 N1_SEND_SLEEP=0.05 \
    "$ROOT/bin/n1-send.sh" sess:win 'route this work' >/dev/null 2>"$err"; then
    fail "n1-send exited zero despite initial tmux send-keys failure"
  fi
  grep -F 'text not sent' "$err" >/dev/null || fail "n1-send did not explain initial send failure: $(cat "$err")"
  pass "n1-send exits non-zero when initial text send fails"
}
