...
#   3. The human/LLM-facing capture path (n1-peek.sh) stays PLAIN - no escape codes
#      ever reach numberone's context.
set -u

# shellcheck source=tests/lib.sh
. "$(dirname "${BASH_SOURCE[0]}")/lib.sh"

LIB="$ROOT/bin/n1-tmux-lib.sh"
PEEK="$ROOT/bin/n1-peek.sh"

# shellcheck source=bin/n1-tmux-lib.sh
. "$LIB"

TMP_ROOT=$(fm_test_tmproot n1-ghost-tests)
...
test_dim_ghost_only_composer_is_not_pending() {
  ...
  if PATH="$fb:$PATH" N1_FAKE_STYLED="$capture" N1_FAKE_CY=0 \
...
test_normal_text_still_pending() {
  ...
  PATH="$fb:$PATH" N1_FAKE_STYLED="$capture" N1_FAKE_CY=0 \
...
# --- n1-peek.sh stays escape-free (LLM-facing path) -------------------------
...
test_peek_output_is_escape_free() {
  ...
  # surface NONE of these escape codes into numberone's context.
  printf 'normal output line\n\xe2\x9d\xaf \033[2mpredicted next prompt\033[0m\n' > "$capture"
  # Empty N1_HOME so n1-guard.sh finds no in-flight task and stays silent.
  home="$dir/home"; mkdir -p "$home/state"
  # Pass an explicit session:window so resolution needs no metadata.
  out=$(PATH="$fb:$PATH" N1_HOME="$home" N1_FAKE_STYLED="$capture" \
        "$PEEK" "sess:win" 2>/dev/null)
  case "$out" in
    *"$ESC"*) fail "n1-peek surfaced ANSI escape codes into LLM-facing output" ;;
  esac
  # And it should still carry the real content.
  case "$out" in
    *"predicted next prompt"*) : ;;
    *) fail "n1-peek dropped pane content (expected the ghost text body as plain text)" ;;
  esac
  pass "n1-peek output is escape-free (no raw -e bytes reach numberone context)"
}
