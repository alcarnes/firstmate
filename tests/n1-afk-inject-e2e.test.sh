#!/usr/bin/env bash
# tests/n1-afk-inject-e2e.test.sh - private-socket end-to-end test for the afk
...
# private socket. The daemon points at a throwaway state dir (N1_STATE_OVERRIDE)
# and the test pane (N1_SUPERVISOR_TARGET). Nothing touches the live fleet.
#
...
STATE_DIR=$(mktemp -d "${TMPDIR:-/tmp}/n1-afk-e2e.XXXXXX")
mkdir -p "$STATE_DIR"
LOG_FILE="$STATE_DIR/submitted.log"
: > "$LOG_FILE"

# Source the daemon to get N1_INJECT_MARK, afk_enter, afk_exit.
...
TMUX_SHIM_DIR=$(mktemp -d "${TMPDIR:-/tmp}/n1-shim.XXXXXX")
...
# Create a fake ensign window (the watcher lists n1-* windows for stale
# detection). The pane is an inert shell — it just needs to exist.
"$REAL_TMUX" -L "$SOCKET" new-window -d -n n1-fake-c1 -t supervisor

start_daemon() {
  PATH="$TMUX_SHIM_DIR:$PATH" \
  N1_STATE_OVERRIDE="$STATE_DIR" \
  N1_SUPERVISOR_TARGET="$SUPERVISOR_PANE" \
  N1_ESCALATE_BATCH_SECS=0 \
  N1_HOUSEKEEPING_TICK=1 \
  N1_POLL=1 \
  N1_SIGNAL_GRACE=1 \
  N1_HEARTBEAT=999999 \
  N1_CHECK_INTERVAL=999999 \
  N1_INJECT_CONFIRM_SLEEP=0.3 \
  N1_INJECT_CONFIRM_RETRIES=5 \
  N1_STALE_ESCALATE_SECS=999999 \
