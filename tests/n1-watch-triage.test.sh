#!/usr/bin/env bash
# tests/n1-watch-triage.test.sh - the always-on wake triage built into
# bin/n1-watch.sh and the shared classifier (bin/n1-classify-lib.sh). The watcher
# now absorbs the benign majority of wakes in bash and exits ONLY on an actionable
# wake, so numberone's LLM re-arms once per actionable event instead of once per
# wake. These tests cover the classifier predicates as pure functions, then drive
# a real n1-watch.sh subprocess to assert the behavioral contract: benign absorbed
...
#
# Daemon-side classification/injection lives in n1-daemon.test.sh; watcher/lock
# liveness in n1-watcher-lock.test.sh; the durable-queue safety matrix in
# n1-wake-queue.test.sh.
set -u

# shellcheck source=tests/wake-helpers.sh
. "$(dirname "${BASH_SOURCE[0]}")/wake-helpers.sh"
# shellcheck source=bin/n1-classify-lib.sh
. "$ROOT/bin/n1-classify-lib.sh"

WATCH="$ROOT/bin/n1-watch.sh"
DRAIN="$ROOT/bin/n1-wake-drain.sh"

TMP_ROOT=$(fm_test_tmproot n1-watch-triage-tests)
...
# fallback (which writes a partial filesystem dump on Linux; see n1-watch.sh).
file_mtime() {
  if [ "$(uname)" = Darwin ]; then stat -f %m "$1" 2>/dev/null; else stat -c %Y "$1" 2>/dev/null; fi
}

# Signature a primed .seen-* marker must hold so the per-poll signal scan does not
# fire on a pre-existing status (mirrors n1-watch.sh's stat_sig exactly).
seen_sig() {
...
# --- pure classifier predicates (n1-classify-lib.sh) ------------------------
...
test_stale_is_terminal_classifier() {
  ...
  stale_is_terminal "sess:n1-term" "$state" || fail "terminal stale status not classified terminal"
  printf 'working: compiling\n' > "$state/nonterm.status"
  stale_is_terminal "sess:n1-nonterm" "$state" && fail "non-terminal stale classified terminal"
  stale_is_terminal "sess:n1-missing" "$state" && fail "stale with no status classified terminal"
...
test_classifier_primitives() {
  ...
  [ "$(window_to_task "sess:n1-fix-login-k3")" = "fix-login-k3" ] || fail "window_to_task did not strip session+n1- prefix"
  N1_CAPTAIN_RE='custom-verb:' status_is_captain_relevant "custom-verb: x" || fail "N1_CAPTAIN_RE override not honored"
  N1_CAPTAIN_RE='custom-verb:' status_is_captain_relevant "done: x" && fail "N1_CAPTAIN_RE override did not replace the default verb set"
  pass "classifier primitives: last line, captain-relevance, window->task, N1_CAPTAIN_RE override"
}
...
test_actionable_signal_surfaced() {
  ...
  N1_STATE_OVERRIDE="$state" "$DRAIN" > "$drain_out" 2>/dev/null || fail "drain after the actionable signal failed"
...
test_terminal_stale_surfaced() {
  ...
  printf '1\n' > "$state/.count-$key"
  PATH="$fakebin:$PATH" N1_FAKE_TMUX_WINDOW="$window" N1_FAKE_TMUX_CAPTURE="$capture_file" \
    N1_STATE_OVERRIDE="$state" N1_POLL=1 N1_SIGNAL_GRACE=1 N1_CHECK_INTERVAL=999999 N1_HEARTBEAT=999999 "$WATCH" > "$out" &
  pid=$!
  ...
  N1_STATE_OVERRIDE="$state" "$DRAIN" > "$drain_out" 2>/dev/null || fail "drain after the terminal stale failed"
...
test_nonterminal_stale_absorbed_then_escalated() {
  ...
  PATH="$fakebin:$PATH" N1_FAKE_TMUX_WINDOW="$window" N1_FAKE_TMUX_CAPTURE="$capture_file" \
    N1_STATE_OVERRIDE="$state" N1_STALE_ESCALATE_SECS=999 N1_POLL=1 N1_SIGNAL_GRACE=1 \
    N1_CHECK_INTERVAL=999999 N1_HEARTBEAT=999999 "$WATCH" > "$out" &
  pid=$!
  if ! wait_live "$pid" 30; then
  ...
  : > "$out"
  PATH="$fakebin:$PATH" N1_FAKE_TMUX_WINDOW="$window" N1_FAKE_TMUX_CAPTURE="$capture_file" \
    N1_STATE_OVERRIDE="$state" N1_STALE_ESCALATE_SECS=240 N1_POLL=1 N1_SIGNAL_GRACE=1 \
    N1_CHECK_INTERVAL=999999 N1_HEARTBEAT=999999 "$WATCH" > "$out" &
  pid=$!
  ...
  N1_STATE_OVERRIDE="$state" "$DRAIN" > "$drain_out" 2>/dev/null || fail "drain after the wedge escalation failed"
...
SH
  ...
  printf 'working: compiling step 2\n' > "$status_file"
  PATH="$fakebin:$PATH" N1_STATE_OVERRIDE="$state" N1_POLL=1 N1_SIGNAL_GRACE=1 \
    N1_CHECK_INTERVAL=999999 N1_HEARTBEAT=999999 N1_WATCH_TRIAGE_LOG_MAX_BYTES=1 "$WATCH" > "$out" &
  pid=$!
  if ! wait_live "$pid" 30; then
...
test_heartbeat_no_change_absorbed() {
  ...
  PATH="$fakebin:$PATH" N1_STATE_OVERRIDE="$state" N1_POLL=1 N1_SIGNAL_GRACE=1 \
    N1_CHECK_INTERVAL=999999 N1_HEARTBEAT=1 "$WATCH" > "$out" &
  pid=$!
  if ! wait_live "$pid" 30; then
...
test_heartbeat_backstop_surfaces_unsurfaced_status() {
  ...
  # fleet-scan backstop must catch it and wake numberone.
  ...
  N1_STATE_OVERRIDE="$state" "$DRAIN" > "$drain_out" 2>/dev/null || fail "drain after the backstop heartbeat failed"
...
test_beacon_stays_fresh_while_absorbing() {
  ...
  reap "$pid"
  pass "the liveness beacon stays fresh while the watcher absorbs benign wakes (n1-guard never false-alarms)"
}
...
test_afk_present_reverts_watcher_to_one_shot() {
  ...
  N1_STATE_OVERRIDE="$state" "$DRAIN" > "$drain_out" 2>/dev/null || fail "drain after the afk-mode signal failed"
