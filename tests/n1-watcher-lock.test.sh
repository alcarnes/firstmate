#!/usr/bin/env bash
# tests/n1-watcher-lock.test.sh - watcher singleton + lock-primitive races +
...
WATCH_ARM="$ROOT/bin/n1-watch-arm.sh"
DRAIN="$ROOT/bin/n1-wake-drain.sh"
LIB="$ROOT/bin/n1-wake-lib.sh"

TMP_ROOT=$(fm_test_tmproot n1-watcher-lock-tests)
...
test_singleton_start() {
  ...
  PATH="$fakebin:$PATH" N1_STATE_OVERRIDE="$state" N1_POLL=5 N1_SIGNAL_GRACE=1 N1_CHECK_INTERVAL=999999 N1_HEARTBEAT=999999 "$WATCH" > "$out1" &
  pid1=$!
  PATH="$fakebin:$PATH" N1_STATE_OVERRIDE="$state" N1_POLL=5 N1_SIGNAL_GRACE=1 N1_CHECK_INTERVAL=999999 N1_HEARTBEAT=999999 "$WATCH" > "$out2" &
  pid2=$!
  sleep 0.5
  live=0
...
test_stale_watch_lock_reclaimed() {
  ...
  PATH="$fakebin:$PATH" N1_STATE_OVERRIDE="$state" N1_POLL=5 N1_SIGNAL_GRACE=1 N1_CHECK_INTERVAL=999999 N1_HEARTBEAT=999999 "$WATCH" > "$out" &
  pid=$!
  sleep 0.5
  live=0
...
test_live_stale_watch_lock_is_actionable() {
  ...
  status=0
  PATH="$fakebin:$PATH" N1_STATE_OVERRIDE="$state" N1_GUARD_GRACE=1 N1_POLL=5 N1_SIGNAL_GRACE=1 N1_CHECK_INTERVAL=999999 N1_HEARTBEAT=999999 "$WATCH" > "$out" 2> "$err" || status=$?
...
test_guard_warnings() {
  ...
  # N1_ROOT_OVERRIDE points the worktree-tangle check at a non-git dir so it stays
  ...
  N1_ROOT_OVERRIDE="$dir" N1_STATE_OVERRIDE="$state" N1_GUARD_GRACE=1 "$ROOT/bin/n1-guard.sh" 2> "$err" >/dev/null || fail "guard failed"
  ...
  grep -F 'bin/n1-watch-arm.sh' "$err" >/dev/null || fail "guard banner missing the fix command"
  ...
  # Non-git N1_ROOT keeps the worktree-tangle check inert so "fresh watcher ->
  # total silence" stays a pure assertion about watcher state.
  N1_ROOT_OVERRIDE="$dir" N1_STATE_OVERRIDE="$state" N1_GUARD_GRACE=300 "$ROOT/bin/n1-guard.sh" 2> "$err" >/dev/null || fail "guard failed"
...
test_lock_single_winner_under_concurrency() {
  ...
  i=1
  while [ "$i" -le 40 ]; do
    N1_STATE_OVERRIDE="$state" bash -c '
      . "$1"
...
test_lock_steals_dead_pid_lock() {
  ...
  printf '%s\n' "$dead" > "$lockdir/pid"
  rc=0
  newpid=$(N1_STATE_OVERRIDE="$state" bash -c '
    . "$1"
...
test_lock_live_steal_mutex_is_not_reclaimed() {
  ...
  out=$(N1_LOCK_STALE_AFTER=0 N1_STATE_OVERRIDE="$state" bash -c '
    . "$1"
    if fm_lock_try_acquire "$2"; then rc=0; else rc=1; fi
    printf "rc=%s held=%s lockpid=%s stealpid=%s\n" "$rc" "${N1_LOCK_HELD_PID:-}" "$(cat "$2/pid" 2>/dev/null || true)" "$(cat "$2.steal/pid" 2>/dev/null || true)"
  ' _ "$LIB" "$lockdir")
...
test_lock_does_not_steal_live_lock() {
  ...
  mkdir "$lockdir"
  printf '%s\n' "$live" > "$lockdir/pid"
  out=$(N1_STATE_OVERRIDE="$state" bash -c '
    . "$1"
    if fm_lock_try_acquire "$2"; then rc=0; else rc=1; fi
    printf "rc=%s held=%s\n" "$rc" "${N1_LOCK_HELD_PID:-}"
  ' _ "$LIB" "$lockdir")
  ...
  case "$out" in
    *"held=$live"*) ;;
    *) fail "live holder pid not reported via N1_LOCK_HELD_PID: $out" ;;
  esac
...
test_watch_restart_rejects_reused_pid() {
  ...
  printf '%s\n' "$dir" > "$state/.watch.lock/n1-home"
  printf '%s\n' "$WATCH" > "$state/.watch.lock/watcher-path"
  printf '%s\n' "stale watcher identity" > "$state/.watch.lock/pid-identity"
  PATH="$fakebin:$PATH" N1_HOME="$dir" N1_POLL=5 N1_SIGNAL_GRACE=1 N1_CHECK_INTERVAL=999999 N1_HEARTBEAT=999999 "$WATCH_ARM" --restart > "$out" &
  pid=$!
...
test_arm_reports_healthy_for_live_fresh_watcher() {
  ...
  status=0
  PATH="$fakebin:$PATH" N1_STATE_OVERRIDE="$state" "$WATCH_ARM" > "$armout" || status=$?
...
test_arm_starts_and_self_heals() {
  ...
  for row in clean dead-pid; do
    ...
    PATH="$fakebin:$PATH" N1_HOME="$dir" N1_POLL=5 N1_SIGNAL_GRACE=1 N1_CHECK_INTERVAL=999999 N1_HEARTBEAT=999999 "$WATCH_ARM" > "$armout" &
    armpid=$!
    i=0
    while [ "$i" -lt 80 ]; do
...
test_arm_hup_cleans_child_and_temp_output() {
  ...
  fakebin="$dir/fakebin"
  armout="$dir/arm.out"
  PATH="$fakebin:$PATH" N1_STATE_OVERRIDE="$state" N1_POLL=5 N1_SIGNAL_GRACE=1 N1_CHECK_INTERVAL=999999 N1_HEARTBEAT=999999 "$WATCH_ARM" > "$armout" &
  armpid=$!
  i=0
  while [ "$i" -lt 80 ]; do
    grep -qF 'watcher: started pid=' "$armout" 2>/dev/null && break
...
SH
  chmod +x "$check_file"
  rc=0
  PATH="$fakebin:$PATH" N1_STATE_OVERRIDE="$state" N1_GUARD_GRACE=0 N1_POLL=5 N1_SIGNAL_GRACE=1 N1_CHECK_INTERVAL=0 N1_HEARTBEAT=999999 "$WATCH_ARM" > "$armout" || rc=$?
  ...
  N1_STATE_OVERRIDE="$state" "$DRAIN" > "$drain_out" || fail "drain after immediate arm wake failed"
...
test_arm_waits_for_peer_beacon_after_child_stands_down() {
  ...
  sleep 300 &
  peer=$!
  identity=$(N1_STATE_OVERRIDE="$state" bash -c '. "$1"; fm_pid_identity "$2"' _ "$LIB" "$peer") || fail "could not identify peer pid"
  mkdir "$state/.watch.lock"
  ...
  beater=$!
  status=0
  PATH="$fakebin:$PATH" N1_HOME="$dir" N1_POLL=5 N1_SIGNAL_GRACE=1 N1_CHECK_INTERVAL=999999 N1_HEARTBEAT=999999 N1_ARM_CONFIRM_TIMEOUT=4 "$WATCH_ARM" > "$armout" || status=$?
...
test_arm_fails_loud_when_no_fresh_watcher_confirmable() {
  ...
  printf '%s\n' "$live" > "$state/.watch.lock/pid"
  touch -t 200001010000 "$state/.last-watcher-beat"
  PATH="$fakebin:$PATH" N1_STATE_OVERRIDE="$state" N1_POLL=5 N1_SIGNAL_GRACE=1 N1_CHECK_INTERVAL=999999 N1_HEARTBEAT=999999 N1_ARM_CONFIRM_TIMEOUT=3 "$WATCH_ARM" > "$armout" &
  armpid=$!
