#!/usr/bin/env bash
# n1-send pre-submit popup-settle selection (the codex `$<skill>` fix).
#
...
# popup settles lets it swallow the Enter, so the line never submits. n1-send
...
# n1-send's own post-submit N1_SEND_SETTLE pause. So tail-vs-head matters: this
# suite asserts on the HEAD sleep, distinct from n1-send-settle.test.sh which pins
...
TMP_ROOT=$(fm_test_tmproot n1-send-popup-settle)

# Same stub shape as n1-send-settle.test.sh: a fake tmux that drives the submit
# path to a clean "empty" verdict on the first Enter, and a fake sleep that records
# every requested duration (one per line) into N1_SLEEP_LOG instead of sleeping.
...
# (the popup-settle) equals <expected>. N1_SEND_SETTLE=0 strips the trailing
# post-submit pause so the log holds only the popup-settle plus the 0.4 Enter wait,
# keeping the head assertion crisp. N1_ROOT_OVERRIDE points at a non-repo dir so
# n1-guard's tangle check stays silent; its watcher-liveness note goes to stderr
# (discarded).
first_settle() {  # <expected> <label> <harness|--explicit> <message>
  ...
  : > "$log"
  env N1_SEND_SETTLE=0 PATH="$fb:$PATH" \
  ...
  pass "n1-send popup-settle: $label -> ${expected}s"
}
