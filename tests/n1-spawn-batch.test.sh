#!/usr/bin/env bash
# Behavior tests for n1-spawn.sh batch dispatch (`id=repo` pairs).
#
...
# the tests create no windows or worktrees. N1_SPAWN_NO_GUARD=1 keeps them off the
...
SPAWN="$ROOT/bin/n1-spawn.sh"
TMP_ROOT=$(fm_test_tmproot n1-spawn-batch)

# Clear ambient numberone overrides so the behavior test owns its environment.
run_spawn() {
  N1_ROOT_OVERRIDE='' \
    N1_HOME='' \
    N1_STATE_OVERRIDE='' \
    N1_DATA_OVERRIDE='' \
    N1_PROJECTS_OVERRIDE='' \
    N1_CONFIG_OVERRIDE='' \
    N1_SPAWN_NO_GUARD=1 \
    "$SPAWN" "$@" 2>&1
}
...
# A projects/ path is resolved through the numberone home, never the caller cwd,
...
test_projects_path_scoping() {
  ...
  while IFS='|' read -r label use_override id; do
    ...
    if [ "$use_override" = yes ]; then
      out=$(N1_ROOT_OVERRIDE='' N1_STATE_OVERRIDE='' N1_DATA_OVERRIDE='' N1_CONFIG_OVERRIDE='' \
        N1_HOME="$home" N1_PROJECTS_OVERRIDE="$projects" N1_SPAWN_NO_GUARD=1 \
        "$SPAWN" "$id" projects/alpha codex 2>&1)
    else
      mkdir -p "$home/projects/alpha"
      out=$(N1_ROOT_OVERRIDE='' N1_STATE_OVERRIDE='' N1_DATA_OVERRIDE='' N1_PROJECTS_OVERRIDE='' N1_CONFIG_OVERRIDE='' \
        N1_HOME="$home" N1_SPAWN_NO_GUARD=1 \
        "$SPAWN" "$id" projects/alpha codex 2>&1)
    fi
    status=$?
...
N1_HOME scopes projects/|no|nope-home-z7
N1_PROJECTS_OVERRIDE scopes projects/|yes|nope-override-z8
ROWS
  pass "projects/ paths are scoped through the numberone home for single-task spawn"
}
