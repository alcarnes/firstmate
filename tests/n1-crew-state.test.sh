#!/usr/bin/env bash
# Behavior tests for bin/n1-crew-state.sh - the deterministic crew-current-state
# helper.
#
# The status file (state/<id>.status) is a best-effort append-only EVENT LOG, so
# `tail -1` of it reports the last event, not the current state. n1-crew-state
...
#   (i) kind=survey skips the run lookup                           -> pane/status-log
...
CREW_STATE="$ROOT/bin/n1-crew-state.sh"
TMP_ROOT=$(fm_test_tmproot n1-crew-state)
...
# bare `axi` (the run list). Each returns the matching N1_FAKE_AXI_* env text.
...
case "${1:-}" in
  status)
    shift
    if [ "${1:-}" = --run ]; then printf '%s\n' "${N1_FAKE_AXI_STATUS_RUN:-}"
    else printf '%s\n' "${N1_FAKE_AXI_STATUS:-}"; fi ;;
  '') printf '%s\n' "${N1_FAKE_AXI_LIST:-}" ;;
esac
exit 0
SH
...
case "${1:-}" in
  display-message)
    [ "${N1_FAKE_TMUX_MISSING:-0}" = 1 ] && exit 1
    printf '%%1\n' ;;
  capture-pane)
    [ "${N1_FAKE_TMUX_MISSING:-0}" = 1 ] && exit 1
    if [ "${N1_FAKE_BUSY:-0}" = 1 ]; then printf 'work in progress\nesc to interrupt\n'
...
# Run the helper for one case dir. N1_FAKE_* env (run output, busy flag) are read
# from the caller's environment by the fakes above.
run_crew_state() {  # <case-dir> <id>
  PATH="$1/fakebin:$PATH" N1_STATE_OVERRIDE="$1/state" "$CREW_STATE" "$2"
}
...
reset_fakes() {
  ...
  N1_FAKE_AXI_LIST=""
  N1_FAKE_BUSY=0
  N1_FAKE_TMUX_MISSING=0
  export N1_FAKE_AXI_STATUS N1_FAKE_AXI_STATUS_RUN N1_FAKE_AXI_LIST N1_FAKE_BUSY N1_FAKE_TMUX_MISSING
}
...
test_active_run_is_authoritative() {
  ...
  fm_write_meta "$d/state/feat-a.meta" "window=fm:n1-feat-a" "worktree=$d/wt" "kind=mission"
  N1_FAKE_AXI_STATUS="$(run_running fm/feat-a)"
...
test_stale_needs_decision_superseded() {
  ...
  make_fakebin "$d" >/dev/null
  fm_write_meta "$d/state/feat-b.meta" "window=fm:n1-feat-b" "worktree=$d/wt" "kind=mission"
  printf 'working: started\nneeds-decision: pick A or B\n' > "$d/state/feat-b.status"
  N1_FAKE_AXI_STATUS="$(run_fixing fm/feat-b)"
...
test_stale_blocked_superseded() {
  ...
  make_fakebin "$d" >/dev/null
  fm_write_meta "$d/state/feat-bb.meta" "window=fm:n1-feat-bb" "worktree=$d/wt" "kind=mission"
  printf 'blocked: waiting on review answer\n' > "$d/state/feat-bb.status"
  N1_FAKE_AXI_STATUS="$(run_running fm/feat-bb)"
...
test_genuine_parked_not_superseded() {
  ...
  make_fakebin "$d" >/dev/null
  fm_write_meta "$d/state/feat-c.meta" "window=fm:n1-feat-c" "worktree=$d/wt" "kind=mission"
  printf 'needs-decision: review gate\n' > "$d/state/feat-c.status"
  N1_FAKE_AXI_STATUS="$(run_parked fm/feat-c)"
...
test_scalar_gate_parked_not_superseded() {
  ...
  make_fakebin "$d" >/dev/null
  fm_write_meta "$d/state/feat-cs.meta" "window=fm:n1-feat-cs" "worktree=$d/wt" "kind=mission"
  printf 'needs-decision: review gate\n' > "$d/state/feat-cs.status"
  N1_FAKE_AXI_STATUS="$(run_parked_scalar_gate_running fm/feat-cs)"
...
test_gate_block_parked_not_superseded() {
  ...
  make_fakebin "$d" >/dev/null
  fm_write_meta "$d/state/feat-cb.meta" "window=fm:n1-feat-cb" "worktree=$d/wt" "kind=mission"
  printf 'needs-decision: review gate\n' > "$d/state/feat-cb.status"
  N1_FAKE_AXI_STATUS="$(run_parked_in_gate_block fm/feat-cb)"
...
test_ci_ready_done_log_beats_monitoring_run() {
  ...
  make_fakebin "$d" >/dev/null
  fm_write_meta "$d/state/feat-ci.meta" "window=fm:n1-feat-ci" "worktree=$d/wt" "kind=mission"
  printf 'done: PR https://github.com/o/r/pull/2 checks green\n' > "$d/state/feat-ci.status"
  N1_FAKE_AXI_STATUS="$(run_ci_monitoring fm/feat-ci)"
...
test_terminal_passed() {
  ...
  make_fakebin "$d" >/dev/null
  fm_write_meta "$d/state/feat-d.meta" "window=fm:n1-feat-d" "worktree=$d/wt" "kind=mission"
  N1_FAKE_AXI_STATUS="$(run_passed fm/feat-d)"
...
test_terminal_failed() {
  ...
  make_fakebin "$d" >/dev/null
  fm_write_meta "$d/state/feat-e.meta" "window=fm:n1-feat-e" "worktree=$d/wt" "kind=mission"
  N1_FAKE_AXI_STATUS="$(run_failed fm/feat-e)"
...
test_cross_branch_attribution_via_list() {
  ...
  make_fakebin "$d" >/dev/null
  fm_write_meta "$d/state/feat-f.meta" "window=fm:n1-feat-f" "worktree=$d/wt" "kind=mission"
  # The repo-wide active/most-recent run belongs to a different crew's branch.
  N1_FAKE_AXI_STATUS="$(run_running fm/other-crew)"
  N1_FAKE_AXI_LIST="$(cat <<EOF
runs[2]{id,branch,status,head,pr}:
  "01OTHER",fm/other-crew,running,aa,""
  "01MINE",fm/feat-f,running,bb,""
EOF
)"
  N1_FAKE_AXI_STATUS_RUN="$(run_running fm/feat-f)"
...
test_cross_branch_attribution_unquoted_run_list() {
  ...
  make_fakebin "$d" >/dev/null
  fm_write_meta "$d/state/feat-fq.meta" "window=fm:n1-feat-fq" "worktree=$d/wt" "kind=mission"
  N1_FAKE_AXI_STATUS="$(run_running fm/other-crew)"
  N1_FAKE_AXI_LIST="$(cat <<EOF
runs[2]{id,branch,status,head,pr}:
  01OTHER, "fm/other-crew" ,running,aa,""
...
EOF
  ...
  N1_FAKE_AXI_STATUS_RUN="$(run_running fm/feat-fq)"
...
test_other_branch_run_ignored() {
  ...
  make_fakebin "$d" >/dev/null
  fm_write_meta "$d/state/feat-g.meta" "window=fm:n1-feat-g" "worktree=$d/wt" "kind=mission"
  printf 'done: implemented, ready to validate\n' > "$d/state/feat-g.status"
  N1_FAKE_AXI_STATUS="$(run_running fm/some-other)"
  N1_FAKE_AXI_LIST="$(cat <<EOF
...
test_no_run_busy_pane() {
  ...
  make_fakebin "$d" >/dev/null
  fm_write_meta "$d/state/feat-h.meta" "window=fm:n1-feat-h" "worktree=$d/wt" "kind=mission"
...
test_no_run_idle_pane_uses_log() {
  ...
  make_fakebin "$d" >/dev/null
  fm_write_meta "$d/state/feat-i.meta" "window=fm:n1-feat-i" "worktree=$d/wt" "kind=mission"
...
test_dead_window_ignores_stale_status_log() {
  ...
  make_fakebin "$d" >/dev/null
  fm_write_meta "$d/state/feat-dead.meta" "window=fm:n1-feat-dead" "worktree=$d/wt" "kind=mission"
  printf 'done: old completion event\n' > "$d/state/feat-dead.status"
  N1_FAKE_AXI_STATUS=""
  N1_FAKE_AXI_LIST=""
  N1_FAKE_TMUX_MISSING=1
...
test_dead_window_still_reports_terminal_run_step() {
  ...
  make_fakebin "$d" >/dev/null
  fm_write_meta "$d/state/feat-dead-done.meta" "window=fm:n1-feat-dead-done" "worktree=$d/wt" "kind=mission"
  printf 'done: PR https://github.com/o/r/pull/3 checks green\n' > "$d/state/feat-dead-done.status"
  N1_FAKE_AXI_STATUS="$(run_passed fm/feat-dead-done)"
  N1_FAKE_TMUX_MISSING=1   # the crew's window has closed
...
test_dead_window_still_reports_active_run_step() {
  ...
  make_fakebin "$d" >/dev/null
  fm_write_meta "$d/state/feat-dead-act.meta" "window=fm:n1-feat-dead-act" "worktree=$d/wt" "kind=mission"
  N1_FAKE_AXI_STATUS="$(run_running fm/feat-dead-act)"
  N1_FAKE_TMUX_MISSING=1
...
SH
  ...
  fm_write_meta "$d/state/feat-timeout.meta" "window=fm:n1-feat-timeout" "worktree=$d/wt" "kind=mission"
  N1_FAKE_BUSY=1
  start=$SECONDS
  out=$(PATH="$d/fakebin:$toolbin" N1_STATE_OVERRIDE="$d/state" N1_CREW_STATE_NM_TIMEOUT=1 "$CREW_STATE" feat-timeout)
...
# (i) kind=survey skips the run lookup entirely (its deliverable is a report).
test_survey_skips_run_lookup() {
  reset_fakes
  local d; d=$(new_case survey)
  make_repo_on_branch "$d/wt" fm/survey-j
  make_fakebin "$d" >/dev/null
  fm_write_meta "$d/state/survey-j.meta" "window=fm:n1-survey-j" "worktree=$d/wt" "kind=survey"
  # Even if a run existed on this branch, a survey must not read it.
  N1_FAKE_AXI_STATUS="$(run_running fm/survey-j)"
  N1_FAKE_BUSY=1
  local out; out=$(run_crew_state "$d" survey-j)
  assert_not_contains "$out" "source: run-step" "survey ignores no-mistakes run-step"
  assert_contains "$out" "source: pane" "survey reads pane busy-signature"
  pass "survey skips the run lookup"
}
...
test_torn_down_worktree() {
  ...
  make_fakebin "$d" >/dev/null
  fm_write_meta "$d/state/gone-k.meta" "window=fm:n1-gone-k" "worktree=$d/no-such-worktree" "kind=mission"
  local out rc
...
echo "all n1-crew-state tests passed"
