#!/usr/bin/env bash
# Behavior tests for n1-bootstrap.sh tool detection.
#
# Bootstrap prints one line per problem or capability fact and is silent when all
# is well. numberone consumes the exact 'MISSING: treehouse (install: ...)' and
...
TMP_ROOT=$(fm_test_tmproot n1-bootstrap-tests)

# A fake toolchain where every required tool is present and gh is authenticated.
# treehouse's `get --help` advertises --lease only when N1_FAKE_TREEHOUSE_LEASE_HELP=1.
make_fake_toolchain() {
  local dir=$1 fakebin
...
if [ "${1:-}" = get ] && [ "${2:-}" = --help ]; then
  if [ "${N1_FAKE_TREEHOUSE_LEASE_HELP:-}" = 1 ]; then
...
#!/usr/bin/env bash
if [ "${1:-}" = --version ]; then
  printf '%s\n' "${N1_FAKE_NO_MISTAKES_VERSION:-no-mistakes version v1.31.2 (fake) 2026-06-27T00:02:18Z}"
  exit 0
fi
exit 0
SH
...
test_bootstrap_reporting() {
  ...
  while IFS='^' read -r label lease tasks mode expect notcontains; do
    ...
    # N1_ROOT_OVERRIDE points the worktree-tangle check at the non-git home dir so
    ...
    out=$(PATH="$fakebin:$BASE_PATH" N1_HOME="$case_dir/home" N1_ROOT_OVERRIDE="$case_dir/home" \
      N1_FAKE_TREEHOUSE_LEASE_HELP="$lease" "$ROOT/bin/n1-bootstrap.sh")
    case "$mode" in
      empty)
...
test_no_mistakes_min_version() {
  ...
  while IFS='^' read -r label version mode; do
    ...
    fakebin=$(make_fake_toolchain "$case_dir")
    out=$(PATH="$fakebin:$BASE_PATH" N1_HOME="$case_dir/home" N1_ROOT_OVERRIDE="$case_dir/home" \
      N1_FAKE_TREEHOUSE_LEASE_HELP=1 N1_FAKE_NO_MISTAKES_VERSION="$version" "$ROOT/bin/n1-bootstrap.sh")
    case "$mode" in
      empty)
        [ -z "$out" ] || fail "$label: expected silence, got: $out" ;;
      missing)
