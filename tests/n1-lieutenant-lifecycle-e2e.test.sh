#!/usr/bin/env bash
# tests/n1-lieutenant-lifecycle-e2e.test.sh - the happy-path lieutenant operator
...
# lease-specific paths live in n1-lieutenant-safety.test.sh.
#
...
#   - spawn meta records kind=lieutenant, home=, and the project list; launch runs
#     in the subhome with the persistent charter and cleared operational overrides
#   - a bare `n1-<id>` send targets the window recorded in THIS home's meta
...
TMP_ROOT=$(fm_test_tmproot n1-lieutenant-lifecycle)
...
EOF
  ...
  N1_LIEUTENANT_SCOPE='customer onboarding from brief' \
    scaffold_lieutenant_charter "$HOME_DIR" design 'customer onboarding charter' alpha beta gamma \
    || fail "filled lieutenant charter scaffold failed"
}

phase_seed() {
  local out
  out=$(PATH="$FAKEBIN:$PATH" N1_HOME="$HOME_DIR" \
    "$ROOT/bin/n1-home-seed.sh" design "$SUB" alpha beta gamma) \
    || fail "seed failed"
  SUB_ABS=$(cd "$SUB" && pwd -P)

  assert_contains "$out" "home=$SUB_ABS" "seed did not report the subhome"
  assert_present "$SUB/.n1-lieutenant-home" "seed did not mark the subhome"
  ...
  assert_grep '- design - customer onboarding charter' "$HOME_DIR/data/lieutenants.md" "registry summary not from the charter"
  assert_grep 'scope: customer onboarding from brief' "$HOME_DIR/data/lieutenants.md" "registry scope not from the filled brief"
  assert_grep 'projects: alpha, beta, gamma' "$HOME_DIR/data/lieutenants.md" "registry did not record the project list"
  assert_no_grep 'owns:' "$HOME_DIR/data/lieutenants.md" "registry used the legacy owns field"

  # Delivery modes preserved in the subhome registry; validation passes.
  [ "$(N1_HOME="$SUB" "$ROOT/bin/n1-project-mode.sh" alpha)" = "direct-PR on" ] \
    || fail "alpha delivery mode not preserved in the subhome"
  [ "$(N1_HOME="$SUB" "$ROOT/bin/n1-project-mode.sh" beta)" = "direct-PR off" ] \
    || fail "beta delivery mode not preserved in the subhome"
  N1_HOME="$HOME_DIR" "$ROOT/bin/n1-home-seed.sh" validate >/dev/null || fail "registry validation failed after seed"
...
phase_spawn() {
  : > "$LOG"
  PATH="$FAKEBIN:$PATH" N1_HOME="$HOME_DIR" N1_CONFIG_OVERRIDE="$HOME_DIR/parent-config" \
    N1_FAKE_TMUX_LOG="$LOG" N1_FAKE_TMUX_CAPTURE="$PANE" \
    "$ROOT/bin/n1-spawn.sh" design "$SUB" codex --lieutenant >/dev/null \
    || fail "lieutenant spawn failed"

  local meta="$HOME_DIR/state/design.meta"
  assert_grep 'kind=lieutenant' "$meta" "spawn meta did not record kind=lieutenant"
  ...
  assert_grep "N1_HOME='$SUB_ABS'" "$LOG" "lieutenant launch did not set N1_HOME to the subhome"
  assert_grep 'N1_ROOT_OVERRIDE= N1_STATE_OVERRIDE= N1_DATA_OVERRIDE= N1_PROJECTS_OVERRIDE=' "$LOG" "launch did not clear operational overrides"
  assert_grep 'N1_CONFIG_OVERRIDE=' "$LOG" "launch did not clear the config override"
  assert_grep "$SUB_ABS/data/charter.md" "$LOG" "launch did not use the persistent charter"
  assert_no_grep 'notify=' "$LOG" "lieutenant codex launch included the parent turn-end notify hook"
  assert_no_grep 'turn-ended' "$LOG" "lieutenant codex launch referenced a parent turn-ended signal"
...
phase_send() {
  : > "$LOG"
  # The meta window (numberone:n1-design) must win over a foreign same-named
  # window returned by list-windows.
  PATH="$FAKEBIN:$PATH" N1_HOME="$HOME_DIR" N1_FAKE_TMUX_WINDOW="other-session:n1-design" \
    N1_FAKE_TMUX_LOG="$LOG" N1_FAKE_TMUX_CAPTURE="$PANE" \
    "$ROOT/bin/n1-send.sh" n1-design 'route this work' >/dev/null 2>&1 \
    || fail "n1-send failed for a bare numberone window with home metadata"
  # design is a kind=lieutenant target, so the request is prefixed with the
  # from-numberone marker (bin/n1-marker-lib.sh): the send targets the meta window
  # AND carries the marker label, and the original payload still follows it.
  assert_grep 'send-keys -t numberone:n1-design -l [n1-from-numberone]' "$LOG" "send did not use the window recorded in this home's meta, or did not mark the lieutenant request"
  assert_grep 'route this work' "$LOG" "the original request text did not survive the marker"
  assert_no_grep 'send-keys -t other-session:n1-design' "$LOG" "send targeted a foreign same-named window"
  pass "send: a bare n1-<id> lieutenant routes to the meta window with the from-numberone marker"
}
...
EOF
  local out before
  out=$(N1_HOME="$HOME_DIR" "$ROOT/bin/n1-backlog-handoff.sh" design feat-x feat-y) \
  ...
  N1_HOME="$HOME_DIR" "$ROOT/bin/n1-backlog-handoff.sh" design feat-x feat-y >/dev/null 2>&1 \
...
phase_recovery() {
  ...
  PATH="$FAKEBIN:$PATH" N1_HOME="$HOME_DIR" N1_FAKE_TMUX_LOG="$LOG" N1_FAKE_TMUX_CAPTURE="$PANE" \
    "$ROOT/bin/n1-spawn.sh" design "echo relaunch" --lieutenant >/dev/null 2>&1 \
  ...
  assert_grep 'window=numberone:n1-design' "$meta" "respawn did not reconstruct the direct-report window"
...
phase_teardown() {
  : > "$LOG"
  PATH="$FAKEBIN:$PATH" N1_HOME="$HOME_DIR" N1_FAKE_TMUX_LOG="$LOG" N1_FAKE_TMUX_CAPTURE="$PANE" \
    "$ROOT/bin/n1-teardown.sh" design >/dev/null 2>&1 \
    || fail "teardown failed for the empty lieutenant home"
  assert_absent "$SUB" "teardown did not remove the retired lieutenant home"
  assert_absent "$HOME_DIR/state/design.meta" "teardown did not clear the parent meta"
  assert_no_grep '- design ' "$HOME_DIR/data/lieutenants.md" "teardown did not remove the registry route"
