#!/usr/bin/env bash
# Tests for bin/n1-teardown.sh's landed-work safety check.
#
# The check refuses to tear down a worktree whose work has not LANDED, because
# treehouse return hard-resets the worktree. "Landed" means reachable from a remote
# OR - for a normal mission task whose commits are not so reachable - its PR is merged
...
#   (m) n1-pr-check rerun after HEAD moved                      -> no stale pr_head
set -u

# shellcheck source=tests/lib.sh
. "$(dirname "${BASH_SOURCE[0]}")/lib.sh"

TEARDOWN="$ROOT/bin/n1-teardown.sh"
PR_CHECK="$ROOT/bin/n1-pr-check.sh"
TMP_ROOT=$(fm_test_tmproot n1-teardown-tests)

# Build a fresh sandbox for one test case. Sets up:
#   $CASE/state/        - numberone state dir (with a fresh watcher beacon)
...
#   $CASE/project/      - clone of origin; acts as the numberone project dir
...
SH
  ...
  # Add a worktree on a fresh task branch; that branch is where the ensign commits.
  git -C "$case_dir/project" worktree add -q -b fm/task-x1 "$case_dir/wt" main

  # Fresh watcher beacon so n1-guard stays quiet.
...
add_fork_with_pushed_branch() {
  ...
  # so refs/remotes/fork/n1-task-x1 is visible from the worktree (shared object db).
...
run_teardown() {
  ...
  N1_ROOT_OVERRIDE="$ROOT" \
  N1_STATE_OVERRIDE="$case_dir/state" \
...
test_local_only_fork_remote_allows() {
  ...
  write_meta "$case_dir" local-only mission
...
test_teardown_prompts_tasks_axi_done_when_compatible() {
  ...
  write_meta "$case_dir" no-mistakes mission
...
test_pr_check_does_not_refresh_stale_pr_head() {
  ...
  pass "n1-pr-check does not refresh PR head after HEAD moves"
}
