#!/usr/bin/env bash
# tests/n1-lieutenant-safety.test.sh - lieutenant home safety invariants:
...
# operator flow lives in n1-lieutenant-lifecycle-e2e.test.sh; this file keeps the
# destructive-invariant coverage that an e2e run cannot deterministically reach.
set -u

# shellcheck source=tests/lieutenant-helpers.sh
. "$(dirname "${BASH_SOURCE[0]}")/lieutenant-helpers.sh"

TMP_ROOT=$(fm_test_tmproot n1-lieutenant-safety)
...
test_fm_home_parameterization() {
  ...
  out=$(N1_HOME="$home_one" "$ROOT/bin/n1-project-mode.sh" app)
  [ "$out" = "local-only on" ] || fail "n1-project-mode did not read projects.md from N1_HOME"
  out=$(N1_HOME="$home_two" "$ROOT/bin/n1-project-mode.sh" app 2>/dev/null)
  [ "$out" = "no-mistakes off" ] || fail "n1-project-mode did not isolate missing registry by home"

  N1_HOME="$home_one" "$ROOT/bin/n1-brief.sh" task-a app >/dev/null || fail "brief scaffold failed under N1_HOME"
  brief="$home_one/data/task-a/brief.md"
  [ -f "$brief" ] || fail "brief was not written under N1_HOME/data"
  grep -F ">> '$home_one/state/task-a.status'" "$brief" >/dev/null || fail "brief did not shell-quote N1_HOME state path"

  N1_HOME="$home_one" "$ROOT/bin/n1-brief.sh" task-b app --survey >/dev/null || fail "survey brief scaffold failed under N1_HOME"
  brief="$home_one/data/task-b/brief.md"
  grep -F ">> '$home_one/state/task-b.status'" "$brief" >/dev/null || fail "survey brief did not shell-quote N1_HOME state path"

  N1_HOME="$home_one" N1_LIEUTENANT_CHARTER='ops domain' "$ROOT/bin/n1-brief.sh" task-c --lieutenant app >/dev/null \
    || fail "lieutenant brief scaffold failed under N1_HOME"
  brief="$home_one/data/task-c/brief.md"
  grep -F ">> '$home_one/state/task-c.status'" "$brief" >/dev/null || fail "lieutenant brief did not shell-quote N1_HOME state path"

  printf 'project=x\n' > "$home_one/state/task-a.meta"
  N1_HOME="$home_one" N1_GUARD_GRACE=999999 "$ROOT/bin/n1-pr-check.sh" task-a https://github.com/example/repo/pull/1 >/dev/null 2>/dev/null \
    || fail "n1-pr-check failed under N1_HOME"
  [ -f "$home_one/state/task-a.check.sh" ] || fail "pr check was not written under N1_HOME/state"
  [ ! -e "$home_two/state/task-a.check.sh" ] || fail "pr check leaked into another home"
  pass "N1_HOME parameterizes data and state paths"
}
...
test_lock_status_is_per_home() {
  ...
  printf '999999\n' > "$home_one/state/.lock"
  out=$(N1_HOME="$home_one" "$ROOT/bin/n1-lock.sh" status)
  printf '%s\n' "$out" | grep -F 'lock: stale' >/dev/null || fail "home one lock status did not read its own lock"
  out=$(N1_HOME="$home_two" "$ROOT/bin/n1-lock.sh" status)
  [ "$out" = "lock: free" ] || fail "home two lock status was affected by home one"
  pass "n1-lock status is scoped per home"
}

test_seed_allows_overlapping_clones_and_drops_owner() {
  # A project may appear in several lieutenants' (non-exclusive) clone lists; the
  ...
  # init, modes preserved - is asserted by n1-lieutenant-lifecycle-e2e.
...
EOF
  ...
  N1_HOME="$home" N1_LIEUTENANT_CHARTER='feature design for alpha beta' \
    N1_LIEUTENANT_SCOPE='feature design for alpha beta' \
    "$ROOT/bin/n1-home-seed.sh" design "$design" alpha beta >/dev/null \
    || fail "initial seed failed"
  assert_grep '- design - feature design for alpha beta' "$home/data/lieutenants.md" "design registry line missing"
  assert_grep 'projects: alpha, beta' "$home/data/lieutenants.md" "design project clone list missing"
  assert_no_grep 'owns:' "$home/data/lieutenants.md" "registry used the legacy owns field"

  # beta is shared with a second lieutenant of a different scope (overlap allowed).
  N1_HOME="$home" N1_LIEUTENANT_CHARTER='issue triage for beta' \
    N1_LIEUTENANT_SCOPE='issue triage for beta' \
    "$ROOT/bin/n1-home-seed.sh" other "$other" beta >/dev/null 2>&1 \
    || fail "seed refused overlapping project clones across different scopes"
  assert_grep '- other - issue triage for beta' "$home/data/lieutenants.md" "overlapping registry line missing"
  N1_HOME="$home" "$ROOT/bin/n1-home-seed.sh" validate >/dev/null || fail "registry validation rejected overlapping clones"

  if N1_HOME="$home" "$ROOT/bin/n1-home-seed.sh" owner alpha >/dev/null 2>&1; then
...
test_home_seed_validate_rejects_duplicate_homes() {
  ...
  cat > "$home/data/lieutenants.md" <<EOF
...
EOF
  ...
  if N1_HOME="$home" "$ROOT/bin/n1-home-seed.sh" validate >/dev/null 2>"$err"; then
    fail "registry validation accepted two lieutenants with the same home"
  fi
  grep -F 'duplicate lieutenant home assignment' "$err" >/dev/null \
...
EOF
  ...
  if N1_HOME="$home" "$ROOT/bin/n1-home-seed.sh" validate >/dev/null 2>"$err"; then
    fail "registry validation accepted two homes for the same lieutenant id"
  fi
  grep -F 'duplicate lieutenant id assignment' "$err" >/dev/null \
...
EOF
  ...
  if N1_HOME="$home" "$ROOT/bin/n1-home-seed.sh" validate >/dev/null 2>"$err"; then
    fail "registry validation accepted nested lieutenant homes"
  fi
  grep -F 'overlapping lieutenant home assignment' "$err" >/dev/null \
...
test_home_seed_uses_treehouse_acquired_home() {
  ...
  out=$(PATH="$fakebin:$PATH" N1_HOME="$home" N1_FAKE_TREEHOUSE_HOME="$acquired" N1_FAKE_TMUX_LOG="$log" \
    N1_FAKE_TREEHOUSE_LEASE_FILE="$lease" \
    N1_LIEUTENANT_CHARTER='dash acquired scope' N1_LIEUTENANT_SCOPE='dash acquired scope' \
    "$ROOT/bin/n1-home-seed.sh" dash - alpha) \
  ...
  grep -F 'treehouse get --lease --lease-holder dash' "$log" >/dev/null || fail "seed did not durably lease a home under the lieutenant id"
  [ -f "$lease" ] || fail "seed did not record a treehouse lease"
  [ "$(cat "$lease")" = dash ] || fail "seed did not set the lease holder to the lieutenant id"
  [ -f "$acquired/.n1-lieutenant-home" ] || fail "seed did not mark acquired home"
  [ "$(cat "$acquired/.n1-lieutenant-home")" = dash ] || fail "seed wrote wrong acquired-home marker"
  [ -d "$acquired/projects/alpha/.git" ] || fail "seed did not clone project into acquired home"
  grep -F "home: $acquired_abs" "$home/data/lieutenants.md" >/dev/null || fail "registry did not record acquired home"
  pass "home seeding durably leases treehouse-acquired dash homes under the lieutenant id"
}
...
test_home_seed_returns_treehouse_acquired_home_on_assignment_failure() {
  ...
  git clone --quiet "$ROOT" "$acquired"
  acquired_abs=$(cd "$acquired" && pwd -P)
  printf 'other\n' > "$acquired/.n1-lieutenant-home"
  fakebin=$(make_fake_tmux "$TMP_ROOT/dash-fail-fake")
  log="$TMP_ROOT/dash-fail-fake/tmux.log"

  if PATH="$fakebin:$PATH" N1_HOME="$home" N1_FAKE_TREEHOUSE_HOME="$acquired" N1_FAKE_TMUX_LOG="$log" \
    N1_LIEUTENANT_CHARTER='dash acquired scope' N1_LIEUTENANT_SCOPE='dash acquired scope' \
    "$ROOT/bin/n1-home-seed.sh" dash - alpha >/dev/null 2>"$err"; then
    fail "seed reused an acquired home marked for another lieutenant"
  fi
  ...
  if [ -f "$home/data/lieutenants.md" ] && grep -F -- '- dash ' "$home/data/lieutenants.md" >/dev/null; then
...
test_home_seed_warns_when_acquired_home_return_fails() {
  ...
  if PATH="$fakebin:$PATH" N1_HOME="$home" N1_FAKE_TREEHOUSE_HOME="$acquired" N1_FAKE_TMUX_LOG="$log" \
    N1_FAKE_TREEHOUSE_LEASE_FILE="$lease" N1_FAKE_TREEHOUSE_RETURN_FAIL=1 \
    N1_LIEUTENANT_CHARTER='dash acquired scope' N1_LIEUTENANT_SCOPE='dash acquired scope' \
    "$ROOT/bin/n1-home-seed.sh" dash - alpha >/dev/null 2>"$err"; then
...
test_home_seed_does_not_return_unsafe_acquired_home() {
  ...
  if PATH="$fakebin:$PATH" N1_HOME="$home" N1_FAKE_TREEHOUSE_HOME="$home" N1_FAKE_TMUX_LOG="$log" \
    "$ROOT/bin/n1-home-seed.sh" dash - alpha >/dev/null 2>"$err"; then
    fail "seed accepted an acquired home matching the active numberone home"
  fi
  grep -F 'lieutenant home cannot be the active numberone home' "$err" >/dev/null \
  ...
  : > "$log"
  if PATH="$fakebin:$PATH" N1_HOME="$home" N1_FAKE_TREEHOUSE_HOME="$descendant" N1_FAKE_TMUX_LOG="$log" \
    "$ROOT/bin/n1-home-seed.sh" dash - alpha >/dev/null 2>"$err"; then
    fail "seed accepted an acquired home inside the active numberone home"
  fi
  grep -F 'lieutenant home cannot be inside the active numberone home' "$err" >/dev/null \
...
EOF
  ...
  if N1_HOME="$home" N1_LIEUTENANT_CHARTER='rollback scope' N1_LIEUTENANT_SCOPE='rollback scope' \
    "$ROOT/bin/n1-home-seed.sh" rollback "$subhome" alpha beta >/dev/null 2>"$err"; then
  ...
  [ ! -e "$subhome" ] || fail "failed seed left the newly created lieutenant home behind"
  [ ! -e "$subhome/.n1-lieutenant-home" ] || fail "failed seed left a subhome marker"
  ...
  if [ -f "$home/data/lieutenants.md" ] && grep -F -- '- rollback ' "$home/data/lieutenants.md" >/dev/null; then
...
test_home_seed_refuses_missing_filled_charter() {
  ...
  if N1_HOME="$home" "$ROOT/bin/n1-home-seed.sh" design "$subhome" alpha >/dev/null 2>"$err"; then
    fail "seed accepted a direct seed without a filled charter"
  fi
  grep -F 'no filled lieutenant charter brief' "$err" >/dev/null \
...
test_home_seed_refuses_placeholder_charter() {
  ...
  printf '%s\n' '- alpha [direct-PR] - alpha project (added 2026-06-22)' > "$home/data/projects.md"
  N1_HOME="$home" "$ROOT/bin/n1-brief.sh" design --lieutenant alpha >/dev/null \
...
test_home_seed_refuses_empty_charter_fields() {
  ...
  if N1_HOME="$home" N1_LIEUTENANT_CHARTER='   ' "$ROOT/bin/n1-home-seed.sh" design "$subhome" alpha >/dev/null 2>"$err"; then
  ...
  N1_LIEUTENANT_SCOPE='   ' scaffold_lieutenant_charter "$home" design 'filled charter' alpha \
...
test_home_seed_refuses_local_only_project() {
  ...
  if N1_HOME="$home" "$ROOT/bin/n1-home-seed.sh" design "$subhome" alpha >/dev/null 2>"$err"; then
    fail "seed allowed a local-only project into a lieutenant home"
  fi
  grep -F 'project alpha is local-only; lieutenant routes support only no-mistakes and direct-PR projects' "$err" >/dev/null \
...
test_home_seed_refuses_registry_delimiter_home() {
  ...
  if N1_HOME="$home" N1_LIEUTENANT_CHARTER='delimiter charter' "$ROOT/bin/n1-home-seed.sh" design "$subhome" alpha >/dev/null 2>"$err"; then
    fail "seed accepted a home path with registry delimiters"
  fi
  grep -F 'lieutenant home path contains registry delimiters' "$err" >/dev/null \
    || fail "seed did not explain delimiter home refusal"
  [ ! -e "$subhome/.n1-lieutenant-home" ] || fail "delimiter home seed wrote a marker"
  if [ -f "$home/data/lieutenants.md" ] && grep -F -- '- design ' "$home/data/lieutenants.md" >/dev/null; then
...
test_home_seed_refuses_active_home_and_root() {
  ...
  printf '%s\n' '- alpha [direct-PR] - alpha project (added 2026-06-22)' > "$home/data/projects.md"
  scaffold_lieutenant_charter "$home" design 'design domain' alpha || fail "charter scaffold failed for active-home seed test"

  if N1_HOME="$home" "$ROOT/bin/n1-home-seed.sh" design "$home" alpha >/dev/null 2>"$err"; then
    fail "seed allowed lieutenant home to reuse active N1_HOME"
  fi
  grep -F 'lieutenant home cannot be the active numberone home' "$err" >/dev/null \
    || fail "seed did not explain active N1_HOME rejection"

  if N1_HOME="$home" "$ROOT/bin/n1-home-seed.sh" design "$active_descendant" alpha >/dev/null 2>"$err"; then
    fail "seed allowed lieutenant home inside active N1_HOME"
  fi
  grep -F 'lieutenant home cannot be inside the active numberone home' "$err" >/dev/null \
    || fail "seed did not explain active N1_HOME descendant rejection"
  [ ! -e "$home/nested" ] || fail "seed created a directory inside active N1_HOME before descendant rejection"

  if N1_HOME="$home" "$ROOT/bin/n1-home-seed.sh" design "$active_ancestor" alpha >/dev/null 2>"$err"; then
    fail "seed allowed lieutenant home to contain active N1_HOME"
  fi
  grep -F 'lieutenant home cannot be an ancestor of the active numberone home' "$err" >/dev/null \
    || fail "seed did not explain active N1_HOME ancestor rejection"
  [ ! -f "$active_ancestor/.n1-lieutenant-home" ] || fail "seed marked an ancestor of active N1_HOME"

  if N1_HOME="$home" "$ROOT/bin/n1-home-seed.sh" design "$ROOT" alpha >/dev/null 2>"$err"; then
    fail "seed allowed lieutenant home to reuse N1_ROOT"
  fi
  grep -F 'lieutenant home cannot be the numberone repo' "$err" >/dev/null \
    || fail "seed did not explain N1_ROOT rejection"

  git clone --quiet "$ROOT" "$root_clone"
  if N1_HOME="$home" N1_ROOT_OVERRIDE="$root_clone" "$ROOT/bin/n1-home-seed.sh" design "$root_descendant" alpha >/dev/null 2>"$err"; then
    fail "seed allowed lieutenant home inside N1_ROOT"
  fi
  grep -F 'lieutenant home cannot be inside the numberone repo' "$err" >/dev/null \
    || fail "seed did not explain N1_ROOT descendant rejection"
  [ ! -e "$root_clone/tmp" ] || fail "seed created a directory inside N1_ROOT before descendant rejection"

  git clone --quiet "$ROOT" "$root_ancestor"
  git clone --quiet "$ROOT" "$root_inside"
  if N1_HOME="$home" N1_ROOT_OVERRIDE="$root_inside" "$ROOT/bin/n1-home-seed.sh" design "$root_ancestor" alpha >/dev/null 2>"$err"; then
    fail "seed allowed lieutenant home to contain N1_ROOT"
  fi
  grep -F 'lieutenant home cannot be an ancestor of the numberone repo' "$err" >/dev/null \
    || fail "seed did not explain N1_ROOT ancestor rejection"
  [ ! -f "$root_ancestor/.n1-lieutenant-home" ] || fail "seed marked an ancestor of N1_ROOT"
...
test_home_seed_refuses_home_marked_for_another_id() {
  ...
  printf 'other\n' > "$subhome/.n1-lieutenant-home"
  printf '%s\n' '- alpha [direct-PR] - alpha project (added 2026-06-22)' > "$home/data/projects.md"
  scaffold_lieutenant_charter "$home" design 'design domain' alpha || fail "charter scaffold failed for marked-home seed test"

  if N1_HOME="$home" "$ROOT/bin/n1-home-seed.sh" design "$subhome" alpha >/dev/null 2>"$err"; then
    fail "seed reused a home marked for another lieutenant"
  fi
  grep -F 'already marked for other' "$err" >/dev/null || fail "seed did not explain marked-home rejection"
  [ "$(cat "$subhome/.n1-lieutenant-home")" = "other" ] || fail "seed overwrote another lieutenant marker"
...
test_home_seed_refuses_home_registered_to_another_id() {
  ...
  subhome_abs=$(cd "$subhome" && pwd -P)
  printf '%s\n' '- alpha [direct-PR] - alpha project (added 2026-06-22)' > "$home/data/projects.md"
  printf '%s\n' '- other - other domain (home: '"$subhome_abs"'; scope: other domain; projects: beta; added 2026-06-22)' > "$home/data/lieutenants.md"
  scaffold_lieutenant_charter "$home" design 'design domain' alpha || fail "charter scaffold failed for registered-home seed test"

  if N1_HOME="$home" "$ROOT/bin/n1-home-seed.sh" design "$subhome" alpha >/dev/null 2>"$err"; then
    fail "seed reused a home registered to another lieutenant"
  fi
  grep -F 'already registered to other' "$err" >/dev/null || fail "seed did not explain registered-home rejection"
  [ ! -e "$subhome/.n1-lieutenant-home" ] || fail "seed wrote a marker before rejecting a registered home"
...
test_home_seed_refuses_reassigning_existing_id_to_different_home() {
  ...
  N1_HOME="$home" N1_LIEUTENANT_CHARTER='design domain' N1_LIEUTENANT_SCOPE='design domain' \
    "$ROOT/bin/n1-home-seed.sh" design "$first" alpha >/dev/null \
    || fail "initial seed failed for reassigning-id test"
  first_abs=$(cd "$first" && pwd -P)

  if N1_HOME="$home" N1_LIEUTENANT_CHARTER='design domain' N1_LIEUTENANT_SCOPE='design domain' \
    "$ROOT/bin/n1-home-seed.sh" design "$second" alpha >/dev/null 2>"$err"; then
    fail "seed reassigned an existing lieutenant id to a different home"
  fi
  grep -F "lieutenant id design is already registered to home $first_abs" "$err" >/dev/null \
    || fail "seed did not explain same-id different-home rejection"
  [ ! -e "$second" ] || fail "failed id reassignment created the new subhome"
  [ "$(cat "$first/.n1-lieutenant-home")" = design ] || fail "failed id reassignment changed the original marker"
  grep -F "home: $first_abs" "$home/data/lieutenants.md" >/dev/null \
  ...
  grep -F "home: $second_abs" "$home/data/lieutenants.md" >/dev/null \
...
EOF
  ...
  if N1_HOME="$home" "$ROOT/bin/n1-home-seed.sh" design "$nested" alpha >/dev/null 2>"$err"; then
    fail "seed accepted a home inside a registered lieutenant home"
  fi
  grep -F 'overlaps registered lieutenant home' "$err" >/dev/null \
    || fail "seed did not explain registered ancestor overlap"
  [ ! -e "$nested" ] || fail "seed created a nested home inside a registered home"

  if N1_HOME="$home" "$ROOT/bin/n1-home-seed.sh" design "$parent" alpha >/dev/null 2>"$err"; then
    fail "seed accepted a home containing a registered lieutenant home"
  fi
  grep -F 'overlaps registered lieutenant home' "$err" >/dev/null \
    || fail "seed did not explain registered descendant overlap"
  [ ! -f "$parent/.n1-lieutenant-home" ] || fail "seed marked a home containing a registered home"
...
test_home_seed_refuses_remote_backed_project_without_origin() {
  ...
  fm_git_init_commit "$home/projects/alpha"
  printf '%s\n' '- alpha [direct-PR] - alpha project (added 2026-06-22)' > "$home/data/projects.md"
  scaffold_lieutenant_charter "$home" design 'design domain' alpha || fail "charter scaffold failed for no-origin seed test"
...
test_home_seed_refuses_existing_remote_backed_project_with_wrong_origin() {
  ...
  printf '%s\n' '- alpha [direct-PR] - alpha project (added 2026-06-22)' > "$home/data/projects.md"
  scaffold_lieutenant_charter "$home" design 'design domain' alpha || fail "charter scaffold failed for wrong-origin seed test"
...
test_home_seed_resolves_relative_source_origins() {
  ...
  printf '%s\n' '- alpha [direct-PR] - alpha project (added 2026-06-22)' > "$home/data/projects.md"
  scaffold_lieutenant_charter "$home" design 'design domain' alpha || fail "charter scaffold failed for relative origin seed test"

  out=$(N1_HOME="$home" "$ROOT/bin/n1-home-seed.sh" design "$subhome" alpha)
  subhome_abs=$(cd "$subhome" && pwd -P)
  ...
  N1_HOME="$home" "$ROOT/bin/n1-home-seed.sh" design "$subhome" alpha >/dev/null \
...
test_home_seed_skips_initialized_existing_no_mistakes_projects() {
  ...
  if PATH="$fakebin:$PATH" N1_FAKE_NO_MISTAKES_LOG="$log" N1_FAKE_NO_MISTAKES_FAIL_PROJECT=beta \
    N1_HOME="$home" N1_LIEUTENANT_CHARTER='existing init rollback scope' N1_LIEUTENANT_SCOPE='existing init rollback scope' \
    "$ROOT/bin/n1-home-seed.sh" design "$subhome" alpha beta >/dev/null 2>"$err"; then
...
test_home_seed_refuses_uninitialized_existing_no_mistakes_project() {
  ...
  if PATH="$fakebin:$PATH" N1_FAKE_NO_MISTAKES_LOG="$log" \
    N1_HOME="$home" N1_LIEUTENANT_CHARTER='existing uninitialized scope' \
    "$ROOT/bin/n1-home-seed.sh" design "$subhome" alpha >/dev/null 2>"$err"; then
...
test_home_seed_refuses_project_destinations_outside_subhome() {
  ...
  printf '%s\n' '- alpha [direct-PR] - alpha project (added 2026-06-22)' > "$home/data/projects.md"
  scaffold_lieutenant_charter "$home" design 'design domain' alpha || fail "charter scaffold failed for symlink destination seed test"

  if N1_HOME="$home" "$ROOT/bin/n1-home-seed.sh" design "$subhome" alpha >/dev/null 2>"$err"; then
    fail "seed followed a subhome projects symlink outside the subhome"
  fi
  grep -F 'lieutenant projects directory must resolve inside the lieutenant home' "$err" >/dev/null \
  ...
  [ ! -f "$subhome/.n1-lieutenant-home" ] || fail "seed marked subhome after unsafe project destination rejection"
...
test_home_seed_refuses_operational_dirs_outside_subhome() {
  ...
  printf '%s\n' '- alpha [direct-PR] - alpha project (added 2026-06-22)' > "$home/data/projects.md"
  scaffold_lieutenant_charter "$home" design 'design domain' alpha || fail "charter scaffold failed for symlink operational dir seed test"
  ...
  for opdir in data state config; do
    ...
    grep -F "lieutenant $opdir directory must resolve inside the lieutenant home" "$err" >/dev/null \
      || fail "seed did not explain unsafe $opdir directory rejection"
    [ ! -f "$subhome/.n1-lieutenant-home" ] || fail "seed marked subhome after unsafe $opdir directory rejection"
  done
...
test_home_seed_refuses_symlinked_leaf_files() {
  ...
  printf '%s\n' '- alpha [direct-PR] - alpha project (added 2026-06-22)' > "$home/data/projects.md"
  scaffold_lieutenant_charter "$home" design 'design domain' alpha || fail "charter scaffold failed for symlink leaf seed test"

  for leaf in data/projects.md data/charter.md .n1-lieutenant-home; do
    ...
    expected=outside
    if [ "$leaf" = ".n1-lieutenant-home" ]; then
      expected=design
    fi
    printf '%s\n' "$expected" > "$sink"
    ...
    grep -F 'lieutenant leaf file must not be a symlink:' "$err" >/dev/null \
    ...
    [ ! -f "$subhome/.n1-lieutenant-home" ] || [ "$leaf" = ".n1-lieutenant-home" ] || fail "seed marked subhome after symlinked leaf refusal"
  done
  pass "home seeding refuses symlinked leaf files"
}

test_lieutenant_spawn_requires_seeded_matching_home() {
  ...
  cat > "$fakeroot/bin/n1-guard.sh" <<'SH'
#!/usr/bin/env bash
exit 0
SH
  chmod +x "$fakeroot/bin/n1-guard.sh"
  mkdir -p "$ancestor_active_home/data" "$ancestor_active_home/state" "$active_ancestor/data" "$root_ancestor/data" "$root_inside/bin"
  cat > "$root_inside/bin/n1-guard.sh" <<'SH'
#!/usr/bin/env bash
exit 0
SH
  chmod +x "$root_inside/bin/n1-guard.sh"
  ...
  if PATH="$fakebin:$PATH" N1_HOME="$home" N1_FAKE_TMUX_LOG="$log" N1_FAKE_TMUX_CAPTURE="$TMP_ROOT/spawn-validate-fake/pane.txt" \
    "$ROOT/bin/n1-spawn.sh" domain "$subhome" codex --lieutenant >/dev/null 2>"$err"; then
    fail "lieutenant spawn accepted an unseeded home"
  fi
  grep -F 'not a seeded lieutenant home' "$err" >/dev/null || fail "spawn did not explain missing seed marker"
  ...
  printf 'other\n' > "$wronghome/.n1-lieutenant-home"
  if PATH="$fakebin:$PATH" N1_HOME="$home" N1_FAKE_TMUX_LOG="$log" N1_FAKE_TMUX_CAPTURE="$TMP_ROOT/spawn-validate-fake/pane.txt" \
    "$ROOT/bin/n1-spawn.sh" domain "$wronghome" codex --lieutenant >/dev/null 2>"$err"; then
    fail "lieutenant spawn accepted a home marked for another lieutenant"
  fi
  grep -F 'marked for lieutenant other, expected domain' "$err" >/dev/null || fail "spawn did not explain marker mismatch"

  printf 'domain\n' > "$marker_only/.n1-lieutenant-home"
  printf 'charter\n' > "$marker_only/data/charter.md"
  if PATH="$fakebin:$PATH" N1_HOME="$home" N1_FAKE_TMUX_LOG="$log" N1_FAKE_TMUX_CAPTURE="$TMP_ROOT/spawn-validate-fake/pane.txt" \
    "$ROOT/bin/n1-spawn.sh" domain "$marker_only" codex --lieutenant >/dev/null 2>"$err"; then
    fail "lieutenant spawn accepted a marked home missing AGENTS.md"
  fi
  grep -F 'not a numberone home (missing AGENTS.md)' "$err" >/dev/null || fail "spawn did not explain missing AGENTS.md"

  printf '# Number One\n' > "$marker_only/AGENTS.md"
  if PATH="$fakebin:$PATH" N1_HOME="$home" N1_FAKE_TMUX_LOG="$log" N1_FAKE_TMUX_CAPTURE="$TMP_ROOT/spawn-validate-fake/pane.txt" \
    "$ROOT/bin/n1-spawn.sh" domain "$marker_only" codex --lieutenant >/dev/null 2>"$err"; then
    fail "lieutenant spawn accepted a marked home missing bin"
  fi
  grep -F 'not a numberone home (missing bin/)' "$err" >/dev/null || fail "spawn did not explain missing bin"

  printf 'domain\n' > "$home/.n1-lieutenant-home"
  if PATH="$fakebin:$PATH" N1_HOME="$home" N1_FAKE_TMUX_LOG="$log" N1_FAKE_TMUX_CAPTURE="$TMP_ROOT/spawn-validate-fake/pane.txt" \
    "$ROOT/bin/n1-spawn.sh" domain "$home" codex --lieutenant >/dev/null 2>"$err"; then
    fail "lieutenant spawn accepted the active home"
  fi
  grep -F 'lieutenant home cannot be the active numberone home' "$err" >/dev/null || fail "spawn did not reject active home"

  if PATH="$fakebin:$PATH" N1_HOME="$home" N1_FAKE_TMUX_LOG="$log" N1_FAKE_TMUX_CAPTURE="$TMP_ROOT/spawn-validate-fake/pane.txt" \
    "$ROOT/bin/n1-spawn.sh" domain "$ROOT" codex --lieutenant >/dev/null 2>"$err"; then
    fail "lieutenant spawn accepted the numberone repo root"
  fi
  grep -F 'lieutenant home cannot be the numberone repo' "$err" >/dev/null || fail "spawn did not reject numberone repo root"

  printf 'domain\n' > "$active_descendant/.n1-lieutenant-home"
  printf 'charter\n' > "$active_descendant/data/charter.md"
  if PATH="$fakebin:$PATH" N1_HOME="$home" N1_FAKE_TMUX_LOG="$log" N1_FAKE_TMUX_CAPTURE="$TMP_ROOT/spawn-validate-fake/pane.txt" \
    "$ROOT/bin/n1-spawn.sh" domain "$active_descendant" codex --lieutenant >/dev/null 2>"$err"; then
    fail "lieutenant spawn accepted a home inside the active numberone home"
  fi
  grep -F 'lieutenant home cannot be inside the active numberone home' "$err" >/dev/null || fail "spawn did not reject active home descendant"

  printf 'domain\n' > "$active_ancestor/.n1-lieutenant-home"
  printf 'charter\n' > "$active_ancestor/data/charter.md"
  if PATH="$fakebin:$PATH" N1_HOME="$ancestor_active_home" N1_FAKE_TMUX_LOG="$log" N1_FAKE_TMUX_CAPTURE="$TMP_ROOT/spawn-validate-fake/pane.txt" \
    "$ROOT/bin/n1-spawn.sh" domain "$active_ancestor" codex --lieutenant >/dev/null 2>"$err"; then
    fail "lieutenant spawn accepted a home containing the active numberone home"
  fi
  grep -F 'lieutenant home cannot be an ancestor of the active numberone home' "$err" >/dev/null || fail "spawn did not reject active home ancestor"

  printf 'domain\n' > "$root_descendant/.n1-lieutenant-home"
  printf 'charter\n' > "$root_descendant/data/charter.md"
  if PATH="$fakebin:$PATH" N1_ROOT_OVERRIDE="$fakeroot" N1_HOME="$home" N1_FAKE_TMUX_LOG="$log" N1_FAKE_TMUX_CAPTURE="$TMP_ROOT/spawn-validate-fake/pane.txt" \
    "$ROOT/bin/n1-spawn.sh" domain "$root_descendant" codex --lieutenant >/dev/null 2>"$err"; then
    fail "lieutenant spawn accepted a home inside the numberone repo"
  fi
  grep -F 'lieutenant home cannot be inside the numberone repo' "$err" >/dev/null || fail "spawn did not reject repo root descendant"

  printf 'domain\n' > "$root_ancestor/.n1-lieutenant-home"
  printf 'charter\n' > "$root_ancestor/data/charter.md"
  if PATH="$fakebin:$PATH" N1_ROOT_OVERRIDE="$root_inside" N1_HOME="$home" N1_FAKE_TMUX_LOG="$log" N1_FAKE_TMUX_CAPTURE="$TMP_ROOT/spawn-validate-fake/pane.txt" \
    "$ROOT/bin/n1-spawn.sh" domain "$root_ancestor" codex --lieutenant >/dev/null 2>"$err"; then
    fail "lieutenant spawn accepted a home containing the numberone repo"
  fi
  grep -F 'lieutenant home cannot be an ancestor of the numberone repo' "$err" >/dev/null || fail "spawn did not reject repo ancestor"

  pass "lieutenant spawn validates homes before launch"
}

test_lieutenant_spawn_refuses_operational_dirs_outside_subhome() {
  ...
  for opdir in data state config projects; do
    ...
    : > "$log"
    if PATH="$fakebin:$PATH" N1_HOME="$home" N1_FAKE_TMUX_LOG="$log" N1_FAKE_TMUX_CAPTURE="$TMP_ROOT/spawn-opdir-fake/pane.txt" \
      "$ROOT/bin/n1-spawn.sh" domain "$subhome" codex --lieutenant >/dev/null 2>"$err"; then
      fail "lieutenant spawn accepted a subhome with $opdir symlinked outside the subhome"
    fi
    grep -F "lieutenant $opdir directory must resolve inside the lieutenant home" "$err" >/dev/null \
  ...
  pass "lieutenant spawn refuses operational directories outside the subhome"
}

test_fm_send_refuses_bare_window_without_home_meta() {
  # The happy path (a bare n1-<id> resolves the window recorded in THIS home's
  ...
  if PATH="$fakebin:$PATH" N1_HOME="$home" N1_FAKE_TMUX_WINDOW="other-session:n1-missing" N1_FAKE_TMUX_LOG="$log" N1_FAKE_TMUX_CAPTURE="$TMP_ROOT/send-fake/pane.txt" \
    "$ROOT/bin/n1-send.sh" n1-missing 'wrong home' >/dev/null 2>"$err"; then
    fail "n1-send sent to a bare numberone window without home metadata"
  fi
  grep -F "no metadata for n1-missing in $home/state" "$err" >/dev/null \
    || fail "n1-send did not explain missing home metadata"
  grep -F 'send-keys -t other-session:n1-missing' "$log" >/dev/null \
    && fail "n1-send fell back to a foreign same-name window"
  pass "n1-send refuses a bare numberone window with no metadata in this home"
}

test_lieutenant_teardown_retires_empty_home() {
...
projects=alpha
EOF
  printf '%s\n' '- domain - design domain (home: '"$subhome"'; scope: design domain; projects: alpha; added 2026-06-22)' > "$home/data/lieutenants.md"
  fakebin=$(make_fake_tmux "$TMP_ROOT/teardown-fake")
  log="$TMP_ROOT/teardown-fake/tmux.log"
  lease="$TMP_ROOT/teardown-fake/lease"
  printf 'domain\n' > "$lease"
  PATH="$fakebin:$PATH" N1_ROOT_OVERRIDE="$fmroot" N1_HOME="$home" N1_FAKE_TMUX_LOG="$log" N1_FAKE_TMUX_CAPTURE="$TMP_ROOT/teardown-fake/pane.txt" \
    N1_FAKE_TREEHOUSE_LEASE_FILE="$lease" \
    "$ROOT/bin/n1-teardown.sh" domain >/dev/null 2>/dev/null \
    || fail "teardown failed for empty lieutenant home"
  grep -F "treehouse return --force $subhome_abs" "$log" >/dev/null || fail "teardown did not release the lieutenant home lease via treehouse return"
  [ ! -e "$lease" ] || fail "teardown left the lieutenant home lease held after retirement"
  [ ! -d "$subhome" ] || fail "teardown did not remove the retired lieutenant home"
  [ ! -e "$home/state/domain.meta" ] || fail "teardown did not clear parent meta"
  grep -F -- '- domain ' "$home/data/lieutenants.md" >/dev/null && fail "teardown did not remove lieutenant registry route"
  pass "lieutenant teardown retires empty homes and releases routing"
}

test_lieutenant_teardown_refuses_failed_leased_home_return() {
...
EOF
  ...
  set +e
  PATH="$fakebin:$PATH" N1_ROOT_OVERRIDE="$fmroot" N1_HOME="$home" N1_FAKE_TMUX_LOG="$log" N1_FAKE_TMUX_CAPTURE="$TMP_ROOT/teardown-return-fail-fake/pane.txt" \
    N1_FAKE_TREEHOUSE_RETURN_FAIL=1 \
    "$ROOT/bin/n1-teardown.sh" domain >/dev/null 2>"$err"
  rc=$?
  set -e
  ...
  grep -F 'treehouse return failed for lieutenant home' "$err" >/dev/null || fail "teardown did not report failed leased home return"
  ...
  grep -F -- '- domain ' "$home/data/lieutenants.md" >/dev/null || fail "teardown removed registry route after leased home return failed"
  pass "lieutenant teardown refuses to hide failed leased-home return"
}

test_lieutenant_teardown_removes_plain_clone_home_without_treehouse_return() {
...
EOF
  ...
  PATH="$fakebin:$PATH" N1_HOME="$home" N1_FAKE_TMUX_LOG="$log" N1_FAKE_TMUX_CAPTURE="$TMP_ROOT/plain-clone-teardown-fake/pane.txt" \
    N1_FAKE_TREEHOUSE_RETURN_FAIL=1 \
    "$ROOT/bin/n1-teardown.sh" domain >/dev/null 2>/dev/null \
    || fail "teardown failed for plain-clone lieutenant home"
  ...
  [ ! -d "$subhome" ] || fail "teardown did not remove the plain-clone lieutenant home"
  [ ! -e "$home/state/domain.meta" ] || fail "teardown did not clear parent meta for plain-clone home"
  grep -F -- '- domain ' "$home/data/lieutenants.md" >/dev/null && fail "teardown did not remove plain-clone registry route"
  pass "lieutenant teardown raw-removes plain-clone homes"
}

test_lieutenant_force_teardown_discards_child_work() {
...
EOF
  ...
  if PATH="$fakebin:$PATH" N1_HOME="$home" N1_FAKE_TMUX_LOG="$log" N1_FAKE_TMUX_CAPTURE="$TMP_ROOT/force-teardown-fake/pane.txt" \
    "$ROOT/bin/n1-teardown.sh" domain >/dev/null 2>&1; then
    fail "teardown allowed a lieutenant with in-flight child work"
  fi
  PATH="$fakebin:$PATH" N1_HOME="$home" N1_FAKE_TMUX_LOG="$log" N1_FAKE_TMUX_CAPTURE="$TMP_ROOT/force-teardown-fake/pane.txt" \
    "$ROOT/bin/n1-teardown.sh" domain --force >/dev/null 2>/dev/null \
    || fail "force teardown failed to discard child work"
  [ ! -d "$subhome" ] || fail "force teardown did not remove the retired lieutenant home"
  [ ! -d "$childwt" ] || fail "force teardown did not remove child worktree"
  [ ! -e "$home/state/domain.meta" ] || fail "teardown did not clear parent meta"
  grep -F -- '- domain ' "$home/data/lieutenants.md" >/dev/null && fail "force teardown did not remove lieutenant registry route"
  grep -F 'kill-window -t numberone:n1-child' "$log" >/dev/null || fail "force teardown did not kill child window"
  grep -F 'kill-window -t numberone:n1-domain' "$log" >/dev/null || fail "force teardown did not kill parent window"
  pass "lieutenant force teardown discards child work"
}

test_lieutenant_force_teardown_allows_operational_dir_symlinks_inside_home() {
...
EOF
    ...
    PATH="$fakebin:$PATH" N1_HOME="$home" N1_FAKE_TMUX_LOG="$log" N1_FAKE_TMUX_CAPTURE="$TMP_ROOT/symlink-inside-teardown-fake-$opdir/pane.txt" \
      "$ROOT/bin/n1-teardown.sh" domain --force >/dev/null 2>"$err" \
      || fail "force teardown refused $opdir symlinked inside the lieutenant home"
    ...
    grep -F 'kill-window -t numberone:n1-domain' "$log" >/dev/null || fail "force teardown did not kill parent window for inside $opdir symlink"
  done
  pass "force teardown allows operational directory symlinks inside the subhome"
}

test_lieutenant_force_teardown_refuses_operational_dir_symlink_outside_home() {
...
EOF
  ...
  if PATH="$fakebin:$PATH" N1_HOME="$home" N1_FAKE_TMUX_LOG="$log" N1_FAKE_TMUX_CAPTURE="$TMP_ROOT/symlink-state-teardown-fake/pane.txt" \
    "$ROOT/bin/n1-teardown.sh" domain --force >/dev/null 2>"$err"; then
    fail "force teardown accepted a symlinked lieutenant state directory"
  fi
  ...
  grep -F 'resolves outside the lieutenant home' "$err" >/dev/null || fail "teardown did not identify unsafe state symlink"
...
test_lieutenant_teardown_path_boundary_matrix() {
  # The teardown path-boundary matrix: a lieutenant home is refused (and left
  # fully intact, with no window killed before validation) when it is unmarked,
  # an ancestor of the active numberone home, inside the active numberone home,
  # or inside the numberone repo. One row per hazard, one shared assertion block.
  local row base home subhome fmroot fakebin log err expect tid
  while IFS='|' read -r row expect; do
    [ -n "$row" ] || continue
    base="$TMP_ROOT/td-pb-$row"
    fmroot="$ROOT"   # real numberone repo unless a row overrides it
    tid=domain
    case "$row" in
      unmarked)
        home="$base/main"; subhome="$base/sub"
        mkdir -p "$home/state" "$home/data" "$subhome/state"
        # No .n1-lieutenant-home marker on purpose.
        ;;
      ancestor)
        # The home being torn down is an ANCESTOR of the active numberone home.
      ...
      repo-descendant)
        ...
        cat > "$fmroot/bin/n1-guard.sh" <<'SH'
#!/usr/bin/env bash
exit 0
SH
        chmod +x "$fmroot/bin/n1-guard.sh"
        printf 'repo-domain\n' > "$subhome/.n1-lieutenant-home"
        ;;
    esac
    fm_write_lieutenant_meta "$home/state/$tid.meta" "$subhome"
    printf -- '- %s - design domain (home: %s; scope: design domain; projects: alpha; added 2026-06-22)\n' \
      "$tid" "$subhome" > "$home/data/lieutenants.md"
    fakebin=$(make_fake_tmux "$base/fake")
    log="$base/fake/tmux.log"
    err="$base/teardown.err"
    if PATH="$fakebin:$PATH" N1_ROOT_OVERRIDE="$fmroot" N1_HOME="$home" \
      N1_FAKE_TMUX_LOG="$log" N1_FAKE_TMUX_CAPTURE="$base/fake/pane.txt" \
      "$ROOT/bin/n1-teardown.sh" "$tid" >/dev/null 2>"$err"; then
      fail "teardown ($row) accepted a hazardous lieutenant home"
    fi
    ...
    grep -F -- "- $tid " "$home/data/lieutenants.md" >/dev/null || fail "teardown ($row) removed the registry route after refusal"
    grep -F 'kill-window' "$log" >/dev/null && fail "teardown ($row) killed a window before validation"
  done <<'ROWS'
unmarked|not a seeded lieutenant home
ancestor|ancestor of the active numberone home
active-descendant|inside the active numberone home
repo-descendant|inside the numberone repo
ROWS
  pass "lieutenant teardown path-boundary matrix refuses unmarked/ancestor/active-descendant/repo-descendant homes"
}

test_lieutenant_teardown_refuses_registered_nested_home() {
...
EOF
  ...
  if PATH="$fakebin:$PATH" N1_HOME="$home" N1_FAKE_TMUX_LOG="$log" N1_FAKE_TMUX_CAPTURE="$TMP_ROOT/nested-teardown-fake/pane.txt" \
    "$ROOT/bin/n1-teardown.sh" domain >/dev/null 2>"$err"; then
    fail "teardown removed a home containing another registered lieutenant home"
  fi
  ...
  grep -F 'contains registered lieutenant home' "$err" >/dev/null || fail "teardown did not explain registered nested-home refusal"
  pass "lieutenant teardown refuses homes containing registered nested homes"
}

test_lieutenant_teardown_refuses_child_registry_nested_home() {
  local home subhome nested fakebin err log
...
EOF
  printf '%s\n' '- domain - design domain (home: '"$subhome"'; scope: design domain; projects: alpha; added 2026-06-22)' > "$home/data/lieutenants.md"
  printf '%s\n' '- nested - nested domain (home: '"$nested"'; scope: nested domain; projects: beta; added 2026-06-22)' > "$subhome/data/lieutenants.md"
  fakebin=$(make_fake_tmux "$TMP_ROOT/child-registry-teardown-fake")
  log="$TMP_ROOT/child-registry-teardown-fake/tmux.log"
  if PATH="$fakebin:$PATH" N1_HOME="$home" N1_FAKE_TMUX_LOG="$log" N1_FAKE_TMUX_CAPTURE="$TMP_ROOT/child-registry-teardown-fake/pane.txt" \
    "$ROOT/bin/n1-teardown.sh" domain >/dev/null 2>"$err"; then
    fail "teardown removed a home containing a child-registry lieutenant home"
  fi
  ...
  grep -F 'contains registered lieutenant home' "$err" >/dev/null || fail "teardown did not explain child-registry nested-home refusal"
  pass "lieutenant teardown refuses nested homes from the child registry"
}

test_lieutenant_force_teardown_prevalidates_before_child_cleanup() {
...
EOF
  ...
  if PATH="$fakebin:$PATH" N1_HOME="$home" N1_FAKE_TMUX_LOG="$log" N1_FAKE_TMUX_CAPTURE="$TMP_ROOT/prevalidate-teardown-fake/pane.txt" \
    "$ROOT/bin/n1-teardown.sh" domain --force >/dev/null 2>"$err"; then
  ...
  grep -F 'not a seeded lieutenant home' "$err" >/dev/null || fail "force teardown did not explain missing seed marker"
  pass "force teardown validates subhome before child cleanup"
}

test_lieutenant_force_teardown_refuses_child_active_home_descendant() {
  local home subhome childproj childwt fakebin err log
...
EOF
  ...
  if PATH="$fakebin:$PATH" N1_HOME="$home" N1_FAKE_TMUX_LOG="$log" N1_FAKE_TMUX_CAPTURE="$TMP_ROOT/child-active-descendant-fake/pane.txt" \
    "$ROOT/bin/n1-teardown.sh" domain --force >/dev/null 2>"$err"; then
    fail "force teardown removed a child worktree inside active N1_HOME"
  fi
  ...
  grep -F 'inside the active numberone home' "$err" >/dev/null || fail "force teardown did not explain active home descendant rejection"
  pass "force teardown refuses child worktrees inside the active home"
}

test_lieutenant_force_teardown_refuses_child_repo_descendant() {
...
EOF
  ...
  if PATH="$fakebin:$PATH" N1_ROOT_OVERRIDE="$fakeroot" N1_HOME="$home" N1_FAKE_TMUX_LOG="$log" N1_FAKE_TMUX_CAPTURE="$TMP_ROOT/child-repo-descendant-fake/pane.txt" \
    "$ROOT/bin/n1-teardown.sh" domain --force >/dev/null 2>"$err"; then
    fail "force teardown removed a child worktree inside N1_ROOT"
  fi
  ...
  grep -F 'inside the numberone repo' "$err" >/dev/null || fail "force teardown did not explain repo descendant rejection"
  pass "force teardown refuses child worktrees inside the numberone repo"
}

test_lieutenant_force_teardown_refuses_unregistered_child_worktree() {
  local home subhome childproj childwt fakebin err log
...
EOF
  ...
  if PATH="$fakebin:$PATH" N1_HOME="$home" N1_FAKE_TMUX_LOG="$log" N1_FAKE_TMUX_CAPTURE="$TMP_ROOT/unregistered-child-fake/pane.txt" \
    "$ROOT/bin/n1-teardown.sh" domain --force >/dev/null 2>"$err"; then
...
test_lieutenant_idle_pane_is_not_stale() {
...
EOF
  ...
  PATH="$fakebin:$PATH" N1_HOME="$home" N1_FAKE_TMUX_WINDOW="$window" N1_FAKE_TMUX_LOG="$TMP_ROOT/watch-fake/tmux.log" N1_FAKE_TMUX_CAPTURE="$TMP_ROOT/watch-fake/pane.txt" \
    N1_POLL=1 N1_SIGNAL_GRACE=1 N1_CHECK_INTERVAL=999999 N1_HEARTBEAT=999999 "$ROOT/bin/n1-watch.sh" > "$out" &
  pid=$!
  if ! wait_live "$pid" 25; then
    wait "$pid" || true
    grep -F "stale: $window" "$out" >/dev/null && fail "idle lieutenant pane triggered stale wake"
    fail "watcher exited unexpectedly while supervising idle lieutenant"
  fi
  kill "$pid" 2>/dev/null || true
  wait "$pid" 2>/dev/null || true
  grep -F "stale: $window" "$out" >/dev/null && fail "idle lieutenant pane triggered stale wake"
  pass "idle kind=lieutenant pane is healthy and not stale"
}

test_lieutenant_charter_brief_is_idle_by_default() {
  local home brief
  home="$TMP_ROOT/idle-charter-home"
  mkdir -p "$home/data" "$home/state"
  scaffold_lieutenant_charter "$home" idle-sm 'feature work for alpha' alpha
  brief="$home/data/idle-sm/brief.md"
  [ -f "$brief" ] || fail "lieutenant charter brief was not scaffolded"
  # Idle contract: waits for routed work, never self-initiates.
  grep -F 'go idle and wait silently for the main numberone' "$brief" >/dev/null \
    || fail "charter brief does not tell the lieutenant to go idle and wait for routed work"
  grep -F 'Act only on tasks the main numberone routes to you' "$brief" >/dev/null \
  ...
  grep -F 'run normal numberone bootstrap and recovery' "$brief" >/dev/null \
  ...
  pass "lieutenant charter brief is idle by default and does not self-initiate work"
}
...
EOF
  ...
  if N1_HOME="$home" "$ROOT/bin/n1-backlog-handoff.sh" design bug-z no-such-key >/dev/null 2>&1; then
  ...
  before=$(cat "$home/data/backlog.md")
  if N1_HOME="$home" "$ROOT/bin/n1-backlog-handoff.sh" design live-task >/dev/null 2>&1; then
  ...
  [ ! -e "$subhome/data/backlog.md" ] || ! grep -F 'live-task' "$subhome/data/backlog.md" >/dev/null     || fail "in-flight refusal copied the live task into the lieutenant backlog"

  # An unregistered lieutenant id is refused.
  if N1_HOME="$home" "$ROOT/bin/n1-backlog-handoff.sh" ghost bug-z >/dev/null 2>&1; then
    fail "handoff accepted an unregistered lieutenant id"
  fi
  pass "n1-backlog-handoff aborts atomically on unmatched, in-flight, and unregistered targets"
}

test_backlog_handoff_creates_absent_section_and_refuses_non_lieutenant_home() {
  ...
  # A Done item handed into a lieutenant backlog lacking a Done section gets one.
  seed_lieutenant_home_marker "$subhome" archive
  subhome_abs=$(cd "$subhome" && pwd -P)
...
EOF
  N1_HOME="$home" "$ROOT/bin/n1-backlog-handoff.sh" archive delivered-task >/dev/null \
    || fail "handoff of a Done item failed"
  grep -F '## Done' "$subhome/data/backlog.md" >/dev/null \
    || fail "handoff did not create the missing Done section in the lieutenant backlog"
  awk '/^## Done/{d=1;next} /^## /{d=0} d && /delivered-task/{found=1} END{exit found?0:1}' "$subhome/data/backlog.md" \
    || fail "Done item did not land under the created Done section"
  grep -F 'keep-me' "$subhome/data/backlog.md" >/dev/null || fail "handoff clobbered the existing lieutenant backlog content"

  # A registered home that is not a seeded lieutenant home (e.g. a project clone)
  # is refused, and nothing is written into it.
  fm_git_init_commit "$projhome"
  projhome_abs=$(cd "$projhome" && pwd -P)
  printf -- '- proj-sm - bogus (home: %s; scope: bogus; projects: alpha; added 2026-06-22)\n' "$projhome_abs" >> "$home/data/lieutenants.md"
  if N1_HOME="$home" "$ROOT/bin/n1-backlog-handoff.sh" proj-sm delivered-task >/dev/null 2>&1; then
    fail "handoff wrote into a destination that is not a seeded lieutenant home"
  fi
  [ ! -e "$projhome/data/backlog.md" ] || fail "handoff created a backlog inside a non-lieutenant home"

  mkdir -p "$markerhome/data"
  markerhome_abs=$(cd "$markerhome" && pwd -P)
  printf 'marker-sm\n' > "$markerhome/.n1-lieutenant-home"
  printf -- '- marker-sm - bogus (home: %s; scope: bogus; projects: alpha; added 2026-06-22)\n' "$markerhome_abs" >> "$home/data/lieutenants.md"
  cat > "$home/data/backlog.md" <<'EOF'
## Queued
- [ ] marker-task - should not move (repo: alpha)
EOF
  if N1_HOME="$home" "$ROOT/bin/n1-backlog-handoff.sh" marker-sm marker-task >/dev/null 2>&1; then
    fail "handoff accepted a marker-only directory as a lieutenant home"
  fi
  ...
  seed_lieutenant_home_marker "$symlinkhome" symlink-sm
  symlinkhome_abs=$(cd "$symlinkhome" && pwd -P)
  mkdir -p "$outside"
  rm -rf "$symlinkhome/data"
  ln -s "$outside" "$symlinkhome/data"
  printf -- '- symlink-sm - bogus (home: %s; scope: bogus; projects: alpha; added 2026-06-22)\n' "$symlinkhome_abs" >> "$home/data/lieutenants.md"
  cat > "$home/data/backlog.md" <<'EOF'
## Queued
- [ ] symlink-task - should not move (repo: alpha)
EOF
  if N1_HOME="$home" "$ROOT/bin/n1-backlog-handoff.sh" symlink-sm symlink-task >/dev/null 2>&1; then
    fail "handoff accepted a lieutenant home with data outside the home"
  fi
  [ ! -e "$outside/backlog.md" ] || fail "handoff wrote through a symlinked lieutenant data directory"
  grep -F 'symlink-task' "$home/data/backlog.md" >/dev/null || fail "symlink refusal lost the main backlog item"
  pass "n1-backlog-handoff creates absent sections and refuses unsafe homes"
}
