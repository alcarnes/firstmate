#!/usr/bin/env bash
# n1-send post-submit settle pause (N1_SEND_SETTLE).
#
# n1-send's success only proves the composer cleared - the Enter landed and the
# text was submitted. The harness then takes a beat to spin up the turn before its
# busy footer appears, so an immediate peek after n1-send returns would see the
# stale idle pane. n1-send therefore pauses N1_SEND_SETTLE seconds (default 1, 0
...
# no real agent):
#   1. A successful text send pauses for the N1_SEND_SETTLE value (default 1).
#   2. N1_SEND_SETTLE=0 produces no pause at all (sleep is never invoked for it).
#   3. The pause is tunable (N1_SEND_SETTLE=7 pauses 7).
...
TMP_ROOT=$(fm_test_tmproot n1-send-settle)

# A fake tmux that lets n1-send's submit path reach a clean "empty" verdict, plus a
...
# run_send <fakebin> <sleep-log> [env-assignments...] -- <n1-send args...>
# Runs n1-send.sh with the stubs on PATH. N1_ROOT_OVERRIDE points at a non-repo
# temp dir so n1-guard's tangle check stays silent, and N1_HOME at an empty home so
# no in-flight task is seen; guard noise goes to stderr (discarded). Echoes nothing;
# returns n1-send's exit code.
run_send() {
  local fb=$1 log=$2 home; shift 2
  home="$TMP_ROOT/home-$RANDOM"; mkdir -p "$home/state"
  : > "$log"
  env "$@" PATH="$fb:$PATH" \
    N1_ROOT_OVERRIDE="$home" N1_HOME="$home" N1_SLEEP_LOG="$log" \
...
test_default_send_pauses_one_second() {
  ...
  pass "n1-send: a successful text send pauses the default 1s after submit"
}
...
test_zero_disables_pause() {
  ...
  fb=$(make_stubs "$dir"); log="$dir/sleep.log"
  run_send "$fb" "$log" N1_SEND_SETTLE=0; rc=$?
  expect_code 0 "$rc" "N1_SEND_SETTLE=0 send should succeed"
  ...
  if grep -qx '0' "$log"; then
    fail "N1_SEND_SETTLE=0 still paused (a sleep 0 was recorded)"$'\n'"--- sleeps ---"$'\n'"$(cat "$log")"
  fi
  pass "n1-send: N1_SEND_SETTLE=0 produces no settle pause"
}
...
test_pause_is_tunable() {
  ...
  fb=$(make_stubs "$dir"); log="$dir/sleep.log"
  run_send "$fb" "$log" N1_SEND_SETTLE=7; rc=$?
  expect_code 0 "$rc" "N1_SEND_SETTLE=7 send should succeed"
  last=$(tail -1 "$log")
  [ "$last" = 7 ] || fail "N1_SEND_SETTLE=7: expected a trailing 7s settle pause, got '$last'"$'\n'"--- sleeps ---"$'\n'"$(cat "$log")"
  pass "n1-send: the settle pause is tunable via N1_SEND_SETTLE"
}
...
test_key_path_never_pauses() {
  ...
  : > "$log"
  env PATH="$fb:$PATH" N1_ROOT_OVERRIDE="$home" N1_HOME="$home" N1_SLEEP_LOG="$log" \
    "$SEND" "sess:win" --key Escape 2>/dev/null; rc=$?
  expect_code 0 "$rc" "--key send should succeed"
  [ ! -s "$log" ] || fail "--key path paused but must not"$'\n'"--- sleeps ---"$'\n'"$(cat "$log")"
  pass "n1-send: the --key path never pauses (settle scoped to text submit)"
}
