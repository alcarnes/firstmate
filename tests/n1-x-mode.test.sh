#!/usr/bin/env bash
# Behavior tests for X mode: the relay poll client (n1-x-poll.sh), the answer
# poster (n1-x-reply.sh), and bootstrap's .env-presence activation.
#
...
BASE_PATH=${N1_TEST_BASE_PATH:-/usr/bin:/bin:/usr/sbin:/sbin}
...
TMP_ROOT=$(fm_test_tmproot n1-x-mode-tests)
...
test_poll_no_token_is_hard_noop() {
  ...
  out=$(PATH="$fakebin:$BASE_PATH" N1_HOME="$home" FMX_PAIRING_TOKEN='' \
    "$ROOT/bin/n1-x-poll.sh"); rc=$?
  ...
  pass "n1-x-poll is a hard no-op without a token (inert default)"
}
...
test_poll_empty_env_token_overrides_env_file() {
  ...
  pass "n1-x-poll treats an explicitly empty env token as configured"
}
...
test_poll_204_is_silent() {
  ...
  out=$(PATH="$fakebin:$BASE_PATH" N1_HOME="$home" FMX_RELAY_URL="https://relay.test" \
    FAKE_CURL_LOG="$log" FAKE_POLL_CODE=204 \
    "$ROOT/bin/n1-x-poll.sh"); rc=$?
  ...
  pass "n1-x-poll stays silent on HTTP 204 (the common case)"
}
...
test_poll_empty_env_relay_overrides_env_file() {
  ...
  out=$(PATH="$fakebin:$BASE_PATH" N1_HOME="$home" FMX_RELAY_URL='' \
    FAKE_CURL_LOG="$log" FAKE_POLL_CODE=204 \
    "$ROOT/bin/n1-x-poll.sh"); rc=$?
  ...
  pass "n1-x-poll lets an explicitly empty relay env override .env"
}
...
test_poll_auth_error_reports_once() {
  ...
  pass "n1-x-poll surfaces auth/config errors once and clears on recovery"
}
...
test_poll_question_stashes_and_marks() {
  ...
  pass "n1-x-poll stashes the question and prints the compact marker"
}
...
test_poll_preserves_conversation_context() {
  ...
  body='{"request_id":"req-c","tweet_id":"9","author_id":"42","text":"and then what?","in_reply_to":{"author_handle":"@asker","text":"are you delivering today?"}}'
  out=$(PATH="$fakebin:$BASE_PATH" N1_HOME="$home" FMX_RELAY_URL="https://relay.test" \
    FAKE_POLL_CODE=200 FAKE_POLL_BODY="$body" \
    "$ROOT/bin/n1-x-poll.sh"); rc=$?
  ...
  [ "$(jq -r '.in_reply_to.text' "$f")" = "are you delivering today?" ] \
  ...
  pass "n1-x-poll preserves in_reply_to conversation context in the inbox"
}
...
SH
  ...
  pass "n1-x-poll reports inbox commit failures without emitting a mention wake"
}
...
test_poll_rejects_unsafe_request_id() {
  ...
  pass "n1-x-poll rejects an unsafe request_id (path-traversal guard)"
}
...
test_reply_success_posts_request_bound_only() {
  ...
  out=$(PATH="$fakebin:$BASE_PATH" N1_HOME="$home" FMX_RELAY_URL="https://relay.test" \
    ...
    "$ROOT/bin/n1-x-reply.sh" "req-7" "Aye, charting a couple of fixes."); rc=$?
  ...
  pass "n1-x-reply posts a request-bound answer and echoes only the request_id"
}
...
test_reply_non_2xx_fails() {
  ...
  out=$(PATH="$fakebin:$BASE_PATH" N1_HOME="$home" FMX_RELAY_URL="https://relay.test" \
    ...
    "$ROOT/bin/n1-x-reply.sh" "req-7" "hi" 2>"$err"); rc=$?
  [ "$rc" -ne 0 ] || fail "reply must exit non-zero on a non-2xx response"
  assert_grep "HTTP 500" "$err" "reply must report the failing status"
  pass "n1-x-reply exits non-zero on a non-2xx relay response"
}

test_reply_usage_error() {
  local home rc
  home="$TMP_ROOT/reply-usage"; mkdir -p "$home"
  PATH="$BASE_PATH" N1_HOME="$home" "$ROOT/bin/n1-x-reply.sh" "only-one" >/dev/null 2>&1; rc=$?
  expect_code 2 "$rc" "reply usage error exit"
  pass "n1-x-reply rejects missing arguments with a usage error"
}
...
test_reply_whitespace_text_rejected() {
  ...
  err="$home/err.txt"
  out=$(PATH="$BASE_PATH" N1_HOME="$home" FMX_DRY_RUN=1 \
    "$ROOT/bin/n1-x-reply.sh" "req-space" "   " 2>"$err"); rc=$?
  ...
  pass "n1-x-reply rejects whitespace-only reply text"
}
...
test_bootstrap_activates_on_env_token() {
  ...
  out=$(N1_HOME="$home" "$ROOT/bin/n1-bootstrap.sh" 2>/dev/null)
  ...
  assert_grep "n1-x-poll.sh" "$home/state/x-watch.check.sh" "the shim must exec the poll script"
  assert_present "$home/config/x-mode.env" "bootstrap must drop the cadence config"
  assert_grep "export N1_CHECK_INTERVAL=30" "$home/config/x-mode.env" "cadence must be 30s"
  # Cadence inheritance: sourcing the config exports the 30s interval to a child,
  # exactly how n1-watch-arm.sh's forked watcher inherits it.
  local inherited
  # shellcheck source=/dev/null
  inherited=$( . "$home/config/x-mode.env" && bash -c 'echo "${N1_CHECK_INTERVAL:-300}"' )
  [ "$inherited" = "30" ] \
    || fail "sourcing the cadence config must export N1_CHECK_INTERVAL=30 to a child"
  ...
  N1_HOME="$home" "$ROOT/bin/n1-bootstrap.sh" >/dev/null 2>&1
...
SH
  ...
  out=$(PATH="$fakebin" N1_HOME="$home" N1_ROOT_OVERRIDE="$home" \
    "$BASH" "$ROOT/bin/n1-bootstrap.sh" 2>/dev/null)
...
test_bootstrap_does_not_announce_when_arm_fails() {
  ...
  out=$(N1_HOME="$home" N1_CONFIG_OVERRIDE="$home/config" "$ROOT/bin/n1-bootstrap.sh" 2>/dev/null)
...
test_poll_empty_text_is_silent() {
  ...
  pass "n1-x-poll requires a non-empty question before waking"
}
...
test_reply_text_file_and_stdin() {
  ...
  out=$(PATH="$fakebin:$BASE_PATH" N1_HOME="$home" FMX_RELAY_URL="https://relay.test" \
    FAKE_CURL_LOG="$log" FAKE_ANSWER_CODE=200 \
    "$ROOT/bin/n1-x-reply.sh" "req-1" --text-file "$home/reply.txt"); rc=$?
...
SH
  chmod +x "$fakebin/rm"
  printf 'FMX_PAIRING_TOKEN=\n' > "$home/.env"
  out=$(PATH="$fakebin:$PATH" N1_HOME="$home" "$ROOT/bin/n1-bootstrap.sh" 2>/dev/null)
...
test_reply_dry_run_records_not_posts() {
  ...
  out=$(PATH="$fakebin:$BASE_PATH" N1_HOME="$home" FMX_RELAY_URL="https://relay.test" \
    ...
    "$ROOT/bin/n1-x-reply.sh" "req-1" "Aye, a couple of fixes underway." 2>"$home/err"); rc=$?
  ...
  pass "n1-x-reply dry-run records the would-be reply and never posts"
}
...
test_reply_dry_run_needs_no_token() {
  ...
  out=$(PATH="$BASE_PATH" N1_HOME="$home" FMX_DRY_RUN=1 \
    "$ROOT/bin/n1-x-reply.sh" "req-2" "preview without creds" 2>/dev/null); rc=$?
  ...
  pass "n1-x-reply dry-run works without a token"
}
...
test_reply_dry_run_from_env_file() {
  ...
  printf 'FMX_PAIRING_TOKEN=tok-d\nFMX_DRY_RUN=1\n' > "$home/.env"
  out=$(PATH="$fakebin:$BASE_PATH" N1_HOME="$home" FMX_RELAY_URL="https://relay.test" \
    FAKE_CURL_LOG="$log" "$ROOT/bin/n1-x-reply.sh" "req-3" "from dotenv" 2>/dev/null); rc=$?
  ...
  pass "n1-x-reply honors FMX_DRY_RUN from .env"
}
...
test_reply_empty_env_dry_run_overrides_env_file() {
  ...
  out=$(PATH="$fakebin:$BASE_PATH" N1_HOME="$home" FMX_RELAY_URL="https://relay.test" \
    ...
    "$ROOT/bin/n1-x-reply.sh" "req-5" "empty env disables dry run" 2>/dev/null); rc=$?
  ...
  pass "n1-x-reply lets an explicitly empty dry-run env override .env"
}
...
test_reply_dry_run_fails_when_outbox_unwritable() {
  ...
  out=$(PATH="$BASE_PATH" N1_HOME="$home" FMX_DRY_RUN=1 \
    "$ROOT/bin/n1-x-reply.sh" "req-4" "preview text" 2>"$err"); rc=$?
  ...
  pass "n1-x-reply dry-run fails when it cannot record the preview"
}

test_split_thread_lib() {
  # shellcheck source=bin/n1-x-lib.sh
  . "$ROOT/bin/n1-x-lib.sh"
  local out n last rejoin maxlen txt
  # A reply that fits one tweet stays a single, UNNUMBERED chunk.
  out=$(printf 'Aye, all in order.' | fmx_split_thread 280 25)
  [ "$(printf '%s' "$out" | jq 'length')" = "1" ] || fail "short reply must be one chunk"
  [ "$(printf '%s' "$out" | jq -r '.[0]')" = "Aye, all in order." ] || fail "short reply must be verbatim and unnumbered"
...
test_reply_single_no_texts() {
  ...
  out=$(N1_HOME="$home" FMX_DRY_RUN=1 "$ROOT/bin/n1-x-reply.sh" req-s "Short and sweet." 2>/dev/null)
  ...
  pass "n1-x-reply keeps a concise reply as a single unnumbered tweet"
}
...
test_reply_thread_dry_run() {
  ...
  out=$(N1_HOME="$home" FMX_DRY_RUN=1 FMX_X_REPLY_MAX_CHARS=50 \
    "$ROOT/bin/n1-x-reply.sh" req-t "$long" 2>/dev/null)
  ...
  pass "n1-x-reply auto-splits a long reply into a numbered thread (texts[])"
}
...
test_reply_max_chars_floor_clamps_to_minimum() {
  ...
  out=$(N1_HOME="$home" FMX_DRY_RUN=1 FMX_X_REPLY_MAX_CHARS=49 \
    "$ROOT/bin/n1-x-reply.sh" req-floor "$long" 2>/dev/null)
  ...
  pass "n1-x-reply clamps a below-floor max to 50 characters"
}
...
test_reply_thread_live_posts_texts() {
  ...
  out=$(PATH="$fakebin:$BASE_PATH" N1_HOME="$home" FMX_RELAY_URL="https://relay.test" \
    ...
    "$ROOT/bin/n1-x-reply.sh" req-l "alpha bravo charlie delta echo foxtrot golf hotel india juliet kilo lima mike november oscar papa quebec romeo")
  ...
  pass "n1-x-reply posts a thread payload (texts[]) to the relay"
}
...
test_reply_followup_live_posts_to_followup_endpoint() {
  ...
  out=$(PATH="$fakebin:$BASE_PATH" N1_HOME="$home" FMX_RELAY_URL="https://relay.test" \
    ...
    "$ROOT/bin/n1-x-reply.sh" "req-7" --followup "Done, captain - the fix has delivered."); rc=$?
  ...
  pass "n1-x-reply --followup posts to /connector/followup with the same request-bound body"
}
...
test_reply_followup_flag_position_is_flexible() {
  ...
  out=$(PATH="$fakebin:$BASE_PATH" N1_HOME="$home" FMX_RELAY_URL="https://relay.test" \
    FAKE_CURL_LOG="$log" FAKE_FOLLOWUP_CODE=200 \
    "$ROOT/bin/n1-x-reply.sh" "req-a" --text-file "$home/reply.txt" --followup); rc=$?
  ...
  out=$(PATH="$fakebin:$BASE_PATH" N1_HOME="$home" FMX_RELAY_URL="https://relay.test" \
    FAKE_CURL_LOG="$log" FAKE_ANSWER_CODE=200 \
    "$ROOT/bin/n1-x-reply.sh" "req-a" --text-file "$home/reply.txt"); rc=$?
  ...
  pass "n1-x-reply --followup is accepted in any position and leaves the answer path default"
}

test_reply_followup_dry_run_marks_endpoint() {
  local home out rc
  home="$TMP_ROOT/reply-followup-dry"; mkdir -p "$home"
  out=$(N1_HOME="$home" FMX_DRY_RUN=1 \
    "$ROOT/bin/n1-x-reply.sh" "req-d" --followup "Delivered - all green." 2>"$home/err"); rc=$?
  ...
  [ "$(jq -r '.text' "$home/state/x-outbox/req-d.json")" = "Delivered - all green." ] \
  ...
  out=$(N1_HOME="$home" FMX_DRY_RUN=1 "$ROOT/bin/n1-x-reply.sh" "req-ans" "Aye." 2>/dev/null)
  ...
  pass "n1-x-reply --followup dry-run marks the endpoint without changing the answer path"
}
...
test_reply_followup_thread_dry_run() {
  ...
  long="The captain has me on a sign-in redirect fix, a docs tidy, and keeping the build green while other jobs run in the background today."
  out=$(N1_HOME="$home" FMX_DRY_RUN=1 FMX_X_REPLY_MAX_CHARS=50 \
    "$ROOT/bin/n1-x-reply.sh" req-ft --followup "$long" 2>/dev/null)
  ...
  pass "n1-x-reply --followup auto-splits a long follow-up into a marked thread"
}

# --- n1-x-dismiss: drop a mention at the relay without replying ---------------
...
test_dismiss_success_posts_request_only() {
  ...
  out=$(PATH="$fakebin:$BASE_PATH" N1_HOME="$home" FMX_RELAY_URL="https://relay.test" \
    ...
    "$ROOT/bin/n1-x-dismiss.sh" "req-9"); rc=$?
  ...
  pass "n1-x-dismiss posts a request-bound dismiss and echoes only the request_id"
}
...
test_dismiss_dry_run_records_not_posts() {
  ...
  out=$(PATH="$fakebin:$BASE_PATH" N1_HOME="$home" FMX_RELAY_URL="https://relay.test" \
    FMX_DRY_RUN=1 FAKE_CURL_LOG="$log" \
    "$ROOT/bin/n1-x-dismiss.sh" "req-1" 2>"$home/err"); rc=$?
  ...
  pass "n1-x-dismiss dry-run records the would-be body and never posts"
}
...
test_dismiss_dry_run_needs_no_token() {
  ...
  # No token at all: dry-run still previews (it neither authenticates nor posts).
  out=$(PATH="$BASE_PATH" N1_HOME="$home" FMX_DRY_RUN=1 \
    "$ROOT/bin/n1-x-dismiss.sh" "req-2" 2>/dev/null); rc=$?
  ...
  pass "n1-x-dismiss dry-run works without a token"
}
...
test_dismiss_non_2xx_fails() {
  ...
  out=$(PATH="$fakebin:$BASE_PATH" N1_HOME="$home" FMX_RELAY_URL="https://relay.test" \
    ...
    "$ROOT/bin/n1-x-dismiss.sh" "req-9" 2>"$err"); rc=$?
  ...
  pass "n1-x-dismiss exits non-zero on a non-2xx relay response"
}
...
SH
  ...
  pass "n1-x-dismiss exits non-zero on a transport failure"
}
...
test_dismiss_unsafe_request_id_rejected() {
  ...
  out=$(PATH="$BASE_PATH" N1_HOME="$home" FMX_DRY_RUN=1 \
    "$ROOT/bin/n1-x-dismiss.sh" "../evil" 2>"$err"); rc=$?
  ...
  pass "n1-x-dismiss rejects an unsafe request_id (path-traversal guard)"
}

test_dismiss_usage_error() {
  local home rc
  home="$TMP_ROOT/dismiss-usage"; mkdir -p "$home"
  PATH="$BASE_PATH" N1_HOME="$home" "$ROOT/bin/n1-x-dismiss.sh" >/dev/null 2>&1; rc=$?
  expect_code 2 "$rc" "dismiss missing-arg usage exit"
  PATH="$BASE_PATH" N1_HOME="$home" "$ROOT/bin/n1-x-dismiss.sh" req-1 extra >/dev/null 2>&1; rc=$?
  expect_code 2 "$rc" "dismiss extra-arg usage exit"
  pass "n1-x-dismiss rejects missing or extra arguments with a usage error"
}

# --- n1-x-link: task <-> X-request association in meta -----------------------
...
test_link_records_request_and_timestamp() {
  ...
  printf 'window=w\nworktree=/wt\nkind=mission\nmode=no-mistakes\nyolo=off\n' > "$meta"
  out=$(N1_HOME="$home" FMX_NOW_OVERRIDE=1700000000 \
    "$ROOT/bin/n1-x-link.sh" fix-login-k3 req-42); rc=$?
  ...
  assert_grep "kind=mission" "$meta" "link must preserve other meta lines"
  ...
  N1_HOME="$home" FMX_NOW_OVERRIDE=1700009999 "$ROOT/bin/n1-x-link.sh" fix-login-k3 req-99 >/dev/null
  ...
  pass "n1-x-link records and refreshes the X-request link without disturbing meta"
}
...
test_meta_rewrites_do_not_depend_on_tmpdir() {
  ...
  printf 'window=w\nkind=mission\n' > "$meta"
  out=$(TMPDIR="$badtmp" N1_HOME="$home" FMX_NOW_OVERRIDE=1700000000 \
    "$ROOT/bin/n1-x-link.sh" fix-meta-k4 req-local); rc=$?
  ...
  out=$(TMPDIR="$badtmp" N1_HOME="$home" FMX_NOW_OVERRIDE=1700000001 FMX_FOLLOWUP_MAX_AGE_SECS=0 \
    "$ROOT/bin/n1-x-followup.sh" --check fix-meta-k4 2>/dev/null); rc=$?
  ...
  assert_grep "kind=mission" "$meta" "clear must preserve other meta lines"
...
test_link_rejects_unsafe_and_missing() {
  ...
  printf 'kind=mission\n' > "$home/state/ok.meta"
  PATH="$BASE_PATH" N1_HOME="$home" "$ROOT/bin/n1-x-link.sh" "../evil" req-1 >/dev/null 2>&1; rc=$?
  expect_code 2 "$rc" "link unsafe task id exit"
  PATH="$BASE_PATH" N1_HOME="$home" "$ROOT/bin/n1-x-link.sh" ok "../../etc/x" >/dev/null 2>&1; rc=$?
  ...
  PATH="$BASE_PATH" N1_HOME="$home" "$ROOT/bin/n1-x-link.sh" no-such req-1 >/dev/null 2>&1; rc=$?
  ...
  PATH="$BASE_PATH" N1_HOME="$home" "$ROOT/bin/n1-x-link.sh" ok >/dev/null 2>&1; rc=$?
  expect_code 2 "$rc" "link missing arg exit"
  pass "n1-x-link rejects unsafe ids, missing meta, and missing arguments"
}

# --- n1-x-followup: detect, post one follow-up, clear the link ---------------
...
mk_linked_task() { # <home> <id> <request_id> <link-epoch>
  ...
  printf 'window=w\nworktree=/wt\nkind=mission\nmode=no-mistakes\nyolo=off\n' > "$meta"
  N1_HOME="$home" FMX_NOW_OVERRIDE="$ts" "$ROOT/bin/n1-x-link.sh" "$id" "$rid" >/dev/null
}
...
test_followup_check_states() {
  ...
  out=$(N1_HOME="$home" FMX_NOW_OVERRIDE=1700003600 \
    "$ROOT/bin/n1-x-followup.sh" --check task-a); rc=$?
  ...
  out=$(N1_HOME="$home" "$ROOT/bin/n1-x-followup.sh" --check plain 2>/dev/null); rc=$?
  ...
  out=$(N1_HOME="$home" "$ROOT/bin/n1-x-followup.sh" --check nope 2>/dev/null); rc=$?
  expect_code 1 "$rc" "check missing-meta exit"
  pass "n1-x-followup --check reports postable / not-linked correctly"
}
...
test_followup_check_expired_prunes_link() {
  ...
  out=$(N1_HOME="$home" FMX_NOW_OVERRIDE=$((1700000000 + 25*3600)) \
    "$ROOT/bin/n1-x-followup.sh" --check task-e 2>/dev/null); rc=$?
  ...
  assert_grep "kind=mission" "$meta" "expired check must preserve other meta lines"
  pass "n1-x-followup --check prunes a link past the 24h window"
}
...
test_followup_post_within_window_posts_and_clears() {
  ...
  printf 'Done, captain - delivered and green.' > "$home/reply.txt"
  out=$(PATH="$fakebin:$BASE_PATH" N1_HOME="$home" FMX_RELAY_URL="https://relay.test" \
    FMX_NOW_OVERRIDE=1700003600 FAKE_CURL_LOG="$log" FAKE_FOLLOWUP_CODE=200 \
    "$ROOT/bin/n1-x-followup.sh" task-p --text-file "$home/reply.txt"); rc=$?
  ...
  data=$(grep '^data=' "$log" | tail -1 | sed 's/^data=//')
  [ "$(printf '%s' "$data" | jq -r .text)" = "Done, captain - delivered and green." ] \
    || fail "post must send the composed follow-up text"
  assert_no_grep "x_request=" "$meta" "a successful post must clear the link"
  assert_grep "kind=mission" "$meta" "clearing the link must preserve other meta lines"
  pass "n1-x-followup posts the follow-up and clears the link on success"
}
...
test_followup_post_failure_keeps_link() {
  ...
  out=$(PATH="$fakebin:$BASE_PATH" N1_HOME="$home" FMX_RELAY_URL="https://relay.test" \
    ...
    "$ROOT/bin/n1-x-followup.sh" task-f - <<<"retry me" 2>/dev/null); rc=$?
  ...
  pass "n1-x-followup keeps the link when the post fails"
}
...
test_followup_post_expired_skips_and_clears() {
  ...
  out=$(PATH="$fakebin:$BASE_PATH" N1_HOME="$home" FMX_RELAY_URL="https://relay.test" \
    ...
    "$ROOT/bin/n1-x-followup.sh" task-x - <<<"too late" 2>/dev/null); rc=$?
  ...
  pass "n1-x-followup skips silently and clears the link past the 24h window"
}
...
test_followup_post_not_linked_is_noop() {
  ...
  printf 'kind=mission\n' > "$home/state/plain.meta"
  out=$(N1_HOME="$home" "$ROOT/bin/n1-x-followup.sh" plain - <<<"nothing to do" 2>/dev/null); rc=$?
  ...
  pass "n1-x-followup is a no-op for a task with no X link"
}
...
test_followup_post_dry_run_records_and_clears() {
  ...
  out=$(N1_HOME="$home" FMX_DRY_RUN=1 FMX_NOW_OVERRIDE=1700003600 \
    "$ROOT/bin/n1-x-followup.sh" task-d - <<<"Delivered in dry run." 2>/dev/null); rc=$?
  ...
  pass "n1-x-followup dry-run records the follow-up and clears the link"
}

test_followup_usage_errors() {
  local home rc
  home="$TMP_ROOT/fu-usage"; mkdir -p "$home/state"
  PATH="$BASE_PATH" N1_HOME="$home" "$ROOT/bin/n1-x-followup.sh" >/dev/null 2>&1; rc=$?
  expect_code 2 "$rc" "followup no-args exit"
  PATH="$BASE_PATH" N1_HOME="$home" "$ROOT/bin/n1-x-followup.sh" --check >/dev/null 2>&1; rc=$?
  expect_code 2 "$rc" "followup --check no-id exit"
  PATH="$BASE_PATH" N1_HOME="$home" "$ROOT/bin/n1-x-followup.sh" some-task >/dev/null 2>&1; rc=$?
  expect_code 2 "$rc" "followup post no-text-source exit"
  PATH="$BASE_PATH" N1_HOME="$home" "$ROOT/bin/n1-x-followup.sh" "../evil" --text-file /dev/null >/dev/null 2>&1; rc=$?
  expect_code 2 "$rc" "followup unsafe-id exit"
  pass "n1-x-followup rejects malformed invocations"
}
