...
#
# Usage: n1-x-dismiss.sh <request_id>
#
# When numberone decides NOT to reply to a mention (a pure acknowledgment, or any
...
usage() {
  echo "usage: n1-x-dismiss.sh <request_id>" >&2
}
...
case "$REQ" in
  ''|.*|*[!A-Za-z0-9._-]*) echo "n1-x-dismiss: unsafe request_id: $REQ" >&2; exit 2 ;;
esac

command -v jq >/dev/null 2>&1 || { echo "n1-x-dismiss: jq not found" >&2; exit 1; }
...
PAYLOAD=$(jq -cn --arg rid "$REQ" '{request_id:$rid}') || {
  echo "n1-x-dismiss: failed to build request payload" >&2; exit 1; }
...
if [ -n "$FMX_DRY" ]; then
  ...
  mkdir -p "$outbox_dir" 2>/dev/null || {
    echo "n1-x-dismiss: cannot create dry-run outbox: $outbox_dir" >&2
    exit 1
  }
  ...
  OUTREC=$(printf '%s' "$PAYLOAD" | jq -c '. + {endpoint:"dismiss"}') || {
    echo "n1-x-dismiss: failed to build dry-run outbox record" >&2; exit 1; }
  printf '%s\n' "$OUTREC" > "$outbox_file" 2>/dev/null || {
    echo "n1-x-dismiss: cannot write dry-run outbox: $outbox_file" >&2
    exit 1
  }
  printf 'n1-x-dismiss: DRY RUN - would POST to %s/connector/dismiss (recorded: state/x-outbox/%s.json)\n' \
    "$FMX_RELAY" "$REQ" >&2
  printf '%s\n' "$REQ"
  exit 0
fi
...
if [ -z "$FMX_TOKEN" ]; then
  echo "n1-x-dismiss: X mode not configured (no FMX_PAIRING_TOKEN)" >&2
  exit 1
fi
command -v curl >/dev/null 2>&1 || { echo "n1-x-dismiss: curl not found" >&2; exit 1; }
AUTH_HEADER_FILE=$(fmx_auth_header_file) || {
  echo "n1-x-dismiss: invalid FMX_PAIRING_TOKEN" >&2
  exit 1
}
...
code=$(curl -m 10 -s -o /dev/null -w '%{http_code}' \
  ...
  echo "n1-x-dismiss: request to relay failed" >&2
  exit 1
}

case "$code" in
  2[0-9][0-9]) printf '%s\n' "$REQ" ;;
  *) echo "n1-x-dismiss: relay returned HTTP $code" >&2; exit 1 ;;
esac
