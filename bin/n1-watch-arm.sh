#!/usr/bin/env bash
# Safe, home-scoped (re-)arm of the numberone watcher, with honest verification.
#
# The watcher (bin/n1-watch.sh) blocks until it has an actionable wake to
...
# call and NOTIFIES on exit, so numberone must run this script as the harness's
...
# liveness beacon (state/.last-watcher-beat) is fresh within N1_GUARD_GRACE (the
# single source of truth, shared with n1-watch.sh and n1-guard.sh), and prints
...
#
# --restart: stop ONLY this N1_HOME's watcher (the pid recorded in THIS home's
...
# bin/n1-watch.sh`: that pattern matches every numberone home's watcher
# (lieutenant homes run the same script) and would kill siblings.
set -u
...
# wake exit propagates out so the harness re-notifies numberone.
child=
child_out=
cleanup_child() {
