#!/usr/bin/env bash
# n1-crew-state.sh - deterministic read of a crew's CURRENT state.
#
# Why this exists: state/<id>.status is an append-only, best-effort EVENT LOG.
# Crews append only wake-worthy transitions (done/needs-decision/blocked/failed)
# and nothing when they silently resume, so `tail -1` of that log reports the
# last EVENT, not the current STATE. After numberone resolves a needs-decision
...
# token-tight line numberone can read every heartbeat:
#
...
#   4. No run for this crew (pre-validation, or kind=survey): fall back to the
#      pane busy-signature (n1-tmux-lib.sh) + the status log's last line.
...
ID=${1:-}
[ -n "$ID" ] || { echo "usage: n1-crew-state.sh <id>" >&2; exit 2; }

META="$STATE/$ID.meta"
LOG="$STATE/$ID.status"
NM_TIMEOUT=${N1_CREW_STATE_NM_TIMEOUT:-10}
...
# CREW_BRANCH is empty at detached HEAD (a just-spawned crew, or a survey's
# scratch worktree); with no branch there is no run to attribute to this crew.
CREW_BRANCH=$(git -C "$WT" symbolic-ref --quiet --short HEAD 2>/dev/null || true)

HAVE_RUN=0
# Surveys and lieutenants never drive a no-mistakes validation of their own
# worktree, so skip the lookup for them and read state from pane/log directly.
if [ "$KIND" = mission ] && [ -n "$CREW_BRANCH" ] && command -v no-mistakes >/dev/null 2>&1; then
...
# Lieutenants idle on their own watcher (idle pane = healthy), so the busy
# signature is not meaningful for them; read their state from the status log only.
if [ "$KIND" != lieutenant ] && fm_pane_is_busy "$WIN"; then
