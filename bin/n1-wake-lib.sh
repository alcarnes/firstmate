...
N1_WAKE_LIB_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
N1_WAKE_DEFAULT_ROOT="$(cd "$N1_WAKE_LIB_DIR/.." && pwd)"
N1_ROOT="${N1_ROOT_OVERRIDE:-${N1_ROOT:-$N1_WAKE_DEFAULT_ROOT}}"
N1_HOME="${N1_HOME:-${N1_ROOT_OVERRIDE:-$N1_ROOT}}"
STATE="${N1_STATE_OVERRIDE:-${STATE:-$N1_HOME/state}}"
N1_WAKE_QUEUE="${N1_WAKE_QUEUE:-$STATE/.wake-queue}"
N1_WAKE_QUEUE_LOCK="${N1_WAKE_QUEUE_LOCK:-$STATE/.wake-queue.lock}"
N1_LOCK_STALE_AFTER="${N1_LOCK_STALE_AFTER:-2}"
mkdir -p "$STATE"
...
fm_lock_mid_acquire_is_fresh() {
  ...
  case "$pid" in
    ''|*[!0-9]*)
      mid_acquire_stale=$N1_LOCK_STALE_AFTER
...
fm_lock_try_acquire() {
  ...
  if ! fm_lock_try_acquire "$steal"; then
    N1_LOCK_HELD_PID=$(cat "$lockdir/pid" 2>/dev/null || true)
    N1_LOCK_OWNER_DIR=
    return 1
  fi
...
fm_wake_append() {
  ...
  printf '%s\n' "$seq" > "$seq_file" || status=$?
  if [ "$status" -eq 0 ]; then
    printf '%s\t%s\t%s\t%s\t%s\n' "$epoch" "$seq" "$kind" "$clean_key" "$clean_payload" >> "$N1_WAKE_QUEUE" || status=$?
  fi
...
fm_wake_restore_queue() {
  ...
  if [ -e "$N1_WAKE_QUEUE" ]; then
    cat "$drained" "$N1_WAKE_QUEUE" > "$restore" && mv "$restore" "$N1_WAKE_QUEUE"
  else
    mv "$drained" "$N1_WAKE_QUEUE"
  fi
}
