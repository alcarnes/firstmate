...
# Shared worktree-tangle guard for the numberone-on-itself case.
# Usage: . bin/n1-tangle-lib.sh
#
# Number One is a treehouse-pooled git repo of itself: ensign worktrees and
# lieutenant homes are all linked `git worktree`s of the same repo, while the
# PRIMARY checkout (the repo root numberone operates from) is a normal checkout
# on a real branch - normally the default branch, main. The "worktree tangle"
# failure mode is a ensign spawned to work on numberone ITSELF branching and
...
# which is how every linked worktree and lieutenant home legitimately sits on the
...
# echo nothing and return 1. Detached HEAD is how linked worktrees and lieutenant
