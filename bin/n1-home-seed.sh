#!/usr/bin/env bash
# Provision and route persistent lieutenant homes.
#
# Usage:
#   n1-home-seed.sh <id> <home|-> <project>...
#       Provision <home> as an isolated numberone home. If <home> is "-", acquire
#       a fresh numberone worktree via "treehouse get --lease", which durably
#       leases the worktree under the lieutenant <id> so the home survives with
#       no live process and is never recycled until the lease is released with
#       "treehouse return". Projects are cloned
#       from the active home into the lieutenant home's projects/ directory.
...
#       initialized, a .n1-lieutenant-home marker is written, and
#       data/lieutenants.md is updated.
...
#       Set N1_LIEUTENANT_CHARTER='<charter>' to seed from inline charter text
#       when no filled charter brief exists. Set N1_LIEUTENANT_SCOPE='<scope>'
#       to override the registry routing scope. Otherwise the registry summary
#       and scope are derived from the filled charter brief.
#   n1-home-seed.sh validate
...
usage() {
  echo "usage: n1-home-seed.sh <id> <home|-> <project>..." >&2
  echo "       n1-home-seed.sh validate" >&2
}
...
registry_summary_for_brief() {
  local brief=$1
  if [ -n "${N1_LIEUTENANT_CHARTER:-}" ]; then
    printf '%s\n' "$N1_LIEUTENANT_CHARTER" | normalize_registry_text
  else
...
registry_scope_for_brief() {
  local brief=$1
  if [ -n "${N1_LIEUTENANT_SCOPE:-}" ]; then
    printf '%s\n' "$N1_LIEUTENANT_SCOPE" | normalize_registry_text
  else
...
validate_registry_home_text() {
  ...
  case "$home" in
    *';'*|*')'*|*$'\n'*)
      echo "error: lieutenant home path contains registry delimiters: $home" >&2
      return 1
      ;;
  esac
}
...
validate_registry() {
  ...
  tmp=$(mktemp "${TMPDIR:-/tmp}/n1-numberones.XXXXXX")
  if [ -f "$REG" ]; then
    while IFS= read -r line; do
      case "$line" in
        "- "*)
  ...
  ' "$tmp" 2>/dev/null) || {
    rm -f "$tmp"
    printf 'error: duplicate lieutenant home assignment:\n%s\n' "$duplicate_homes" >&2
    return 1
  }
  duplicate_ids=$(awk -F '\t' '
    {
  ...
  ' "$tmp" 2>/dev/null) || {
    rm -f "$tmp"
    printf 'error: duplicate lieutenant id assignment:\n%s\n' "$duplicate_ids" >&2
    return 1
  }
  overlaps=$(awk -F '\t' '
  ...
  ' "$tmp" 2>/dev/null) || {
    rm -f "$tmp"
    printf 'error: overlapping lieutenant home assignment:\n%s\n' "$overlaps" >&2
    return 1
  }
  rm -f "$tmp"
  return 0
...
refuse_active_home_path() {
  ...
  abs_active_home=$(resolved_path "$N1_HOME")
  abs_root=$(resolved_path "$N1_ROOT")
  if [ "$abs_home" = "/" ]; then
...
validate_seed_leaf_files() {
  ...
  for label in "data/projects.md" "data/charter.md" "$SUB_HOME_MARKER"; do
    path="$home/$label"
    if [ -L "$path" ]; then
      echo "error: lieutenant leaf file must not be a symlink: $path" >&2
      return 1
    fi
    [ -e "$path" ] || continue
    ...
    case "$abs_path" in
      "$abs_home"/*) ;;
      *)
        echo "error: lieutenant leaf file must resolve inside the lieutenant home: $path" >&2
        return 1
        ;;
    esac
  done
...
validate_project_destination() {
  ...
  if ! path_is_ancestor_of "$abs_home" "$abs_projects"; then
    echo "error: lieutenant projects directory must resolve inside the lieutenant home: $projects_dir" >&2
    return 1
  fi
  if ! path_is_ancestor_of "$abs_projects" "$abs_dst"; then
    echo "error: seeded project $project destination must resolve inside the lieutenant projects directory: $dst" >&2
    return 1
  fi
  if [ "$abs_dst" = "$abs_active_home" ] || path_is_ancestor_of "$abs_active_home" "$abs_dst"; then
    echo "error: seeded project $project destination cannot be inside the active numberone home: $dst" >&2
    return 1
  fi
  if [ "$abs_dst" = "$abs_root" ] || path_is_ancestor_of "$abs_root" "$abs_dst"; then
    echo "error: seeded project $project destination cannot be inside the numberone repo: $dst" >&2
    return 1
  fi
  printf '%s\n' "$abs_dst"
}
...
acquire_treehouse_home() {
  local id=$1 home
  # Durably lease a numberone worktree from the pool. The lease persists with no
  ...
  home=$(cd "$N1_ROOT" && treehouse get --lease --lease-holder "$id") || {
    echo "error: treehouse get --lease failed to lease a numberone home" >&2
    return 1
  }
  [ -n "$home" ] || { echo "error: treehouse get --lease did not report a numberone home" >&2; return 1; }
  printf '%s\n' "$home"
}
...
ensure_home() {
  ...
  if [ -e "$home" ]; then
    ...
    git clone --quiet "$N1_ROOT" "$home"
  fi
  verify_numberone_home "$home"
}

verify_numberone_home() {
  local home=$1
  refuse_active_home_path "$home" || return 1
  [ -f "$home/AGENTS.md" ] || { echo "error: $home is not a numberone home (missing AGENTS.md)" >&2; return 1; }
  [ -d "$home/bin" ] || { echo "error: $home is not a numberone home (missing bin/)" >&2; return 1; }
...
validate_home_assignment() {
  ...
  if [ -f "$home/$SUB_HOME_MARKER" ]; then
    ...
    if [ "$marker_id" != "$id" ]; then
      echo "error: lieutenant home $home is already marked for ${marker_id:-unknown}" >&2
      return 1
    fi
  fi
  id_conflict=$(registry_id_conflict_for_assignment "$id" "$home" || true)
  if [ -n "$id_conflict" ]; then
    echo "error: lieutenant id $id is already registered to home $id_conflict; retire it before assigning $home" >&2
    return 1
  fi
...
EOF
  if [ "$conflict_type" = exact ]; then
    echo "error: lieutenant home $home is already registered to $owner" >&2
    return 1
  fi
  echo "error: lieutenant home $home overlaps registered lieutenant home $registered_home for $owner" >&2
  return 1
}
...
clone_project() {
  ...
  read -r mode _ <<EOF
$(N1_HOME="$N1_HOME" N1_DATA_OVERRIDE="$DATA" "$N1_ROOT/bin/n1-project-mode.sh" "$project")
EOF
  if [ "$mode" = local-only ]; then
    echo "error: project $project is local-only; lieutenant routes support only no-mistakes and direct-PR projects" >&2
    return 1
  fi
  if [ -e "$dst" ]; then
...
seed_rollback_target() {
  ...
  abs_home=$(resolved_path "$N1_HOME")
  abs_root=$(resolved_path "$N1_ROOT")
  if [ "$abs_target" = "$abs_home" ]; then
    echo "REFUSED: unsafe $label rollback target $target is the active numberone home" >&2
    return 1
  fi
  if [ "$abs_target" = "$abs_root" ]; then
    echo "REFUSED: unsafe $label rollback target $target is the numberone repo" >&2
    return 1
  fi
  if path_is_ancestor_of "$abs_target" "$abs_home"; then
    echo "REFUSED: unsafe $label rollback target $target is an ancestor of the active numberone home" >&2
    return 1
  fi
  if path_is_ancestor_of "$abs_target" "$abs_root"; then
    echo "REFUSED: unsafe $label rollback target $target is an ancestor of the numberone repo" >&2
    return 1
  fi
  if path_is_ancestor_of "$abs_home" "$abs_target"; then
    echo "REFUSED: unsafe $label rollback target $target is inside the active numberone home" >&2
    return 1
  fi
  if path_is_ancestor_of "$abs_root" "$abs_target"; then
    echo "REFUSED: unsafe $label rollback target $target is inside the numberone repo" >&2
    return 1
  fi
  printf '%s\n' "$abs_target"
}
...
seed_return_treehouse_home() {
  ...
  ( cd "$N1_ROOT" && treehouse return --force "$abs_home" >/dev/null ) || {
...
seed_project_rollback_target() {
  ...
  if ! path_is_ancestor_of "$abs_home" "$abs_projects"; then
    echo "REFUSED: unsafe created project rollback target $target has projects directory outside the lieutenant home" >&2
    return 1
  fi
  if ! path_is_ancestor_of "$abs_projects" "$abs_target"; then
    echo "REFUSED: unsafe created project rollback target $target is outside the lieutenant projects directory" >&2
    return 1
  fi
  printf '%s\n' "$abs_target"
}
...
seed_rollback() {
  ...
  if [ -n "${SEED_BACKUP_DIR:-}" ]; then
    restore_seed_file "$SEED_PARENT_REG_EXISTED" "$SEED_BACKUP_DIR/parent-lieutenants.md" "$REG"
...
project_mode_in_home() {
  ...
  read -r mode _ <<EOF
$(N1_ROOT_OVERRIDE='' N1_STATE_OVERRIDE='' N1_DATA_OVERRIDE='' N1_PROJECTS_OVERRIDE='' N1_CONFIG_OVERRIDE='' N1_HOME="$home" "$N1_ROOT/bin/n1-project-mode.sh" "$project")
EOF
  printf '%s\n' "$mode"
}
...
seed_home() {
  ...
  shift 2
  [ $# -gt 0 ] || { echo "error: lieutenant needs at least one project" >&2; return 1; }
  ...
  SEED_HOME_ACQUIRED=0
  SEED_HOME_BACKED_UP=0
  SEED_BACKUP_DIR=$(mktemp -d "${TMPDIR:-/tmp}/n1-home-seed.XXXXXX")
  ...
  if [ -f "$REG" ]; then
    ...
    cp "$REG" "$SEED_BACKUP_DIR/parent-lieutenants.md"
  fi
  ...
  if [ "$requested_home" = "-" ]; then
    ...
    home=$(acquire_treehouse_home "$id")
    SEED_HOME="$home"
    home=$(verify_numberone_home "$home")
  else
  ...
  if [ ! -f "$SEED_PARENT_BRIEF" ]; then
    [ -n "${N1_LIEUTENANT_CHARTER:-}" ] || {
      echo "error: no filled lieutenant charter brief at $SEED_PARENT_BRIEF; set N1_LIEUTENANT_CHARTER or scaffold one and replace {TASK}" >&2
      return 1
    }
    [ -d "$DATA/$id" ] || SEED_PARENT_BRIEF_DIR_CREATED=1
    "$N1_ROOT/bin/n1-brief.sh" "$id" --lieutenant "$@"
    SEED_PARENT_BRIEF_CREATED=1
  fi
  if grep -F '{TASK}' "$SEED_PARENT_BRIEF" >/dev/null 2>&1; then
    echo "error: lieutenant charter brief at $SEED_PARENT_BRIEF still contains {TASK}; fill it before seeding" >&2
    return 1
  fi
  charter_summary=$(registry_summary_for_brief "$SEED_PARENT_BRIEF")
  [ -n "$charter_summary" ] || {
    echo "error: lieutenant charter brief at $SEED_PARENT_BRIEF has an empty Charter section; fill it before seeding" >&2
    return 1
  }
  charter_scope=$(registry_scope_for_brief "$SEED_PARENT_BRIEF")
  [ -n "$charter_scope" ] || {
    echo "error: lieutenant charter brief at $SEED_PARENT_BRIEF has an empty Routing scope section; fill it before seeding" >&2
    return 1
  }
