...
# lieutenant home, kill the tmux window, clear volatile state, refresh/prune
# the project's clone for PR-based mission tasks, then print a backlog-refresh
# reminder.
...
# normal mission task whose commits are not so reachable - when its PR is merged and
...
# branch (numberone performs that merge on the captain's approval) as a fallback
# for the common case where there is no remote at all.
# Survey tasks (kind=survey in meta) carve out of that check: their worktree is
# declared scratch and the report at data/<task-id>/report.md is the work
# product - teardown proceeds once the report exists, and refuses without it.
# Lieutenants (kind=lieutenant in meta) are retired explicitly. Normal
# teardown refuses while their home has in-flight ensign meta files; --force
...
# Usage: n1-teardown.sh <task-id> [--force]
#   --force skips ordinary-task dirty and landed-work checks, skips survey report
#   checks, and discards lieutenant child work for kind=lieutenant. Only use it
...
LIEUTENANT_REG="$DATA/lieutenants.md"
SUB_HOME_MARKER=".n1-lieutenant-home"
# shellcheck source=bin/n1-tasks-axi-lib.sh
. "$SCRIPT_DIR/n1-tasks-axi-lib.sh"
...
numberone_home_has_treehouse_slot() {
  local home=$1
  worktree_registered_for_project "$N1_ROOT" "$home"
}
...
validate_removal_target() {
  ...
  [ -e "$target" ] || return 0
  abs_target=$(removal_target_abs_path "$target")
  if abs_home=$(cd "$N1_HOME" 2>/dev/null && pwd -P); then
    :
  else
    abs_home=
  fi
  abs_root=$(cd "$N1_ROOT" && pwd -P)
  case "$abs_target" in
    ''|/) echo "REFUSED: unsafe $label removal target $target" >&2; return 1 ;;
  esac
  if [ -n "$abs_home" ] && [ "$abs_target" = "$abs_home" ]; then
    echo "REFUSED: unsafe $label removal target $target is the active numberone home" >&2
    return 1
  fi
  if [ "$abs_target" = "$abs_root" ]; then
    echo "REFUSED: unsafe $label removal target $target is the numberone repo" >&2
    return 1
  fi
  if [ -n "$abs_home" ] && path_is_ancestor_of "$abs_target" "$abs_home"; then
    echo "REFUSED: unsafe $label removal target $target is an ancestor of the active numberone home" >&2
    return 1
  fi
  if path_is_ancestor_of "$abs_target" "$abs_root"; then
    echo "REFUSED: unsafe $label removal target $target is an ancestor of the numberone repo" >&2
    return 1
  fi
  if [ -n "$abs_home" ] && path_is_ancestor_of "$abs_home" "$abs_target"; then
    echo "REFUSED: unsafe $label removal target $target is inside the active numberone home" >&2
    return 1
  fi
  if path_is_ancestor_of "$abs_root" "$abs_target"; then
    echo "REFUSED: unsafe $label removal target $target is inside the numberone repo" >&2
    return 1
  fi
  printf '%s\n' "$abs_target"
}
...
validate_numberone_operational_dirs_for_removal() {
  ...
  for name in data state config projects; do
    ...
    if [ -L "$dir" ] && [ ! -e "$dir" ]; then
      echo "REFUSED: unsafe $label $name directory $dir resolves outside the lieutenant home" >&2
      return 1
    fi
    if [ -d "$dir" ]; then
...
validate_child_worktree_for_removal() {
  ...
  if abs_home=$(cd "$N1_HOME" 2>/dev/null && pwd -P); then
    if path_is_ancestor_of "$abs_home" "$abs_target"; then
      echo "REFUSED: unsafe child worktree removal target $target is inside the active numberone home" >&2
      return 1
    fi
  fi
  abs_root=$(cd "$N1_ROOT" && pwd -P)
  if path_is_ancestor_of "$abs_root" "$abs_target"; then
    echo "REFUSED: unsafe child worktree removal target $target is inside the numberone repo" >&2
    return 1
  fi
...
validate_numberone_home_for_removal() {
  ...
  if [ ! -f "$abs_home_path/$SUB_HOME_MARKER" ]; then
    echo "REFUSED: unsafe $label removal target $home is not a seeded lieutenant home" >&2
    return 1
  fi
  if [ -n "$expected_id" ]; then
    marker_id=$(cat "$abs_home_path/$SUB_HOME_MARKER" 2>/dev/null || true)
    if [ "$marker_id" != "$expected_id" ]; then
      echo "REFUSED: unsafe $label removal target $home is marked for lieutenant ${marker_id:-unknown}, expected $expected_id" >&2
      return 1
    fi
  fi
  validate_numberone_operational_dirs_for_removal "$abs_home_path" "$label" || return 1
  conflict=$(registered_descendant_home_for_removal "$LIEUTENANT_REG" "$abs_home_path" || true)
  if [ -z "$conflict" ]; then
    conflict=$(registered_descendant_home_for_removal "$abs_home_path/data/lieutenants.md" "$abs_home_path" || true)
  fi
  if [ -n "$conflict" ]; then
    IFS=$'\t' read -r child_id child_home <<EOF
$conflict
EOF
    echo "REFUSED: unsafe $label removal target $home contains registered lieutenant home $child_home for $child_id" >&2
    return 1
  fi
  printf '%s\n' "$abs_home_path"
}

remove_numberone_home() {
  local home=$1 label=$2 expected_id=${3:-} abs_home_path
  [ -n "$home" ] || return 0
  [ -e "$home" ] || return 0
  abs_home_path=$(validate_numberone_home_for_removal "$home" "$label" "$expected_id") || return 1
  [ -n "$abs_home_path" ] || return 0
  if numberone_home_has_treehouse_slot "$abs_home_path"; then
    command -v treehouse >/dev/null 2>&1 || {
      echo "error: treehouse command not found; cannot return $label $abs_home_path" >&2
      return 1
    }
    ( cd "$N1_ROOT" && treehouse return --force "$abs_home_path" ) || {
...
validate_numberone_home_children_removal() {
  ...
  for child_meta in "$sub_state"/*.meta; do
    ...
    [ -n "$child_kind" ] || child_kind=mission
    if [ "$child_kind" = lieutenant ]; then
      child_home=$(meta_value "$child_meta" home)
      [ -n "$child_home" ] || child_home=$child_wt
      validate_numberone_home_for_removal "$child_home" "child numberone home" "$child_id" >/dev/null || return 1
      validate_numberone_home_children_removal "$child_home" || return 1
...
cleanup_numberone_home_children() {
  ...
  for child_meta in "$sub_state"/*.meta; do
    ...
    if [ "$child_kind" = lieutenant ]; then
      ...
      if [ -n "$child_home" ] && [ -d "$child_home" ]; then
        cleanup_numberone_home_children "$child_home"
        remove_numberone_home "$child_home" "child numberone home" "$child_id"
      fi
    elif [ -n "$child_wt" ] && [ -d "$child_wt" ]; then
      validate_child_worktree_for_removal "$child_wt" "$child_proj" >/dev/null || return 1
      rm -f "$child_wt/.claude/settings.local.json" "$child_wt/.opencode/plugins/n1-turn-end.js"
...
remove_lieutenant_registry_entry() {
  local id=$1 tmp
  [ -f "$LIEUTENANT_REG" ] || return 0
  tmp="$LIEUTENANT_REG.tmp.$$"
  grep -vE "^- $id( |$)" "$LIEUTENANT_REG" > "$tmp" || true
  mv "$tmp" "$LIEUTENANT_REG"
}

if [ "$KIND" = lieutenant ]; then
  [ -n "$HOME_PATH" ] || HOME_PATH=$WT
  validate_numberone_home_for_removal "$HOME_PATH" "lieutenant home" "$ID" >/dev/null || exit 1
  if [ "$FORCE" = "--force" ]; then
    validate_numberone_home_children_removal "$HOME_PATH" || exit 1
  fi
fi

if [ "$KIND" = lieutenant ] && [ "$FORCE" != "--force" ]; then
  SUB_STATE="$HOME_PATH/state"
  if [ -d "$SUB_STATE" ]; then
    for child_meta in "$SUB_STATE"/*.meta; do
      [ -e "$child_meta" ] || continue
      echo "REFUSED: lieutenant $ID still has in-flight work in $SUB_STATE." >&2
...
if [ "$KIND" = lieutenant ] && [ "$FORCE" = "--force" ]; then
  cleanup_numberone_home_children "$HOME_PATH"
fi
...
if [ -d "$WT" ] && [ "$FORCE" != "--force" ]; then
  if [ "$KIND" = lieutenant ]; then
    ...
    # Survey worktrees are scratch by contract, but only once the deliverable exists.
    REPORT="$DATA/$ID/report.md"
    if [ ! -f "$REPORT" ]; then
      echo "REFUSED: survey task $ID has no report at $REPORT." >&2
      echo "The report is the work product. Have the ensign write it (or get the captain's explicit OK to discard, then --force)." >&2
      exit 1
    fi
  else
    # The n1-spawn hook file is ours, never work product; ignore it in the dirty check.
    ...
    if [ -n "$unpushed" ] && [ "$MODE" = local-only ]; then
      # local-only delivers have no remote in the common case, so the "on a remote"
      # test above is expected to be non-empty. The work is safe once it is merged
      # into the local default branch (numberone does that merge on the captain's
      ...
      if [ -n "$dirty" ] || [ -n "$unmerged" ]; then
        ...
        echo "Merge the branch into local $DEFAULT first (bin/n1-merge-local.sh after the captain approves), or push to a fork/remote, or get the captain's explicit OK to discard, then --force." >&2
        exit 1
      fi
    elif [ -n "$dirty" ]; then
...
if [ -d "$WT" ] && [ "$KIND" != lieutenant ]; then
  ...
  rm -f "$WT/.claude/settings.local.json" "$WT/.opencode/plugins/n1-turn-end.js"
...
if [ "$KIND" = lieutenant ]; then
  [ -n "$HOME_PATH" ] || HOME_PATH=$WT
  remove_numberone_home "$HOME_PATH" "lieutenant home" "$ID"
  remove_lieutenant_registry_entry "$ID"
fi
rm -f "$STATE/$ID.status" "$STATE/$ID.turn-ended" "$STATE/$ID.check.sh" "$STATE/$ID.meta" "$STATE/$ID.pi-ext.ts"
if [ "$KIND" != survey ] && [ "$KIND" != lieutenant ] && [ "$MODE" != local-only ]; then
  "$N1_ROOT/bin/n1-fleet-sync.sh" "$PROJ" || true
fi
