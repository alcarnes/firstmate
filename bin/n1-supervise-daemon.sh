#!/usr/bin/env bash
# n1-supervise-daemon.sh — presence-gated sub-supervisor (closes #27's P2).
#
# Wraps bin/n1-watch.sh: runs it as a child, classifies each wake reason, and
# either SELF-HANDLES the routine majority in bash (no numberone turn) or
...
# numberone context; only done/needs-decision/blocked/failed/persistent-wedge/
...
# user message clears it and numberone resumes full responsiveness.
# When afk is off, normal n1-watch.sh always-on triage is the active mechanism.
...
# N1_INJECT_MARK (ASCII unit separator, 0x1f) — a byte a human would never type
# at the start of a message. Number One's contract: a message that starts with
...
#     crash/restart/missed injection is recovered on the next n1-wake-drain.sh.
...
#     ensign is therefore detected within STALE_ESCALATE_SECS + a tick, never
#     lost. Ensigns are autonomous, so a delayed stale response does not stall
#     a healthy ensign's own progress.
#     Buffered escalation delivery also has a max-defer alarm: if a digest stays
#     undelivered past N1_MAX_DEFER_SECS, the daemon retries a normal flush and
...
#
# Usage: n1-supervise-daemon.sh
#          Long-lived background loop. Normally started by the /afk skill, which
#          sets state/.afk first. Env knobs:
#          N1_SUPERVISOR_TARGET     supervisor tmux target (override; otherwise
#                                   auto-discovered from TMUX_PANE, then
#                                   numberone:0 fallback)
#          N1_INJECT_SKIP           |-prefixes force-self-handle bypassing
#                                   classification (default "heartbeat"); empty
#                                   disables. Use sparingly: it overrides the
#                                   captain-relevant escalation for matching
#                                   kinds.
#          N1_STALE_ESCALATE_SECS   idle seconds before a stale pane escalates
#                                   as a possible wedge (default 240)
#          N1_ESCALATE_BATCH_SECS   buffer window for batched escalation
#                                   digests; 0 = flush immediately (default 90)
#          N1_HEARTBEAT_SCAN_SECS   cadence for the catch-all status scan
#                                   (default 300)
#          N1_HOUSEKEEPING_TICK     seconds between housekeeping passes while
#                                   the watcher is mid-cycle (default 15)
#          N1_BUSY_REGEX            OR-ed busy signatures (mirrors n1-watch.sh)
#          N1_COMPOSER_IDLE_RE      empty-composer regex applied after dim-ghost
#                                   and structural border stripping (default:
#                                   bare prompt glyphs plus busy footers)
#          N1_MAX_DEFER_SECS        max seconds a buffered escalation may sit
#                                   undelivered before one normal flush attempt;
#                                   if that cannot confirm a submit, a wedge
#                                   alarm fires (default 300; 0 disables)
#          N1_INJECT_CONFIRM_RETRIES Enter-retry attempts on a swallowed Enter
#                                   (default 3); the digest is typed once, only
#                                   Enter is retried. Composer-empty detection is
#                                   structural and style-aware (bin/n1-tmux-lib.sh):
...
#          N1_INJECT_CONFIRM_SLEEP  seconds between daemon submit checks
#                                   (default 0.5)
#          N1_LOG_MAX_BYTES / N1_LOG_KEEP_LINES / N1_CRASH_*  log + crash guards
#          N1_STATE_OVERRIDE        alternate state dir (testing)
...
#          lock. A crashing n1-watch.sh is logged and restarted, never killing
#          the daemon; a tight crash-restart spin is detected and backed off.
set -u

N1_DAEMON_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
N1_ROOT="${N1_ROOT_OVERRIDE:-$(cd "$N1_DAEMON_DIR/.." && pwd)}"
...
# shellcheck source=bin/n1-classify-lib.sh
. "$N1_DAEMON_DIR/n1-classify-lib.sh"

# --- tunables ---------------------------------------------------------------
N1_SUPERVISOR_TARGET_DEFAULT="numberone:0"
...
# live in bin/n1-classify-lib.sh, shared with the always-on watcher.
# Busy footers + composer-empty detection now live in bin/n1-tmux-lib.sh
# (N1_TMUX_BUSY_REGEX_DEFAULT / fm_tmux_composer_state); N1_BUSY_REGEX still
...
# injection is prefixed with this byte; numberone treats a leading marker as an
...
# Resolve the effective state dir. N1_STATE_OVERRIDE wins (testing); otherwise
# $N1_HOME/state. Kept as a function so the pure
# classifiers can take an explicit state arg without depending on globals.
_state_root() { printf '%s' "${N1_STATE_OVERRIDE:-$N1_HOME/state}"; }

# --- portable stat (same trap as n1-watch.sh: no `stat -f || stat -c`) -------
...
# skill (enter) and by numberone on user return (exit). Durable: a plain file,
...
# should_exit_afk: encodes numberone's afk-exit contract as a testable function.
...
# marker (a daemon escalation), 1 otherwise (a real user message). Number One's
...
# distilled content numberone relays to the captain or feeds back to classifiers.
strip_injection_marker() {  # <message-text>
  local msg=$1
  printf '%s' "${msg#"$N1_INJECT_MARK"}"
}
...
#   1. N1_SUPERVISOR_TARGET env (explicit override) — caller passes it in.
#   2. $TMUX_PANE — tmux sets this in every pane's environment; inherited by
#      the daemon when the /afk skill launches it from numberone's own pane.
#   3. numberone:0 — legacy fallback (may not resolve if the session is named
...
discover_supervisor_target() {
  if [ -n "${N1_SUPERVISOR_TARGET:-}" ]; then
    printf '%s' "$N1_SUPERVISOR_TARGET"
    return 0
  fi
  if [ -n "${TMUX_PANE:-}" ]; then
    printf '%s' "$TMUX_PANE"
  ...
  printf '%s' "$N1_SUPERVISOR_TARGET_DEFAULT"
  return 1
}

# --- classification helpers (PURE: no side effects, testable) ---------------
# last_status_line, status_is_captain_relevant, window_to_task, and
# scan_captain_relevant_statuses come from bin/n1-classify-lib.sh (sourced above),
# the single classifier shared with bin/n1-watch.sh. The decision-string wrappers
...
# summary numberone would otherwise have to re-read.
...
classify_check() {  # <full reason>  — check scripts print only when numberone should wake
  printf 'escalate|%s' "$1"
}
...
# Busy + composer-empty detection are the shared primitives in n1-tmux-lib.sh
# (one source of truth with n1-send.sh). These thin wrappers keep the daemon's
...
# an ERROR, drops a durable marker numberone/recovery can surface, and flashes
...
inject_wedge_alarm() {  # <state> <age-seconds>
  ...
  if [ "$(_file_age "$marker")" -lt "${N1_MAX_DEFER_SECS:-$MAX_DEFER_SECS_DEFAULT}" ]; then
    return 0
  fi
  ...
  target="${N1_SUPERVISOR_TARGET:-$N1_SUPERVISOR_TARGET_DEFAULT}"
...
housekeeping() {  # <state>
  ...
  # (1) batch flush
  if [ "${N1_ESCALATE_BATCH_SECS:-$ESCALATE_BATCH_SECS_DEFAULT}" -le 0 ]; then
    escalate_flush "$state" || true
  else
    due=$(_oldest_line_age "$state/.subsuper-escalations")
    if [ "$due" -ge "${N1_ESCALATE_BATCH_SECS:-$ESCALATE_BATCH_SECS_DEFAULT}" ]; then
      escalate_flush "$state" || true
    fi
  fi
  ...
  max_defer=${N1_MAX_DEFER_SECS:-$MAX_DEFER_SECS_DEFAULT}
  ...
  for marker in "$state"/.subsuper-stale-*; do
    ...
    [ "$age" -ge "${N1_STALE_ESCALATE_SECS:-$STALE_ESCALATE_SECS_DEFAULT}" ] || continue
    # Reconstruct the window name from the key (best-effort: session is unknown,
    # so probe the live n1-* windows for one whose task matches).
    ...
    if pane_is_busy "$win"; then
      rm -f "$marker"   # ensign resumed: benign
    else
  ...
  if [ "$(_file_age "$state/.subsuper-last-scan")" -ge "${N1_HEARTBEAT_SCAN_SECS:-$HEARTBEAT_SCAN_SECS_DEFAULT}" ]; then
...
# Find a live n1-* window whose task id matches the given marker key.
window_for_task() {  # <task-key>
  local key=$1 w t
  for w in $(tmux list-windows -a -F '#{session_name}:#{window_name}' 2>/dev/null | grep ':n1-' || true); do
    t=$(window_to_task "$w")
...
inject_msg() {  # <message> [state]
  ...
  # daemon self-handles and stays quiet; numberone drives the normal always-on
  ...
  # them. Then prepend the sentinel marker — numberone's afk-exit contract
  # keys off its presence at the start of the message.
  msg=$(_collapse_newlines "$msg")
  msg="${N1_INJECT_MARK}${msg}"
  target="${N1_SUPERVISOR_TARGET:-$N1_SUPERVISOR_TARGET_DEFAULT}"
  ...
  retries=${N1_INJECT_CONFIRM_RETRIES:-$INJECT_CONFIRM_RETRIES_DEFAULT}
  sleep_s=${N1_INJECT_CONFIRM_SLEEP:-$INJECT_CONFIRM_SLEEP_DEFAULT}
...
should_force_self() {  # <reason>
  local reason=$1 skip="${N1_INJECT_SKIP:-$INJECT_SKIP_DEFAULT}" prefix
  [ -n "$skip" ] || return 1
  local -a prefixes
...
handle_wake() {  # <reason> <state>
  ...
  if should_force_self "$reason"; then
    log "wake force-self (N1_INJECT_SKIP): $reason"
    return
  fi
  case "$reason" in
  ...
  if [ "$action" = "escalate" ]; then
    ...
    mark_escalated_seen "$kind" "$arg" "$state"
    [ "${N1_ESCALATE_BATCH_SECS:-$ESCALATE_BATCH_SECS_DEFAULT}" -le 0 ] && { escalate_flush "$state" || true; }
  else
...
trim_log() {
  ...
  [ "$sz" -ge "${N1_LOG_MAX_BYTES:-$LOG_MAX_BYTES_DEFAULT}" ] || return 0
  tmp=$(mktemp "${TMPDIR:-/tmp}/n1-daemon-log.XXXXXX") || return 0
  tail -n "${N1_LOG_KEEP_LINES:-$LOG_KEEP_LINES_DEFAULT}" "$LOG" >"$tmp" 2>/dev/null && mv -f "$tmp" "$LOG"
}

# ============================================================================
# Everything below runs only when the script is EXECUTED, not sourced. The pure
# classifiers above are sourceable for unit tests (tests/n1-daemon.test.sh).
# ============================================================================
...
fm_super_main() {
  ...
  # Export N1_STATE_OVERRIDE so the lib resolves the same state dir.
  # shellcheck source=bin/n1-wake-lib.sh
  N1_STATE_OVERRIDE="$STATE" . "$N1_DAEMON_DIR/n1-wake-lib.sh"

  local WATCH="$N1_DAEMON_DIR/n1-watch.sh"
  ...
  local INJECT_FAIL_SLEEP=${N1_INJECT_FAIL_SLEEP:-$INJECT_FAIL_SLEEP_DEFAULT}
  local CRASH_THRESHOLD=${N1_CRASH_THRESHOLD:-$CRASH_THRESHOLD_DEFAULT}
  local CRASH_WINDOW=${N1_CRASH_WINDOW:-$CRASH_WINDOW_DEFAULT}
  local CRASH_BACKOFF=${N1_CRASH_BACKOFF:-$CRASH_BACKOFF_DEFAULT}
  local CRASH_NORMAL_SLEEP=${N1_CRASH_NORMAL_SLEEP:-$CRASH_NORMAL_SLEEP_DEFAULT}
  ...
  if ! fm_lock_try_acquire "$LOCK"; then
    if [ -n "${N1_LOCK_HELD_PID:-}" ]; then
      echo "error: another n1-supervise-daemon is already running (pid $N1_LOCK_HELD_PID, lock $LOCK held)" >&2
    else
      echo "error: another n1-supervise-daemon is already running (lock $LOCK held)" >&2
    fi
    exit 1
  fi
  echo "$$" > "$PIDFILE"

  # --- auto-discover the supervisor target (the pane running numberone) -----
  # Priority: N1_SUPERVISOR_TARGET override > $TMUX_PANE (inherited from the
  # pane that launched the daemon, normally numberone's own) > numberone:0
  # fallback. Exporting the result into N1_SUPERVISOR_TARGET makes inject_msg
  # (which reads that env var) use the discovered pane without an extra global.
  local discovered target_source
  target_source="N1_SUPERVISOR_TARGET"
  if [ -z "${N1_SUPERVISOR_TARGET:-}" ]; then
    if [ -n "${TMUX_PANE:-}" ]; then
      target_source="TMUX_PANE"
    else
      target_source="FALLBACK(numberone:0)"
    fi
  fi
  if discovered=$(discover_supervisor_target); then
    : # resolved cleanly
  else
    echo "warn: could not auto-discover supervisor pane (no N1_SUPERVISOR_TARGET or TMUX_PANE); falling back to '$discovered' — verify this is numberone's pane" >&2
  fi
  N1_SUPERVISOR_TARGET="$discovered"
  local TARGET="$N1_SUPERVISOR_TARGET"
  ...
  if ! tmux display-message -p -t "$TARGET" '#{pane_id}' >/dev/null 2>&1; then
    echo "error: supervisor target '$TARGET' does not resolve to a tmux pane; set N1_SUPERVISOR_TARGET" >&2
  ...
  log "daemon starting (pid $$); target=$TARGET; target_source=$target_source; afk=$afk_status; inject_skip='${N1_INJECT_SKIP:-$INJECT_SKIP_DEFAULT}'; stale_escalate=${N1_STALE_ESCALATE_SECS:-$STALE_ESCALATE_SECS_DEFAULT}s; batch=${N1_ESCALATE_BATCH_SECS:-$ESCALATE_BATCH_SECS_DEFAULT}s"
  ...
  start_watcher() {
    CUR_TMP=$(mktemp "${TMPDIR:-/tmp}/n1-watch.XXXXXX") || { log "error: mktemp failed; retrying in 5s"; sleep 5; return 1; }
  ...
  while true; do
    ...
    # has nowhere to go, and numberone itself is the consumer of escalations.
    ...
    # The watcher child runs on its own N1_POLL cadence internally; we only need
    ...
    sleep 1
    if [ "$(_file_age "$STATE/.subsuper-last-housekeep")" -ge "${N1_HOUSEKEEPING_TICK:-$HOUSEKEEPING_TICK_DEFAULT}" ]; then
