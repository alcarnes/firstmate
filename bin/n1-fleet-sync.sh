...
# worktree, so it cannot discard unlanded work; set N1_FLEET_PRUNE=0 to disable it.
# Usage: n1-fleet-sync.sh [<project-dir>]
set -eu
...
usage() {
  echo "usage: n1-fleet-sync.sh [<project-dir>]" >&2
}
...
prune_gone_branches() {
  ...
  # net. Set N1_FLEET_PRUNE=0 to skip pruning entirely.
  [ "${N1_FLEET_PRUNE:-1}" != "0" ] || return 0
...
sync_project() {
  ...
  mode_line=$("$N1_ROOT/bin/n1-project-mode.sh" "$label" 2>/dev/null || echo "no-mistakes off")
  mode=${mode_line%% *}
