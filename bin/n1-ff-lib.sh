...
# Shared fast-forward machinery for numberone self-sync.
# Usage: . bin/n1-ff-lib.sh   (after N1_ROOT and N1_HOME are set)
#
# This is the one implementation of "advance a numberone checkout to a base by a
# clean fast-forward, never forcing, merging, or stashing" used by every sync
# path:
#   - /updatenumberone (bin/n1-update.sh) pulls from origin: base_mode "origin".
#   - the local-HEAD lieutenant sync (bin/n1-spawn.sh on launch, bin/n1-bootstrap.sh
...
#
# Every lieutenant home is a worktree of this same repo, so it already holds the
...
# config/, projects/, .no-mistakes/), so a lieutenant's backlog, projects, and
...
SUB_HOME_MARKER="${SUB_HOME_MARKER:-.n1-lieutenant-home}"
...
# sync target every lieutenant follows. Reads the default branch *ref* rather than
...
validate_operational_dirs() {
  ...
  for name in data state config projects; do
    dir="$abs_home/$name"
    if [ -L "$dir" ] && [ ! -e "$dir" ]; then
      VALIDATION_ERROR="lieutenant $name directory must resolve inside the lieutenant home"
      return 1
    fi
    if [ -d "$dir" ]; then
      abs_dir=$(cd "$dir" && pwd -P) || {
        VALIDATION_ERROR="lieutenant $name directory cannot be resolved"
        return 1
      }
    elif [ -e "$dir" ]; then
      VALIDATION_ERROR="lieutenant $name path is not a directory"
      return 1
    else
    ...
    if [ "$abs_dir" = "$abs_active_home" ] || path_is_ancestor_of "$abs_active_home" "$abs_dir"; then
      VALIDATION_ERROR="lieutenant $name directory cannot be inside the active numberone home"
      return 1
    fi
    if [ "$abs_dir" = "$abs_root" ] || path_is_ancestor_of "$abs_root" "$abs_dir"; then
      VALIDATION_ERROR="lieutenant $name directory cannot be inside the numberone repo"
      return 1
    fi
  done
}
...
validate_lieutenant_home() {
  ...
  abs_active_home=$(resolved_existing_dir "$N1_HOME") || {
    VALIDATION_ERROR="active numberone home is not a directory"
    return 1
  }
  abs_root=$(resolved_existing_dir "$N1_ROOT") || {
    VALIDATION_ERROR="numberone repo is not a directory"
    return 1
  }
  if [ "$abs_home" = "/" ]; then
    VALIDATION_ERROR="lieutenant home cannot be the filesystem root"
    return 1
  fi
  if [ "$abs_home" = "$abs_active_home" ]; then
    VALIDATION_ERROR="lieutenant home cannot be the active numberone home"
    return 1
  fi
  if [ "$abs_home" = "$abs_root" ]; then
    VALIDATION_ERROR="lieutenant home cannot be the numberone repo"
    return 1
  fi
  if path_is_ancestor_of "$abs_active_home" "$abs_home"; then
    VALIDATION_ERROR="lieutenant home cannot be inside the active numberone home"
    return 1
  fi
  if path_is_ancestor_of "$abs_root" "$abs_home"; then
    VALIDATION_ERROR="lieutenant home cannot be inside the numberone repo"
    return 1
  fi
  if path_is_ancestor_of "$abs_home" "$abs_active_home"; then
    VALIDATION_ERROR="lieutenant home cannot be an ancestor of the active numberone home"
    return 1
  fi
  if path_is_ancestor_of "$abs_home" "$abs_root"; then
    VALIDATION_ERROR="lieutenant home cannot be an ancestor of the numberone repo"
    return 1
  fi
  validate_operational_dirs "$abs_home" "$abs_active_home" "$abs_root" || return 1
  if [ -L "$abs_home/$SUB_HOME_MARKER" ]; then
    VALIDATION_ERROR="lieutenant marker must not be a symlink"
    return 1
  fi
  if [ ! -f "$abs_home/$SUB_HOME_MARKER" ]; then
    VALIDATION_ERROR="not a seeded lieutenant home"
    return 1
  fi
  marker_id=$(cat "$abs_home/$SUB_HOME_MARKER" 2>/dev/null || true)
  if [ "$marker_id" != "$id" ]; then
    VALIDATION_ERROR="marked for lieutenant ${marker_id:-unknown}, expected $id"
    return 1
  fi
  if [ ! -f "$abs_home/AGENTS.md" ]; then
    VALIDATION_ERROR="not a numberone home (missing AGENTS.md)"
    return 1
  fi
  if [ ! -d "$abs_home/bin" ]; then
    VALIDATION_ERROR="not a numberone home (missing bin/)"
    return 1
  fi
...
#   origin       - fetch origin and advance to origin/<default> (the /updatenumberone
...
#                  dependency (the local-HEAD lieutenant sync). The commit must
...
# Validate and fast-forward one lieutenant home, accumulating its window into
...
# numberone repo itself (N1_ROOT) is never processed as its own lieutenant, and
...
process_lieutenant() {
  ...
  [ -n "$id" ] || return 0
  [ -n "$home" ] || return 0
  fm_root_real=$(resolve_path "$N1_ROOT")
  home_real=$(resolve_path "$home")
  [ "$home_real" != "$fm_root_real" ] || return 0
  if ! validate_lieutenant_home "$id" "$home"; then
    echo "lieutenant $id: skipped: unsafe home: $VALIDATION_ERROR"
    return 0
  fi
  ...
  ff_target "$home_real" "lieutenant $id" "$base_mode" yes yes
...
# Sweep this home's LIVE lieutenant direct reports - state/<id>.meta files with
# kind=lieutenant - fast-forwarding each to base_mode. Passes base_mode and
# nudge_requires_instr through to process_lieutenant. Accumulates into
# FF_NUDGE_WINDOWS / FF_SEEN_HOMES, which the caller resets before and reads after.
sweep_live_lieutenant_metas() {
  ...
  for meta in "$state"/*.meta; do
    ...
    process_lieutenant "$id" "$home" "$window" "$base_mode" "$nudge_requires_instr"
  done
}
