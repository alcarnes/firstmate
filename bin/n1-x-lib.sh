#!/usr/bin/env bash
# Shared config resolution for the X-mode connector client (n1-x-poll.sh and
# n1-x-reply.sh). X mode is opt-in: a user drops a non-empty FMX_PAIRING_TOKEN
# into the numberone home's .env. FMX_ENV_FILE can point direct client calls at
# another .env-style file, but bootstrap activation still checks $N1_HOME/.env.
...
# Callers must have N1_HOME set before calling fmx_load_config.
...
# composes a reply but records it instead of posting (see n1-x-reply.sh).
fmx_load_config() {
  local env_file="${FMX_ENV_FILE:-$N1_HOME/.env}" dry
  ...
  # shellcheck disable=SC2034 # FMX_DRY is read by callers (n1-x-reply.sh) after sourcing.
  ...
  # shellcheck disable=SC2034 # FMX_MAX is read by callers (n1-x-reply.sh) after sourcing.
  FMX_MAX=$maxraw
  ...
  # shellcheck disable=SC2034 # FMX_THREAD_MAX is read by callers (n1-x-reply.sh) after sourcing.
...
fmx_auth_header_file() {
  ...
  file=$(umask 077; mktemp "${TMPDIR:-/tmp}/n1-x-auth.XXXXXX") || return 1
...
# On the task's terminal completion numberone posts ONE follow-up reply to that
# request (within the window) and clears the link. These helpers own the
# read/write/clear so n1-x-link.sh and n1-x-followup.sh never hand-edit meta and
