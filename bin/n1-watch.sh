...
# bin/n1-watch-arm.sh through the harness's tracked background mechanism. Direct
...
. "$SCRIPT_DIR/n1-classify-lib.sh"

WATCH_LOCK="$STATE/.watch.lock"
WATCH_PATH="$SCRIPT_DIR/n1-watch.sh"
WATCHER_STALE_GRACE=${N1_WATCHER_STALE_GRACE:-${N1_GUARD_GRACE:-300}}
  ...
  if [ -n "${N1_LOCK_HELD_PID:-}" ]; then
    if [ -e "$BEAT" ]; then
      ...
      if [ "$beat_age" -ge "$WATCHER_STALE_GRACE" ]; then
        echo "watcher: lock held by live pid $N1_LOCK_HELD_PID but heartbeat is stale for ${beat_age}s (>${WATCHER_STALE_GRACE}s); inspect or stop that watcher before re-arming." >&2
        exit 1
      fi
    elif [ "$(fm_path_age "$WATCH_LOCK")" -ge "$WATCHER_STALE_GRACE" ]; then
      echo "watcher: lock held by live pid $N1_LOCK_HELD_PID but no heartbeat exists; inspect or stop that watcher before re-arming." >&2
      exit 1
    fi
    echo "watcher: already running pid $N1_LOCK_HELD_PID"
  else
...
WATCHER_PID=${BASHPID:-$$}
printf '%s\n' "$N1_HOME" > "$WATCH_LOCK/n1-home" || true
...
POLL=${N1_POLL:-15}                   # seconds between cycles
HEARTBEAT=${N1_HEARTBEAT:-600}        # base seconds between heartbeat scans
HEARTBEAT_MAX=${N1_HEARTBEAT_MAX:-7200}  # heartbeat backoff cap
CHECK_INTERVAL=${N1_CHECK_INTERVAL:-300}  # seconds between *.check.sh sweeps
CHECK_TIMEOUT=${N1_CHECK_TIMEOUT:-30}     # seconds allowed per *.check.sh
SIGNAL_GRACE=${N1_SIGNAL_GRACE:-30}   # seconds to linger after a signal so trailing
                                      # signals (a status write, then the same turn's
                                      # turn-end hook) coalesce into one wake
# Busy signatures per harness, OR-ed. Extend via env when new adapters are verified.
# claude/codex: "esc to interrupt"; opencode: "esc interrupt"; pi: "Working..."
BUSY_REGEX=${N1_BUSY_REGEX:-'esc (to )?interrupt|Working\.\.\.'}
...
# heartbeat). Rather than wake numberone's LLM for each, this watcher classifies
...
# (n1-classify-lib.sh) backs the away-mode daemon; while state/.afk exists the
# daemon owns triage, so this watcher reverts to one-shot (enqueue + exit on every
# wake) and never double-triages.
STALE_ESCALATE_SECS=${N1_STALE_ESCALATE_SECS:-240}  # idle secs before a non-terminal stale escalates as a possible wedge
TRIAGE_LOG="$STATE/.watch-triage.log"
TRIAGE_LOG_MAX_BYTES=${N1_WATCH_TRIAGE_LOG_MAX_BYTES:-262144}
...
# captain-relevant status line it SURFACED (woke numberone for) in
...
# fleet-scan can tell apart a captain-relevant status that already woke numberone
...
# any captain-relevant status has NOT already been surfaced to numberone (its
...
# numberone, this normally finds nothing and the heartbeat is absorbed; it
...
while :; do
  ...
  # Liveness beacon for n1-guard.sh: a fresh mtime here means a watcher is
  # alive. Supervision scripts warn when this goes stale with tasks in flight.
  touch "$STATE/.last-watcher-beat"

  # Slow per-task checks (numberone writes these, e.g. a merged-PR poll).
  # Time-based via .last-check mtime so the cadence survives watcher restarts.
  # Evaluated BEFORE the signal scan: wake() exits the cycle, so a check placed
  # after the signal scan would be starved whenever a chatty sibling ensign
  ...
  # classifying: a ensign's final status write and the same turn's turn-end
  # hook land seconds apart, and reporting them as separate actionable wakes
  # costs a full numberone turn each. The re-scan also picks up a newer
...
EOF
  ...
  # signature means the ensign finished, is waiting, or is wedged. Each distinct
  # stale hash is surfaced, absorbed, or timed toward escalation once (.stale-*
  # remembers the hash already classified).
  while IFS= read -r w; do
    # A lieutenant idling on its own watcher is healthy. Its parent supervises
    # it through status writes and heartbeats, not pane-idle staleness.
    [ "$(window_kind "$w")" = lieutenant ] && continue
    ...
    if [ "$h" = "$prev" ]; then
      ...
      if [ "$n" -ge 2 ] && ! printf '%s' "$tail40" | grep -v '^[[:space:]]*$' | tail -6 | grep -qiE "$BUSY_REGEX"; then
        ...
        # numberone. Detection itself is unchanged from above.
        if afk_present; then
