...
# watcher wake is captain-relevant (must reach numberone's LLM) or benign
# (absorbed in bash). Sourced by BOTH the always-on watcher (bin/n1-watch.sh)
# and the away-mode daemon (bin/n1-supervise-daemon.sh) so the triage policy
...
# it needs as arguments and touches no globals beyond the optional N1_CAPTAIN_RE
...
# numberone must see; everything else (working: notes, bare turn-ended) is
# benign. N1_CAPTAIN_RE overrides the whole set when a home needs a custom verb
# vocabulary; absent, this default applies.
N1_CLASSIFY_CAPTAIN_RE_DEFAULT='done:|needs-decision:|blocked:|failed:|PR ready|checks green|ready in branch|merged'
...
status_is_captain_relevant() {
  local line=$1
  [ -n "$line" ] || return 1
  printf '%s' "$line" | grep -qiE "${N1_CAPTAIN_RE:-$N1_CLASSIFY_CAPTAIN_RE_DEFAULT}"
}

# task id from a tmux window name "<session>:n1-<id>" -> "<id>"
window_to_task() {
  local w=$1 t
  t="${w##*:}"; t="${t#n1-}"; printf '%s' "$t"
}
