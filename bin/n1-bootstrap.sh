...
#                 "LIEUTENANT_SYNC: lieutenant <id>: skipped: <reason>",
#                 "NUDGE_LIEUTENANTS: <window-targets...>",
#                 "FMX: X mode on ..." or "FMX: X mode off ...".
#          A NUDGE_LIEUTENANTS line lists the RUNNING lieutenant windows whose
#          worktree was fast-forwarded to numberone's own current default-branch
#          commit (a purely LOCAL fast-forward, never an origin fetch) AND whose
#          instruction surface actually changed; numberone nudges each to re-read.
#          Already-current or no-instruction-change homes are silently left alone.
#          LIEUTENANT_SYNC lines report actionable skipped local-HEAD syncs for
#          live lieutenant homes; no-op/current and successful updates stay quiet.
#          A TANGLE line means the numberone primary checkout (N1_ROOT) is stranded
#          on a feature branch instead of its default branch - a ensign's work
...
#          X mode is OPTIONAL and inert unless N1_HOME/.env has a non-empty
#          FMX_PAIRING_TOKEN. When opted in, bootstrap requires curl+jq, writes
#          the relay poll shim and 30s cadence config, and prints an FMX line.
#          Fleet sync fetches, fast-forwards safe default-branch states, reports
#          recovered and STUCK clone drift, and prunes gone local branches; it is
#          bounded by N1_FLEET_SYNC_BOOTSTRAP_TIMEOUT, default 20s.
#          Set N1_FLEET_PRUNE=0 to skip branch pruning during that refresh.
#        n1-bootstrap.sh install <tool>...
...
CONFIG="${N1_CONFIG_OVERRIDE:-$N1_HOME/config}"
...
fleet_sync() {
  [ -x "$N1_ROOT/bin/n1-fleet-sync.sh" ] || return 0
  [ -d "$PROJECTS" ] || return 0

  tmp=$(mktemp "${TMPDIR:-/tmp}/n1-fleet-sync.XXXXXX" 2>/dev/null) || return 0
  monitor_was_on=0
  case $- in *m*) monitor_was_on=1 ;; esac
  set -m 2>/dev/null || true
  "$N1_ROOT/bin/n1-fleet-sync.sh" >"$tmp" 2>/dev/null &
  pid=$!

  timeout=${N1_FLEET_SYNC_BOOTSTRAP_TIMEOUT:-20}
...
lieutenant_sync() {
  # Local-HEAD lieutenant sync: fast-forward every LIVE lieutenant home's worktree
  # to the primary checkout's current default-branch commit. Purely LOCAL - no
  # fetch, no origin dependency: a lieutenant home is a worktree of this same repo
  # and already holds the primary's commit (n1-ff-lib.sh). Emits NUDGE_LIEUTENANTS:
  # only for RUNNING lieutenants whose instruction surface actually changed, so a
  # lieutenant already on the primary's version is never disturbed (AGENTS.md
  # bootstrap + supervision). Mirrors n1-update's nudge-lieutenants: report so
  # numberone can live-converge the listed windows.
  [ -d "$STATE" ] || return 0
  local primary_head
  if ! primary_head=$(primary_head_commit "$N1_ROOT"); then
    local meta id
    for meta in "$STATE"/*.meta; do
      [ -f "$meta" ] || continue
      grep -q '^kind=lieutenant' "$meta" 2>/dev/null || continue
      id=$(basename "$meta" .meta)
      echo "LIEUTENANT_SYNC: lieutenant $id: skipped: primary default-branch commit cannot be resolved"
    done
    return 0
  fi
  FF_NUDGE_WINDOWS=""
  FF_SEEN_HOMES=""
  local tmp line
  tmp=$(mktemp "${TMPDIR:-/tmp}/n1-lieutenant-sync.XXXXXX" 2>/dev/null) || return 0
  sweep_live_lieutenant_metas "$STATE" "$primary_head" yes >"$tmp"
  while IFS= read -r line; do
    case "$line" in
      lieutenant\ *': skipped:'*) echo "LIEUTENANT_SYNC: $line" ;;
    esac
  done < "$tmp"
  rm -f "$tmp"
  [ -n "$FF_NUDGE_WINDOWS" ] && echo "NUDGE_LIEUTENANTS:$FF_NUDGE_WINDOWS"
  return 0
}
...
# n1-watch.sh or any other watcher-backbone file. Drops two idempotent,
# gitignored artifacts:
#   state/x-watch.check.sh - check shim that execs bin/n1-x-poll.sh each cycle
#   config/x-mode.env      - exports N1_CHECK_INTERVAL=30, sourced by the watcher
...
x_mode_setup() {
  ...
  if [ -z "$token" ]; then
    ...
    if [ -e "$shim" ] || [ -e "$cadence" ]; then
      if x_mode_remove_artifacts; then
        echo "FMX: X mode off - removed relay poll shim and 30s cadence; restart the watcher (bin/n1-watch-arm.sh --restart) to drop back to the default cadence"
      else
  ...
  shim_body=$(cat <<EOF
#!/usr/bin/env bash
# Auto-generated by n1-bootstrap.sh - X mode connector poll shim.
# The watcher runs this each check cycle; output becomes a check: wake.
export N1_HOME=$(printf '%q' "$N1_HOME")
exec $(printf '%q' "$N1_ROOT/bin/n1-x-poll.sh")
EOF
)
...
# Auto-generated by n1-bootstrap.sh - X mode watcher cadence.
# Source this before arming the watcher (see AGENTS.md "X mode") so n1-watch.sh
# polls the X check every 30s. Non-X instances have no such file and keep the
# default 300s cadence.
export N1_CHECK_INTERVAL=30
EOF
)
...
if [ "${1:-}" = "install" ]; then
  shift
  [ $# -gt 0 ] || { echo "usage: n1-bootstrap.sh install <tool>..." >&2; exit 1; }
  for t in "$@"; do
...
# Worktree-tangle check: the numberone primary checkout (N1_ROOT) must sit on its
# default branch, not a feature branch (see n1-tangle-lib.sh). Scoped to the
# primary only; detached-HEAD worktrees and lieutenant homes never trip it.
tangle_branch=$(fm_primary_tangle_branch "$N1_ROOT" 2>/dev/null || true)
if [ -n "$tangle_branch" ]; then
  tangle_default=$(fm_default_branch "$N1_ROOT" 2>/dev/null || echo main)
  echo "TANGLE: primary checkout on feature branch '$tangle_branch' (expected '$tangle_default'); the work is safe on that ref - restore the primary with: git -C $N1_ROOT checkout $tangle_default, then re-validate the branch in a proper worktree"
fi
crew=
