#!/usr/bin/env bash
# n1-tmux-lib.sh — shared tmux pane primitives for numberone.
#
...
# daemon (bin/n1-supervise-daemon.sh) and bin/n1-send.sh so the composer/submit
...
# idle pane as holding pending input and the daemon deferred injection / numberone
...
# surfaced (n1-peek and every human/LLM-facing path stay plain), and only the
...
#
# Per-harness override: N1_COMPOSER_IDLE_RE matches an empty composer after
# dim-ghost and structural border stripping. N1_BUSY_REGEX overrides the busy
# footer set (mirrors n1-watch.sh / the daemon).
#
# All functions are `set -u` and `set -e` safe (guarded tmux calls, explicit
# returns) so they can be sourced into either context.

# Busy footers per harness (mirror n1-watch.sh). claude/codex: "esc to
# interrupt"; opencode: "esc interrupt"; pi: "Working...".
N1_TMUX_BUSY_REGEX_DEFAULT='esc (to )?interrupt|Working\.\.\.'
...
fm_tmux_composer_state() {  # <target> -> empty|pending|unknown
  ...
  if [ -n "${N1_COMPOSER_IDLE_RE:-}" ] \
     && printf '%s' "$stripped" | grep -qiE "$N1_COMPOSER_IDLE_RE"; then
  ...
  if printf '%s' "$stripped" | grep -qiE "${N1_BUSY_REGEX:-$N1_TMUX_BUSY_REGEX_DEFAULT}"; then
    printf 'empty'; return 0
  fi
...
# (an agent mid-turn). Scans a 40-line tail like n1-watch.sh.
fm_pane_is_busy() {  # <target>
  local win=$1 tail40
  tail40=$(tmux capture-pane -p -t "$win" -S -40 2>/dev/null) || return 1
  printf '%s' "$tail40" | grep -v '^[[:space:]]*$' | tail -6 \
    | grep -qiE "${N1_BUSY_REGEX:-$N1_TMUX_BUSY_REGEX_DEFAULT}"
}
...
#   - n1-send fails only on "pending" (lenient: a positively-confirmed swallow),
