#!/usr/bin/env bash
# Post numberone's composed answer back to the relay for a pending X mention.
#
# Usage: n1-x-reply.sh <request_id> <text>
#        n1-x-reply.sh <request_id> --text-file <path>   # read the reply from a file
#        n1-x-reply.sh <request_id> -                    # read the reply from stdin
#        n1-x-reply.sh <request_id> --followup ...       # post a completion follow-up
#
...
usage() {
  echo "usage: n1-x-reply.sh <request_id> [--followup] <text> | [--followup] --text-file <path> | [--followup] -" >&2
}
...
case "$1" in
  --text-file)
    if [ "$#" -lt 2 ]; then
      echo "usage: n1-x-reply.sh <request_id> [--followup] --text-file <path>" >&2
      exit 2
    fi
    TEXT=$(cat -- "$2") || { echo "n1-x-reply: cannot read text file: $2" >&2; exit 1; }
    ;;
  -)
    TEXT=$(cat)
    ;;
...
if [ -z "$TEXT" ]; then
  echo "n1-x-reply: empty reply text" >&2
  exit 2
fi
...
case "$REQ" in
  ''|.*|*[!A-Za-z0-9._-]*) echo "n1-x-reply: unsafe request_id: $REQ" >&2; exit 2 ;;
esac

command -v jq >/dev/null 2>&1 || { echo "n1-x-reply: jq not found" >&2; exit 1; }
...
CHUNKS=$(printf '%s' "$TEXT" | fmx_split_thread "$FMX_MAX" "$FMX_THREAD_MAX") || {
  echo "n1-x-reply: failed to split reply into a thread" >&2
  exit 1
}
N=$(printf '%s' "$CHUNKS" | jq 'length' 2>/dev/null) || N=
case "$N" in ''|*[!0-9]*) echo "n1-x-reply: failed to split reply into a thread" >&2; exit 1 ;; esac
[ "$N" -gt 0 ] || { echo "n1-x-reply: empty reply text" >&2; exit 2; }
...
if [ "$N" -le 1 ]; then
  PAYLOAD=$(printf '%s' "$CHUNKS" | jq -c --arg rid "$REQ" '{request_id:$rid, text:(.[0] // "")}') || {
    echo "n1-x-reply: failed to build request payload" >&2; exit 1; }
else
...
if [ -n "$FMX_DRY" ]; then
  ...
  mkdir -p "$outbox_dir" 2>/dev/null || {
    echo "n1-x-reply: cannot create dry-run outbox: $outbox_dir" >&2
    exit 1
  }
  ...
  if [ "$FOLLOWUP" = 1 ]; then
    OUTREC=$(printf '%s' "$PAYLOAD" | jq -c '. + {endpoint:"followup"}') || {
      echo "n1-x-reply: failed to build dry-run outbox record" >&2; exit 1; }
  else
    OUTREC=$PAYLOAD
  fi
  printf '%s\n' "$OUTREC" > "$outbox_file" 2>/dev/null || {
    echo "n1-x-reply: cannot write dry-run outbox: $outbox_file" >&2
    exit 1
  }
  if [ "$N" -le 1 ]; then
    printf 'n1-x-reply: DRY RUN - would POST to %s/connector/%s (recorded: state/x-outbox/%s.json): %s\n' \
      "$FMX_RELAY" "$ENDPOINT" "$REQ" "$(printf '%s' "$CHUNKS" | jq -r '.[0]')" >&2
  else
    printf 'n1-x-reply: DRY RUN - would POST a %s-tweet thread to %s/connector/%s (recorded: state/x-outbox/%s.json):\n' \
...
if [ -z "$FMX_TOKEN" ]; then
  echo "n1-x-reply: X mode not configured (no FMX_PAIRING_TOKEN)" >&2
  exit 1
fi
command -v curl >/dev/null 2>&1 || { echo "n1-x-reply: curl not found" >&2; exit 1; }
AUTH_HEADER_FILE=$(fmx_auth_header_file) || {
  echo "n1-x-reply: invalid FMX_PAIRING_TOKEN" >&2
  exit 1
}
...
code=$(curl -m 10 -s -o /dev/null -w '%{http_code}' \
  ...
  echo "n1-x-reply: request to relay failed" >&2
  exit 1
}

case "$code" in
  2[0-9][0-9]) printf '%s\n' "$REQ" ;;
  *) echo "n1-x-reply: relay returned HTTP $code" >&2; exit 1 ;;
esac
