#!/usr/bin/env bash
# Launch a claude ensign INSIDE a container, with only the worktree, the
# numberone communication dir (state/), the task's data dir (brief + report),
# and the project's shared git object store mounted in. This bounds an
# autonomous (--dangerously-skip-permissions) ensign's blast radius to those
# mounts instead of the whole host.
#
# Usage (typed into the ensign's tmux pane by n1-spawn.sh's launch template):
#   n1-crew-container.sh <worktree> <brief> <turnend>
#
# Runs ATTACHED (-it) so the pane is a transparent passthrough to the container
# TTY: n1-send.sh's `tmux send-keys` (steer/interrupt) and n1-watch.sh's
...
# (e.g. numberone sends /exit) the container exits and the pane returns.
#
# Tunables (env):
#   N1_CREW_IMAGE  container image (default numberone-crew:latest)
#   N1_CREW_NET    docker network (default bridge); set to an egress-allowlisted
#                  network for tighter exfiltration control
set -eu

WT=$1       # treehouse worktree: the ensign's working copy
...
IMAGE=${N1_CREW_IMAGE:-numberone-crew:latest}
NET=${N1_CREW_NET:-bridge}
...
# data/<id> holds the brief (read) and, for surveys, report.md (written) -> rw.
TASK_DIR=$(cd "$(dirname "$BRIEF")" && pwd -P)
# Read the brief inside the container via its RESOLVED path: TASK_DIR is mounted
# at its pwd -P path, so N1_BRIEF must use the same (on macOS /tmp -> /private/tmp,
...
# are only needed by mission/direct-PR tasks (push + PR); a survey needs neither.
...
CRED_TMP=$(mktemp -d "${TMPDIR:-/tmp}/n1-crew-cred.XXXXXX")
...
# Permissions warning, which numberone accepts at spawn (see harness-adapters).
...
# ANTHROPIC_API_KEY in the numberone env from hijacking a subscription login:
...
if [ -f "$HOME/.claude/.credentials.json" ]; then
  ...
  echo "n1-crew-container: no Claude credentials found (log claude in on the" \
...
# N1_BRIEF + cat-inside-container: the brief is read in the container shell, so
...
# ensign exits; when claude exits, docker exits, and the pane returns.
docker run --rm -it \
  --network "$NET" \
  -w "$WT_REAL" \
  -e CLAUDE_CODE_ENABLE_PROMPT_SUGGESTION=false \
  -e N1_BRIEF="$BRIEF_REAL" \
  -v "$WT_REAL:$WT_REAL" \
  ...
  "${CRED[@]}" \
  "$IMAGE" \
  bash -lc 'exec claude --dangerously-skip-permissions "$(cat "$N1_BRIEF")"'
