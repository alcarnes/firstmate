---
name: fmx-respond
description: Agent-only playbook for handling an X mention in X mode. Use on an "x-mention <request_id>" check: wake - read the stashed mention (with any in_reply_to conversation context); the direct author is the numberone's own owner (captain) under owner-only routing, so classify it as an actionable request to act on through the normal lifecycle, a question to answer from live fleet state, or a pure acknowledgment to dismiss without replying; act autonomously (escalating only destructive/irreversible/security-sensitive work). For a request that spawns real work, acknowledge first, act, link the task with bin/n1-x-link.sh, and let the completion follow-up post on the done wake; for a question or completed action, post or preview a short public-safe reply with bin/n1-x-reply.sh; for a pure acknowledgment, call bin/n1-x-dismiss.sh. Clear the inbox file only after a successful reply or dismiss. Loaded only when X mode is enabled.
user-invocable: false
---

# fmx-respond

X mode lets a numberone instance answer and act on public mentions of the shared `@myfirstmate` bot on X.
...
The myfirstmate relay uses **owner-only routing**: it wakes a numberone only for *that numberone's own owner's* mentions.
...
Because the author is the captain, a mention that asks for work - "add this to the backlog", "look into X", "fix Y", "mission Z" - is a **real captain instruction**, exactly as if the captain had typed it into their own session.
Acting on it means running numberone's **normal lifecycle**: intake to resolve the project, then file the backlog item, dispatch a ensign, start an investigation, or mission through the gate - whatever the request calls for.
...
- **Work that spawns a real, longer-running job** (dispatching a ensign, a survey investigation, a mission task) cannot report an outcome yet, so it follows **acknowledge first -> act -> follow up on completion**:
  1. **Acknowledge first.** Post an immediate, public-safe reply that you have the captain's order and are on it (the normal answer endpoint, via `bin/n1-x-reply.sh`). This is the legitimate, work-backed version of "aye, will do": it is paired with actually starting the work in the same turn, never a promise left empty.
  2. **Act.** Dispatch the work through the normal lifecycle right away.
  3. **Link it for the follow-up.** Associate the spawned task with this mention so the completion follow-up can be posted later: `bin/n1-x-link.sh <task-id> <request_id>` (records the request id and a timestamp in the task's state). Do this right after the task is spawned.
  4. **Follow up on completion.** When that task reaches a terminal state (delivered / reported / merged / failed), numberone posts **one** follow-up reply - "done, here's the result" - within a 24h window, then the link clears. That post happens on the task's completion wake, driven by AGENTS.md section 14, not this turn.
...
- **Pure acknowledgment** ("thanks", a reaction, a loop-closing nicety with nothing to add) - skip: post nothing, but first **dismiss it at the relay** (`bin/n1-x-dismiss.sh <request_id>`) so the relay drops the request and stops re-offering it, then clear the inbox file.
...
Normal reversible work - filing backlog, a survey investigation, gated code changes, dispatching a ensign - proceeds autonomously under the standing X-mode authorization.
...
- Tooling/internal vocabulary: ensign, survey, mission, lieutenant, harness names, watcher, heartbeat, brief, teardown, no-mistakes, yolo, delivery modes.
...
Speak only in **outcomes**: what is being built, fixed, looked into, or delivered, described the way you would to an outsider.
...
Reply in numberone's own voice - the crisp, lightly nautical first-mate persona - but **public-facing**:
...
Compose the reply as one piece of prose; if it is genuinely too long for one tweet, `bin/n1-x-reply.sh` automatically splits it into a numbered thread on word boundaries.
...
2. **Drain every pending mention.** For each `state/x-inbox/*.json` file:
   ...
   c. **Act on an actionable request through the normal lifecycle.** Treat it exactly as a captain prompt typed in session: run ordinary intake (resolve the project), then file the backlog item, dispatch a ensign, start a survey, or mission through the gate - whatever the request calls for.
   ...
   e. **Submit it without ever inlining the reply into a shell command.**
      ...
      ```sh
      bin/n1-x-reply.sh <request_id> --text-file <path-to-reply-file>
      ```

      (`bin/n1-x-reply.sh <request_id> -`, reading the reply on stdin, is equally fine.) It echoes the `request_id` and exits 0 on success; non-zero on a failed live post or failed dry-run record.
   e-skip. **For a skip, dismiss it at the relay instead of replying.** A pure acknowledgment gets no reply, but clearing only the local inbox file is not enough: the relay keeps re-offering that request on every poll until it times out to a polite "offline" auto-reply. So before clearing the file, tell the relay to drop the request:
      ...
      ```sh
      bin/n1-x-dismiss.sh <request_id>
      ```

      It posts nothing, stops the re-offer, and prevents the offline auto-reply; it echoes the `request_id` and exits 0 on success (it honors `FMX_DRY_RUN` like `bin/n1-x-reply.sh`, recording the would-be dismiss to `state/x-outbox/` instead of posting). Do **not** call `bin/n1-x-reply.sh` for a skip.
   ...
   g. **On failure** (a non-zero exit from `bin/n1-x-reply.sh` or `bin/n1-x-dismiss.sh`), leave that inbox file in place, move on to the next, and do not retry blindly.
      If you had already acted on this mention in step 2c before the post failed, do **not** redo that work on a later drain - check whether it is already done (e.g. the backlog item exists, the ensign is already running) and only retry the reply.
...
When `FMX_DRY_RUN` is set (truthy, in the environment or `.env`), `bin/n1-x-reply.sh` does **not** post and `bin/n1-x-dismiss.sh` does **not** call the relay.
...
Your procedure does not change: compose as usual and call `bin/n1-x-reply.sh ... --text-file <path>`, or call `bin/n1-x-dismiss.sh <request_id>` for a skip.
...
The completion follow-up honors `FMX_DRY_RUN` the same way (it flows through `bin/n1-x-reply.sh --followup`): the would-be follow-up is recorded to `state/x-outbox/` and the link is cleared exactly as a live post would clear it, so the whole acknowledge -> act -> follow-up loop is testable without a public tweet.
...
That post is numberone's job on the task's completion wake and is governed by AGENTS.md §14; this skill's only follow-up responsibility is linking the task in step 2c.
For context, the completion path is:

- On a terminal wake (PR merged / survey report / local merge / failed), numberone checks whether the task is X-linked with `bin/n1-x-followup.sh --check <task-id>` (prints the `request_id` when a follow-up is due; silent when not linked or past the 24h window, pruning an expired link).
- If due, it composes a short, public-safe outcome ("done, here's the result"; for a failure, an honest "this one didn't pan out") and posts the single follow-up with `bin/n1-x-followup.sh <task-id> --text-file <path>` (or stdin), which posts via the relay's follow-up endpoint and clears the link on success.
...
- An actionable mention is **acted on** through the normal lifecycle (intake, backlog, dispatch, investigate, mission), not merely replied to. Work that finishes now gets one outcome reply; work that spawns a real task gets an **acknowledgement now** plus a single **completion follow-up** later (link the task with `bin/n1-x-link.sh` so that follow-up can post). A reply alone, with no work behind an actionable ask, is the bug to avoid.
...
- One answered mention = one reply (plus at most one completion follow-up for a spawned task); a skipped mention posts no reply but is **dismissed at the relay** (`bin/n1-x-dismiss.sh`) so the relay drops it rather than re-offering it (which would otherwise churn every poll and end in an "offline" auto-reply). A single wake may cover several pending mentions - drain them all.
...
- Never edit `bin/n1-x-poll.sh`, `bin/n1-x-reply.sh`, or the watcher to "answer faster"; the cadence is handled in bootstrap.
